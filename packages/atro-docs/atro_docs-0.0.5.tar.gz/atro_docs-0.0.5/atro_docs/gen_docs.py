...
class DocsGenerator(BaseModel, ABC):
  ...
  def get_files_content(self, file_path: Path, doc_aut_get_prefix: str) -> str:
    ...
    for file_data_path in files_to_include_in_doc_file:
      content.append(f"### {file_data_path.relative_to(file_path).as_posix()}")

      match (file_data_path.suffix):
        case ".yaml":
          content.append(self.get_yaml_file_snippet(file_data_path, file_path))
        case ".md":
          content.append(self.get_md_file_snippet(file_data_path, file_path))
        case _:
  ...
  def get_readme_content(self, file_path: Path, doc_aut_get_prefix: str) -> str:
    ...
    for par_dir in par_dirs:
      if (par_dir / "README.md").exists():
        content.append(self.get_md_file_snippet(par_dir / "README.md", par_dir))
  ...
  def get_yaml_file_snippet(self , yaml_file: Path, file_path: Path) -> str:
    content = []
    content.append("~~~yaml")
    content.append('{% include "' +  self.get_relative_path(yaml_file, file_path) + '" %}')
    content.append("~~~")
    
    return "\n".join(content)

  def get_relative_prefix(self, file_path: Path) -> str:
    depth = len(file_path.parts)  + self.base_depth 
    return "../" * depth + f"repos/{self.repo_name}/"
  
  def get_relative_path(self, file_data_path: Path, file_path: Path) -> str:
    relative_prefix = self.get_relative_prefix(file_path)
    relative_path = f"{relative_prefix}{file_data_path.relative_to(self.base_dir)}"

    return relative_path
  
  def get_md_file_snippet(self, md_file: Path, file_path: Path) -> str:
    output = []
    
    if md_file.name == "Dockerfile":
      output.append("```Dockerfile")
      output.append('{% include "' +  self.get_relative_path(md_file, file_path) + '" %}')
      output.append("```")
    else:
      output.append('{% include-markdown "' +  self.get_relative_path(md_file, file_path) + '" %}')
