...
import logging 
from constants import MKDOCS_INFO_LINE_START, MKDOCS_WARNING_LINE_START, MKDOCS_ERROR_LINE_START
...
def process_output(line):
    ...
    elif line.startswith(MKDOCS_ERROR_LINE_START):
      print_combined_text()
      latest_line_start = LineStartTypes.Error
...
def print_combined_text():
    ...
    match (latest_line_start):
      ...
      case LineStartTypes.Error:
        combined_text = combined_text.strip(MKDOCS_ERROR_LINE_START)
      case _:
          pass
...
def run_mkdocs(docs_parent_dir: Path, bg: bool = False):
  ...
  output = mkdocs("serve", "--dev-addr=0.0.0.0:8000", _cwd=docs_parent_dir.as_posix(), _out=process_output, _err=process_output, _bg=bg)
  output.wait()

#run_mkdocs(Path("/tmp/tmplhwqqaux/"))