...
from inverse_websockify import Proxy
...
class OceanSparkSession(RemoteSparkSession):
    class Builder(RemoteSparkSession.Builder):
        ...
        _session: RemoteSparkSession = None
        ...
        def getOrCreate(self):
            ...
            self.config("spark.remote", url)
            self._session = super().getOrCreate()
            self._session.oldStop = self._session.stop
            self._session.stop = self.stop
            return self._session


if __name__ == "__main__":
    spark = OceanSparkSession.Builder().cluster_id("osc-239fd6f0").appid("spark-connect-343e1-unity").profile(
        "default").config("spark.jars.ivy", "/tmp").getOrCreate()
