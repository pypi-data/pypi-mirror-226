...
class UnitRequest:
    ...
    def is_assert_compare_status(res):
        ...
        try:
            ...
            url = "http://127.0.0.1:8000/api" + "/apilog/add_api_log/"
            try:
            ...
            data = {"url": res.url, "method": res.request.method,
                    "headers": get_rsp(res.request.headers), "req_time": res.run_time,
                    "req_code": str(res.status_code),
                    "req": req,
                    "data": body}
            out = requests.post(url, json=data)
            out = out.json()
            return out.get("compare_status"), out.get("compare_status")
        except Exception as e:
