...
def _fields(schema: str):
  # TODO: likely breaks when schema has NUMERIC(2,0)
  return [re.split('\s+', f.strip())[0] for f in schema.split(',')]
...
def _build_select_statement(
    input_component_id,
    input_schema: pb2.ComponentSchemaResponse.Schema,
    expand: bool,
):
  if expand:
    fields = '  ' + ',\n  '.join(_fields(input_schema.schema))
    return f"SELECT\n{fields}\nFROM {{{{{input_component_id}}}}}"
  else:
    return f"SELECT * FROM {{{{{input_component_id}}}}}"


def _build_transform_union(data_service_type: str, input_components: List[pb2.Transform], sql: str):
  union_transform = definitions.Transform(
      id=f'{input_components[0].id}_Union_Transform',
      name=f'{input_components[0].name} Union Transform',
      description=f"Merge from components '{[f'{c.name} ({c.id})' for c in input_components]} created with CLI at {datetime.datetime.now()}",
      input_ids=[c.id for c in input_components],
      operator=operator.Operator(spark_function=operator.Spark.Function(
          executable=io.Executable(code=io.Code(
              language=_language_factory(data_service_type),
              source=io.Code.Source(inline=sql),
          ), ),
          reduction=operator.Reduction(full=operator.Reduction.Full(), ),
          tests=function.QualityTests(),
      ), ),
  ...
  id_map = {in_c.id: definitions.ComponentUuidType(type=in_c.type, uuid=in_c.uuid) for in_c in input_components}
  return union_transform, id_map
...
def union(
    ...
    data_service_id: str = typer.Argument(..., help='Data Service id', show_default=False),
    dataflow_id: str = typer.Argument(..., help='Dataflow id', show_default=False),
    component_ids: List[str] = typer.Argument(..., help='Transformation component ids to merge from (requires at least two)', show_default=False),
    union_all: bool = typer.Option(True, help='Use UNION ALL or UNION', show_default=True),
    expand: bool = typer.Option(True, help="Expand table schemas into full SELECT statements or use '*'"),
):
  """Build a basic UNION command between two or more components."""
  client = get_client(ctx)

  data_service = client.get_data_service(data_service_id).data
  components = client.list_dataflow_components(data_service_id, dataflow_id, kind='source,view').data

  upstream_components = [c for c in components if c.id in component_ids]

  if not upstream_components or len(upstream_components) < 2:
    raise Exception(f'At least two upstream components are required: {component_ids}')

  # get the schema information in the string (easier)
  schemas = {}
  for upstream in upstream_components:
    schema_method = getattr(client, GET_SCHEMA_METHODS[upstream.type])
    upstream_schema = schema_method(data_service_id, dataflow_id, upstream.id).data
    if not upstream_schema:
      raise Exception(f'Schema for component {data_service_id}.{dataflow_id}.{upstream.id} not found.')
    schemas[upstream.id] = upstream_schema

  # build the SQL statement
  sql = f'\nUNION{" ALL" if union_all else ""}\n'.join([_build_select_statement(k, v, expand) for (k, v) in schemas.items()])

  union_transform, id_map = _build_transform_union(data_service.data_service_type, upstream_components, sql)

  # actually send the transform
  resp = client.create_transform(data_service_id, dataflow_id, union_transform.to_proto(id_map))
  print_response(ctx, resp.data)
