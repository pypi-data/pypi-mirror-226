...
Output a dict with all the parameters needed to process experiments
"""
...
def parseExptConfig(configFile, librariesToSublibrariesDict):
    ...
    if parser.has_option('growth_values', 'growth_value_string') and len(
        ...
        # because we enforced no duplicates or unexpected values these should match up unless there were values not
        # entered require all growth values to be explicitly entered if some were
...
def parseLibraryConfig(libConfigFile):
    """Parse the library configuration file to get the available libraries, sub-libraries, and
    corresponding library table files
