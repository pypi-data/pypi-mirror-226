"""pipeline to generate read counts and phenotype scores
Directly handel sequencing data, gzipped FASTQ files
V2 library
"""
...
def parallelSeqFileToCounts(
        fastqGzFileNameList, fastaFileNameList, countFileNameList, processPool, libraryFasta,
...
def seqFileToCounts(
        infileName, fastaFileName, countFileName, libraryFasta,
        startIndex=None, stopIndex=None, test=False
):
    """
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('Seq_File_Names', nargs='+',
                        help='Name(s) of sequencing file(s).'
                             ...
                             'The script will search for all *.fastq.gz, *.fastq, and *.fa(/fasta/fna) files')
    ...
    parser.add_argument('--test', action='store_true', default=False,
                        help='Run the entire script on only the first %d reads of each file.'
                             'Be sure to delete or move all test files before re-running script as '
                             'they will not be overwritten.' % testLines)
    ...
    try:
        resultList = parallelSeqFileToCounts(infileList, fastaFilePathList, countFilePathList, pool,
