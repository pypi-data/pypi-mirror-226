...
_diamond_columns= 'qseqid sseqid qlen slen length bitscore'.split()
...
def diamond_blastp(diamond, fpath, f_reference, f_out, quiet= False):
    command= [diamond, 'blastp', '-d',
              f_reference, '-q', fpath,
              '-o', f_out, '-f', '6'] + _diamond_columns
    if quiet:
        command+= ['--quiet']
    return run(command)
