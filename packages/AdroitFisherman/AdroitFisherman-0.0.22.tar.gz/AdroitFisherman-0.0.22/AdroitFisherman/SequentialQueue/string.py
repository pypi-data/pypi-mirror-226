import ctypes
class SequentialQueue_string:
    def __init__(self):
        self.__queue=ctypes.cdll.LoadLibrary('./Libraries/SequentialQueue_string')
    ...
    def get_head(self):
        ...
        get_head.restype=ctypes.c_char_p
        return ctypes.c_char_p(get_head()).value
    def en_queue(self,elem):
        en_queue=self.__queue.en_queue
        en_queue.argtypes=[ctypes.c_char_p]
        en_queue.restype=ctypes.c_bool
        return en_queue(elem)
    def de_queue(self):
        de_queue=self.__queue.de_queue
        de_queue.restype=ctypes.c_char_p
        return ctypes.c_char_p(de_queue()).value
