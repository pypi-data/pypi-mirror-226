...
class SequentialStack_float:
    def __init__(self):
        self.__stack = ctypes.cdll.LoadLibrary('./Libraries/SequentialStack_float')
    ...
    def get_top(self):
        ...
        get_top.restype = ctypes.c_float
        return get_top()

    def push(self, elem):
        push = self.__stack.push
        push.argtypes = [ctypes.c_float]
        push.restype = ctypes.c_bool
        return push(elem)

    def pop(self):
        pop = self.__stack.pop
        pop.restype = ctypes.c_float
        return pop()
