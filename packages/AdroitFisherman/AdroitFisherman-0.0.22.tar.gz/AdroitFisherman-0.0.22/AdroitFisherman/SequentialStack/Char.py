import ctypes


class SequentialStack_char:
    def __init__(self):
        self.__stack = ctypes.cdll.LoadLibrary('./Libraries/SequentialStack_char')

    def init_stack(self, length):
        init_stack = self.__stack.init_stack
        init_stack.argtypes = [ctypes.c_int]
        init_stack.restype = ctypes.c_bool
        return init_stack(length)

    def destroy_stack(self):
        destroy_stack = self.__stack.destroy_stack
        destroy_stack()

    def clear_stack(self):
        clear_stack = self.__stack.clear_stack
        clear_stack()

    def stack_empty(self):
        stack_empty = self.__stack.stack_empty
        stack_empty.restype = ctypes.c_bool
        return stack_empty()

    def stack_length(self):
        stack_length = self.__stack.stack_length
        stack_length.restype = ctypes.c_int
        return stack_length()

    def get_top(self):
        get_top = self.__stack.get_top
        get_top.restype = ctypes.c_char
        return ctypes.c_char(get_top()).value

    def push(self, elem):
        push = self.__stack.push
        push.argtypes = [ctypes.c_char]
        push.restype = ctypes.c_bool
        return push(ctypes.c_char(bytes(elem, encoding='utf-8')))

    def pop(self):
        pop = self.__stack.pop
        pop.restype = ctypes.c_char
        return ctypes.c_char(pop()).value

    def stack_traverse(self):
        stack_traverse = self.__stack.stack_traverse
        stack_traverse()
