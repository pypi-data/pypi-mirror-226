...
class LoggerSettings(BaseModel):
    level = logging.getLevelName(os.getenv("settings.level", "DEBUG"))
    serialize: bool = os.getenv("LOG_SERIALIZE", False)
    enqueue: bool = os.getenv("LOG_ENQUEUE", True)
...
def init_logger() -> None:
    ...
    # sys.stderr
    handlers = [{"sink": 'test.log', "level": settings.level, "serialize": settings.serialize}]
    logger.configure(
        handlers=handlers + [{"sink": sys.stdout, "level": settings.level, "serialize": settings.serialize}]
    )
