...
def _ketos_run_output_function(batch_detections, output_folder, mode, threshold=0.5, merge_detections=False, buffer=None, running_avg=None, labels=None, **output_function_arguments):
    ...
    if batch_detections['score'].ndim == 2: # snapshot
        batch_detections = filter_by_threshold(batch_detections, threshold)
    else: # sequence model
...
def ketos_run(model_file, audio_data, file_list=None, table_name=None, output_folder=None, overwrite=True, log_file="ketos-run.log", step_size=None, batch_size=32, threshold=0.5, labels=None, merge_detections=False, buffer=None, running_avg=None, output_function_arguments=None):
    """ 
    ...
    Args:
        ...
        audio_data: str
            Path to the audio data to be processed. Can be a single file, a list of files, or a directory.
        file_list: str
    ...
    audio_representation = audio_representation[0] # Currently the script only works with one audio representation
    ...
    if audio_path.is_file() and audio_path.suffix == '.h5':
        # is hdf5
        batch_generator = lambda: batch_load_hdf5_data(hdf5_file_path=audio_path, batch_size=batch_size, audio_representation=audio_representation, table_name=table_name)
        ...
        batch_generator = lambda: batch_load_audio_file_data(loader=loader, batch_size=batch_size, logger=logger)
    ...
    try:
        ...
        for idx, batch_data in enumerate(batch_generator()):
            successful_process = False
            # If the batch was processed before interruption, skip it
            if idx < last_processed_batch:
                continue
            try:
                ...
                output_function(raw_output, output_folder, mode, threshold=threshold, labels=labels, merge_detections=merge_detections, buffer=buffer, running_avg=running_avg, **output_function_arguments)
...
def main():
    ...
    parser.add_argument('audio_data', type=str, help='Path to either a folder with audio files or an HDF5 database file with the data to process.')
    ...
    try:
        ...
        # Given that ketos_run() already handles its own exceptions, this might never be executed.
