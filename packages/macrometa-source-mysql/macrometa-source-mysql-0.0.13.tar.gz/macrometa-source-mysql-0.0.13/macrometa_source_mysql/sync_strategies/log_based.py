...
def _run_binlog_sync(
    ...
    for binlog_event in reader:
        ...
        if (log_file > end_log_file) or (end_log_file == log_file and log_pos >= end_log_pos):
            ...
            # Removing the 'break' statement to continuously check for binlog changes.
            # break
            # Instead, sleep for 1 second before the next iteration to avoid high CPU usage
            time.sleep(2)
...
def sync_binlog_stream(
    ...
    """
    LOGGER.info(f"*** inside: sync_binlog_stream")
