...
USER_AGENT = f"python-datalama/{__version__}"


class BaseClient:
    def __init__(self, token: Optional[str] = None, timeout: Optional[float] = 10):
        if token is None:
            token = os.getenv("DATALAMA_TOKEN")
        assert (
            token is not None
        ), "Token not found. Use Client(token='<token>') or set env DATALAMA_TOKEN=<token>"
        self._url = "https://api.datalama.io{}"
        self._token = token
