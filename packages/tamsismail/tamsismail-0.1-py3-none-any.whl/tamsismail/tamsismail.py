...
def inbox(session,mail):
        ...
        try:
            ...
            mail = par(ses.get("https://m.kuku.lu/recv._ajax.php",params=param).text,"html.parser")
            for g in mail.find_all("script"):                
                if "openMailData" in str(g):                    
                    if main == None:
                            ...
                            pesan = par("<div"+str(re.search('<head></head><div(.*?)</table></div>',str(masuk)).group(1))+"</table></div>","html.parser")                   
                            res = {"text":str(re.sub(r'\s+', ' ', masuk.text)),"html":pesan}
                            break                    
                    else:                        
                        if mail in str(g):
                            dat = re.search("openMailData(\(.*?)\)",str(g)).group(1).split("', '")
