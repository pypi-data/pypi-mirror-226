...
def _download_to_local(url, file_name, path=None, overwrite=False, cache_path=None, allow_cache=None) -> str:
    ...
    if path is not None:
        path_file = os.path.join(path.replace('\\', '/'), file_name)
        print(f'h path_file:{path_file}')
    if allow_cache is True:
        if cache_path is None:
            cache_path = model_cache_path
        cache_path_file = os.path.join(cache_path.replace('\\', '/'), file_name)
        print(f'h cache_path_file:{cache_path_file}')
