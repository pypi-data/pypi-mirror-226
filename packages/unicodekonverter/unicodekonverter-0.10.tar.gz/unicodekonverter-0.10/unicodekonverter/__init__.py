...
from baumregextrie import Trie


def load_regexfile():
    regecom = get_filepath("compiledregexunicodepickle.pkl")
    with open(regecom, mode="rb") as f:
        compiledregex = pickle.loads(f.read())
    return compiledregex


def create_regex_file():
    Tr = Trie()
    Tr.regex_from_words(
        list(
            set(["u+" + hex(q)[2:].zfill(4) for q in range(20, 0x1FFFF + 1)]).union(
                set(["u+" + hex(q)[2:] for q in range(20, 0x1FFFF + 1)])
            )
        ),
    ).compile(
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "compiledregexunicodepickle.pkl"),
        mode="wb",
    ) as f:
        f.write(pickle.dumps(Tr.compiled_regex))
    return load_regexfile()


try:
    compiledregex = load_regexfile()
except Exception:
    print("Generating regex (only once)")
    compiledregex = create_regex_file()


@cache
def convert_letter(t):
    co = 0
    while t:
        try:
            t = chr(int(t, base=16))
...
def convert_2_unicode(tex: str, concurrent: int | None = None) -> str:
    r"""
    Convert Hexadecimal Unicode Codes with the prefix 'U+/u+' in text to Corresponding Characters

    This function takes a text containing hexadecimal Unicode codes and replaces them with their corresponding
    characters. It utilizes a precompiled regex pattern (compiled at the first run) to efficiently
    ...
    """
    text = array.array("u", tex)
    replace = "\x7F"
    for q in compiledregex.finditer(tex, concurrent=concurrent):
        qsp0, qsp1 = q.span()
        try:
            text[qsp1 - 1] = convert_letter(q.group(1))
            for ch in range(qsp0 - 1, qsp1 - 1):
                text[ch] = replace

        except Exception:
            pass
    text = text.tounicode().replace(replace, "")
    return text
