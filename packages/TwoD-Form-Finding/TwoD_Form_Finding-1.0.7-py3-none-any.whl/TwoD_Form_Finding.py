...
class TwoDShapeFinding:
        ...
        if init_fr_sap:
            try:
                mySapObject = (
                    ...
                    .GetObject("CSI.SAP2000.API.SapObject")
                )
    ...
    def init_fr_sap2000(
        ...
        *args,
        ...
        self.SapModel.PropMaterial.AddMaterial(
            "mat_default", sap_mat_ID, Country, code, mat_type, "mat_default"
        )
        self.SapModel.PropFrame.SetCircle("sec_default", "mat_default", sec_dia)
        ...
        if sap_mat_ID == 7:
            Young, thermal = self.SapModel.PropMaterial.GetMPUniaxial("mat_default")[
                :-1
            ]
        ...
        for gp in args:
            ...
            self.SapModel.PropMaterial.AddMaterial(
                "mat_{}".format(gp1), sm1, Cty1, cd1, mt1, "mat_{}".format(gp1)
            )
            self.SapModel.PropFrame.SetCircle(
                "sec__{}".format(gp1), "mat_{}".format(gp1), sd1
            )
            if sm1 == 7:
                Y, th = self.SapModel.PropMaterial.GetMPUniaxial("mat_{}".format(gp1))[
                    :-1
                ]
            elif sm1 == 2 or sm1 == 1:
                ret = self.SapModel.PropMaterial.GetMPIsotropic("mat_{}".format(gp1))[
                    :-2
                ]
                Y = ret[0]
                th = ret[2]
            sa = self.SapModel.PropFrame.GetSectProps("sec__{}".format(gp1))[0]
            for a in frames_gp:
    ...
    def force_density(self, ret_type, to_sap, *args, tolerance=1e-9, remove=True):
        # initialize data
        ...
        if not self.sap:
            if ret_type == "g":
                ...
                if self.n > 1:
                    ...
                    rgb = ls.shade(
                        Z, cmap=cm.gist_earth, vert_exag=0.1, blend_mode="soft"
                    )
                    ax.plot_surface(
                        X,
                        Y,
        ...
        if to_sap or self.sap:
            if not self.sap:  # if not mod from an existing sap...
                ...
                SapModel.PropMaterial.AddMaterial(
                    "tendon", sap_mat_ID, Country, code, mat_type, "tendon"
                )
                ...
                for i in range(self.n):
                    for j in range(self.m):
                        SapModel.PointObj.AddCartesian(
                            xyz[i * self.m + j],
                            xyz[self.m * self.n + i * self.m + j],
                            xyz[2 * self.m * self.n + i * self.m + j],
                            str(i * self.m + j),
                            str(i * self.m + j),
                        )
                ...
                for nm, ij in zip(self.frame_names, self.frame_end_pts):
                    SapModel.FrameObj.AddByPoint(
                        str(ij[0]), str(ij[1]), str(nm), "T", str(nm)
                    )
                ...
                pre_temp = [
                    -self.frame_force_density[z] * i / (Young * sec_area * thermal)
                ...
                for i, pt in enumerate(self.point_names):
                    SapModel.EditPoint.ChangeCoordinates_1(pt, X[i], Y[i], Z[i])
            ...
            for nm, tp in zip(self.frame_names, pre_temp):
                ...
                SapModel.FrameObj.SetLoadTemperature(str(nm), "Pre_temp", 1, tp)

            # set turn on the "pre" case
            print("--setting pre NL case")
            SapModel.LoadCases.StaticNonlinear.SetCase("pre")
            SapModel.LoadCases.StaticNonlinear.SetGeometricNonlinearity("pre", 2)
            ...
            SapModel.LoadCases.StaticNonlinear.SetLoads(
                "pre", 2, ["Load", "Load"], ["Pre_temp", prel], [1, 1]
            )
...
def mass_assign(SapModel, dead_case_substitute, group="ALL"):
    ...
    for pt, ms in zip(points, masses):
        ...
        SapModel.PointObj.SetLoadForce(
            pt, dead_case_substitute, [0, 0, -ms * 9.80665019960652, 0, 0, 0]
        )
