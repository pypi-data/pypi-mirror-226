...
def append_audio(video_path: Union[str, os.PathLike],
    ...
    if video_meta_data['frame_count'] != audio_src_meta_data['frame_count']:
        InValidUserInputWarning(msg=f'The video ({video_meta_data["frame_count"]}) and audio source ({audio_src_meta_data["frame_count"]}) does not have an equal number of frames.')
