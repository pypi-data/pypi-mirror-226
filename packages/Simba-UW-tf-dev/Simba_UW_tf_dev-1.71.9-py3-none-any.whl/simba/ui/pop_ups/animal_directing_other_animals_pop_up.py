...
class AnimalDirectingAnimalPopUp(ConfigReader, PopUpMixin):
    ...
    def __init__(self,
        ...
        if self.animal_cnt < 2:
            raise AnimalNumberError(msg=f'Directionality between animals require at least two animals. The SimBA project is set to use {self.animal_cnt} animal.')
        if len(self.outlier_corrected_paths) == 0:
            raise NoFilesFoundError(msg=f'No data files found in {self.outlier_corrected_dir}.')
    ...
    def run(self):
        if not self.boolean_tables_var.get() and not self.summary_tables_var.get() and not self.aggregate_statistics_var.get():
            raise InvalidInputError('Please select at least one output format.')
