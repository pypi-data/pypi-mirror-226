    ...
    def import_global_collection(self) -> None:
        ...
        if global_collection_folder.is_dir():
            ...
            with self._app.app_context():
                for folder_item in global_collection_folder.iterdir():
                    if folder_item.is_dir() and folder_item.name not in skip_folders:
                        for py_file in folder_item.glob("*.py"):
                            module_file = import_module(
                                f"{cast_to_import_str(self._app_name, folder_item)}.{py_file.stem}"
                            )
                            if hasattr(module_file, "collection"):
                                module_file.collection(self._app)
                            else:
                                del sys.modules[module_file.__name__]

                    if folder_item.is_file() and folder_item.suffix == ".py":
                        module_file = import_module(
                            f"{cast_to_import_str(self._app_name, global_collection_folder)}.{folder_item.stem}"
                        )
                        if hasattr(module_file, "collection"):
                            module_file.collection(self._app)
                        else:
