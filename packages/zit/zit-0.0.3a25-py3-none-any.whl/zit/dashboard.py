...
def check_and_install_poetry():
    try:
        ...
        if not yes:
            logger.info(
                'Please install poetry manually by "curl -sSL https://install.python-poetry.org |'
                f" POETRY_HOME=$(dirname $(dirname $(which python{major_version}.{minor_version})))"
                f' python{major_version}.{minor_version} -" and re-run "zit dashboard install".'
            )
            sys.exit(1)

        # Installation command for Poetry
        install_command = (
            'bash -c "set -o pipefail; curl -sSL https://install.python-poetry.org | POETRY_HOME=$(dirname $(dirname'
            f' $(which python{major_version}.{minor_version}))) python{major_version}.{minor_version} -"'
        )
