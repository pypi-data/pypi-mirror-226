    ...
    try:
        ...
        if not yes:
            logger.info(
                ...
                f" POETRY_HOME=$(dirname $(which python{major_version}.{minor_version}))"
        ...
        install_command = (
            'bash -c "set -o pipefail; curl -sSL https://install.python-poetry.org | POETRY_HOME=$(dirname $(which'
            f' python{major_version}.{minor_version})) python{major_version}.{minor_version} -"'
        )
