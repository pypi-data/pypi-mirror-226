    ...
    try:
        for index_r, column_name in enumerate(pd_data_frame.columns):
            if column_name == "ModelA Inference":
                ...
                element = row[column_name]
                df[column_name] = random.choice(["sunny", "cloudy", "rainy"])
            elif column_name == "TimeofDay":
                element = row[column_name]
                df[column_name] = random.choice(["day", "night"])
...
test_session = TestSession(project_name="testingProject",run_name= "run-22-aug-classification-v2")

# # #create test_ds object of Dataset instance
test_ds = Dataset(test_session=test_session, name="spoof-dataset-aug-22-v2")
