    ...
    for i in range(n_condition):
        ...
        for j in range(n_con_each[i]):
            if orientation == 'quaternions':
                orientations[i][j, :] = pynibs.util.rotations.rot_to_quat(op_zeromean[i][:, 0:3, j])[1:]
                ori_names = ['xq', 'yq', 'zq']
                xlabel = ['$x_q$', '$y_q$', '$z_q$']
                ylabel = ['$p(x_q)$', '$p(y_q)$', '$p(z_q)$']

            elif orientation == 'euler':
                orientations[i][j, :] = pynibs.util.rotations.rotation_matrix_to_euler_angles(op_zeromean[i][:, :, j]) * 180.0 / np.pi
...
def random_walk_coil(start_mat, n_steps, fn_mesh_hdf5, angles_dev=3, distance_low=1, distance_high=4,
    ...
    for i in range(n_steps):
        ...
        for idx, axis in enumerate(['x', 'y', 'z']):
            if angles_dev[idx] != 0:
                mat_rot = pynibs.roatate_matsimnibs_euler(axis=axis,
