...
def invert(trans):
    ...
    Parameters
    ----------
    trans : np.ndarray of float [3 x 3]
        Rotation matrix

    Returns
    -------
    rot_inv : np.ndarray of float [3 x 3]
...
def cross_product(A, B):
    ...
    B : np.ndarray of float 2 x [N x 3]
        Input vectors, the cross product is evaluated between

    Returns
    -------
    c : np.ndarray of float [N x 3]
