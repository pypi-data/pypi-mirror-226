...
class AppPackage:
    ...
    requirement_specs = {
        ...
        "req-8":
            "The Application Package CWL CommandLineTool classes SHALL contain "
        ...
        "req-10":
            "The Application Package CWL Workflow class “inputs” fields SHALL contain "
            "the following elements: Identifier ('id'); Title ('label'); Abstract ('doc').",
        "req-11":
            "The Application Package CWL Workclass classes SHALL include additional metadata "
        ...
        "req-12":
            "All input parameters of the CWL CommandLineTool that require the staging of EO products "
            "SHALL be of type 'Directory'.",
        "req-13":
            "Input parameters of the CWL Workflow that require the staging of EO products "
            "SHALL be of type 'Directory'.",
        "req-14":
            "The outputs field of the CommandLineTool that requires the stage-out of EO products "
    ...
    @classmethod
    def process_cli(cls, cwl_url, entry_point=None, detail="errors", format="text", stdout=sys.stdout, stderr=sys.stderr):
        ...
        if format == "text":
            ...
            result = {
                "issues": issues,
                "requirements": { r: AppPackage.requirement_specs[r] for r in set([i["req"] for i in issues if i["req"]]) }
            }
    ...
    def check_req_11(self):
        ...
        namespaces = self.cwl["$namespaces"] if "$namespaces" in self.cwl and isinstance(self.cwl["$namespaces"], dict) else {}
        ...
        if not has_version:
            issues.append(
                {
                    "type": "error",
                    "message": f"Missing metadata element for application package: softwareVersion",
                    "req": "req-11",
                }
            )
        ...
        for clt in self.command_line_tools:
            ...
            if docker_requirement and docker_requirement.dockerOutputDirectory:
                ...
                issues.append(
                    {
                        "type": "error",
                        "message": f"Unsupported element in DockerRequirement of {clt_name}: 'dockerOutputDirectory'",
                        "req": None,
                    }
                )
