    ...
    if key in json_obj:
        ...
        if data_type:
            if not isinstance(value, data_type):
                if add_type_err:
                    status.add_error("The value for the key '%s' in policy '%s' is not of type %s" % (key, policy_id, str(data_type)))
                    ret_val = process_status.ERR_wrong_json_structure
    elif add_value_err:
        value = None
        status.add_error("The key '%s' is missing in policy '%s'" % (key, policy_id))
        ret_val = process_status.ERR_wrong_json_structure
    else:
        value = None
