...
def rceive_data(status, mem_view, params, clientSoc, ip_in, port_in, max_buffr_size):
    ...
    while True:
        ...
        if recv_counter == 1:
            ...
            pub_key = None
            if al_auth.is_node_authentication() and command != "job":
                ...
                ret_val, pub_key = permissions.authenticate_tcp_message(status, mem_view)
                #ret_val = permissions.auth_user_test_permissions(status, mem_view, command, "", "")
                if ret_val:
                    break
