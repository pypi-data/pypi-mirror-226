...
from anylog_node.generic.utils_columns import get_current_utc_time, get_time_in_sec
...
cluster_table_attr = [
    ...
    ("dbms",        str,        None,               None,          True,         False),
    ("name",        str,        None,               None,          True,         False),
    ("status",      str,        None,               "active",      True,         False),
]

operator_attr = [
    ("company", str, None, None, True, False),
    ("name", str, None, None, True, False),
    ("ip", str, None, None, True, False),
    ("port", int, None, None, True, False),
    ("member", int, None, None, False, False),
]
...
cluster_attr = [
    ...
    ("name", str, None, None, True, False),
    ("company", str, None, None, True, False),
    ("table", list, cluster_table_attr, None, False, False),
]

table_attr = [
    ("name", str, None, None, True, False),
    ("dbms", str, None, None, True, False),
    ("create", str, None, None, True, False),
]

mapping_schema_attr = [
    # attr name * attr type * child attributes * default value * must exists *  is Unique
    ("condition", str,        None,              None,           False,         False),
    ("bring",     str,        None,              None,           False,         False),
    ("type",      str,        None,              None,           True,          False),
    ("root",      bool,       None,              None,           False,         False),
    ("blob",      bool,       None,              None,           False,         False),
    ("extension", str,        None,              None,           False,         False),
    ("hash",      str,        None,              None,           False,         False),
]

mapping_attr = [
    ("dbms", str, None, None, True, False),
    ("readings", str, None, None, False, False),
    ("condition", str, None, None, False, False),
    ("schema", dict, mapping_schema_attr, None, True, False)
]
...
member_attr = [
    # attr name * attr type * child attributes * default value * must exists *  is unique
    ("type",        str,            None,           None,           True,       False),
    ("public_key",  str,            None,           None,           True,       True),
]

permissions_attr = [
    # Permission policy determines commands and databases allowed
    # attr name * attr type * child attributes * default value * must exists *  is-unique
    ("name",        str,         None,               None,           True,        True),    # A unique name to identify the policy
    ("enable",      list,        None,               None,           True,        False),
    ("disable",     list,        None,               None,           False,       False),
    ("databases",   list,        None,               None,           False,       False),
    ("tables",      list,        None,               None,           False,       False),
    ("public_key",  str,         None,               None,           True,        False),
    ("signature",   str,         None,               None,           True,        False),
...
assignment_attr = [
    ...
    # attr name * attr type * child attributes * default value * must exists *  is-unique
    ("permissions", str,        None,               None,           True,       False),       # Policy id of the permissions
    ("members",     list,       None,               None,           True,       False),
    ("public_key",  str,        None,               None,           True,       False),
    ("signature",   str,        None,               None,           True,       False),
]
...
def new_operaor_policy(status, policy_type, policy_obj, blockchain_file):
    ...
    if not ret_val:
        if "cluster" in policy_obj:
            ...
            # Test that the operator is not assigned to a cluster
            ret_val, policies = get_policies_by_ip_port(status, policy_type, policy_obj, blockchain_file)
            if ret_val:
                status.add_error("Failed to read policies of type: '%s'" % policy_type)
            else:
                if policies and len(policies):
                    for entry in policies:
                        if "cluster" in entry[policy_type]:
                            status.add_error("Failed to add an Operator policy: the Operator is assigned to an existing cluster: '%s'" %  entry[policy_type]["cluster"])
                            ret_val = process_status.Wrong_policy_structure
                if not "dbms" in policy_obj:
                    # A policy to declare specific databases or tables supported by the operator (without a cluster)
                    status.add_error("Operator Policy is is not associated with a cluster or a database")
                    ret_val = process_status.Wrong_policy_structure
