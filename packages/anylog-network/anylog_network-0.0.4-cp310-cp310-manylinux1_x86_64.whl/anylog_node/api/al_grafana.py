...
class AlQueryParams:
    ...
    def set_target_info(self, status, target_id):
        ...
        if not self.column_time or not self.column_value:
...
def grafana_post(status, request_handler, decode_body, timeout):
    ...
    if request_handler.path.startswith("/search"):
        # RETURN A LIST OF TABLES for the DBMS
        ...
        status.add_error("Unrecognized key provided by Grafana to POST request: '%s'" % request_handler.path)
...
# =======================================================================================================================
def set_timeseries_struct(status, table_name, query_params, reply_data):
    ...
    time_column_id = -1  # no time column
    time_column_name = query_params.get_time_column()
    if time_column_name:
        for index, column_name in enumerate(title_list):
            # test for a column name or a function over the column name
            if column_name == time_column_name or (column_name.find("(" + time_column_name + ")") > 0):
                # with time column that can be graphed
                time_column_name = column_name
                time_column_id = index
                break

    # determibe the list of columns that are to be send to Grafana
    targets = []
    first = True
    for index, entry in enumerate(data_types):
        # get the number fields to create the targets
        if index != time_column_id and is_graphed_data_type(entry):
            if first:
                targets.append('{"target": "%s", "datapoints": [' % title_list[index])
                first = False
            else:
                targets.append(', {"target": "%s", "datapoints": [' % title_list[index])
        else:
            targets.append(None)  # this column data is not used
    ...
    if reply_json:
        ...
        for row_counter, row in enumerate(rows):
            if time_column_id != -1:
                time_field_value = row[time_column_name]
                if data_types[time_column_id].startswith("time"):
                    time_value = str(int(utils_columns.get_time_in_sec(time_field_value, False) * 1000))
                else:
                    time_value = time_field_value
            else:
                time_value = '0'  # no time value

            for index, column_name in enumerate(title_list):
                if targets[index] != None:
                    column_value = row[column_name]
                    if data_types[index].startswith("time"):
                        # Change time string to a value
                        column_value = str(int(utils_columns.get_time_in_sec(column_value, False) * 1000))
                    if row_counter:
                        targets[index] += (', [' + column_value + ", " + time_value + ']')
                    else:
                        targets[index] += ('[' + column_value + ", " + time_value + ']')
    ...
    if body_info:
        ...
        for target_id in range(queries_count):  # Execute one or more queries on the Grafana Panel
            ...
            if query_params.is_sql_query():
                ...
                ret_val = native_api.exec_sql_stmt(status, servers, dbms_name, "timezone = %s" % timezone, statement, timeout)

                if ret_val:
                    if trace_level:
                        utils_print.output("\r\n[Grafana] [Query returns] [Error: %u] [%s]" % (ret_val, process_status.get_status_text(ret_val)), True)
                    continue
...
def map_sql_replies(status, query_params, target_id, data_str, reply_data, trace_level):
    ...
    if query_params.is_tsd():
        ...
        if query_params.is_default_query():
            ...
            data_str += set_timeseries_struct(status, table_name, query_params,
