...
# ===================================
# Convnert json object to dictionary 
# ==================================
def convert_json_to_dict(status, json_object: list) -> dict:
    """
    Convert JSON to dict
    :args:
       json_object:list - object containing JSON data
    :return:
       dict from JSON
    """
    json_data = []
    for line in json_object:
        updated_line = map_json_to_insert.format_reading(status, line)
        if updated_line is not False:
            json_data.append(updated_line)

    if json_data is []:
        status.add_error("Failed to generate INSERT statement.")
...
def create_insert(status: process_status, table_name: str, data: list, columns: list):
    ...
    data = map_json_to_insert.map_columns(status, "", "", "", 0, data, columns)
