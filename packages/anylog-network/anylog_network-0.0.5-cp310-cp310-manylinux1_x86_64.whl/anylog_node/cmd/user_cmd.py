...
# sys.path.insert(0, tcpip_dir)
# sys.path.insert(0, dbms_dir)
# sys.path.insert(0, json_sql_dir)
...
class UserInput:
    def __init__(self):
        ...
        utils_print.output("Type \'help\' for list of commands", True)
    ...
    def process_input(self, arguments = 0, arguments_list = None):
        ...
        # Flag if license is needed and not provided
        if cython.compiled:
            if not member_cmd.expiration_:      # License was not provided in the boot script
                member_cmd.activation_license_ = False        # Need activation license
        ...
        while (1):
            ...
            in_command = keyboard.get_cmd(place_in_buff)

            if not len(in_command):
