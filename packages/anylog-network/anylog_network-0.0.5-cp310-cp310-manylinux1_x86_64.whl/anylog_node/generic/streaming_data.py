...
def add_data(status, mode, row_counter, prep_dir, watch_dir, err_dir, dbms_name, table_name, source, instructions, file_type, msg_data):
    ...
    reply = [ret_val, hash_value]       # In case of a file - the hash value is returned
...
# =======================================================================================================================
def set_thresholds(status, io_buff_in, cmd_words, trace):
    global write_threshold
    global default_time_
    global default_volume_
...
def show_info(out_format):
    ...
    if out_format == "json":
        ...
        info_json["Default Time"] = default_time_
        info_json["Default Volume"] = default_volume_
        info_json["Default Immediate"] = write_immediate_
        info_json["Buffered Rows"] = counter_rows_in_buff_
        info_json["Flushed Rows"] = counter_rows_flushed_
    else:
        info_chart = []
        info_chart.append(("Default Time", default_time_, streamer_stat))
        info_chart.append(("Default Volume", format(default_volume_,","),""))
        info_chart.append(("Default Immediate", str(write_immediate_), ""))
