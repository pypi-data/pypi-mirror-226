...
def get_node_info(status, io_buff_in, cmd_words, trace):
    ...
    if not psutil_installed:
        status.add_error("Process failed: psutil not installed")
...
def get_machine_connections(status, io_buff_in, cmd_words, trace):
    ...
    if psutil_installed:
        ...
        info = ""
        status.add_error("Process failed: psutil ot installed")
        ret_val = process_status.Failed_to_import_lib
...
def update_process_statistics(stat_dict, is_operator, is_running):
    ...
    if is_operator:
        ...
        if not node_name:
            node_name = params.get_node_name()
    else:
        node_name = params.get_node_name()
    ...
    if not is_running:
        ...
        if "Query timestamp" in stat_dict:
            query_timestamp = stat_dict["Query timestamp"]  # Timestamp of previous query
            if query_timestamp:
                # Calculate time from previous query
                delta_seconds = current_timestamp - query_timestamp
                hours_time, minutes_time, seconds_time = utils_data.seconds_to_hms(delta_seconds)
                stat_dict["Delta time"] = "%02u:%02u:%02u (H:M:S)" % (hours_time, minutes_time, seconds_time)

                if "Delta events" in stat_dict:
                    delta_rows = stat_dict["Delta events"]  # Events processed since last query
                    if delta_seconds:
                        events_sec = int(delta_rows / delta_seconds)
                    else:
                        events_sec = 0

                    stat_dict["Events/sec"] = events_sec
