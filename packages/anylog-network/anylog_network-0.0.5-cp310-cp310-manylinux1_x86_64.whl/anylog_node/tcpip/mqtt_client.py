...
from anylog_node.generic.params import get_node_name, get_value_if_available
...
class SubscriptionInfo:
    ...
    def __init__(self):
        ...
        self.connect_rc = mqtt.MQTT_ERR_NO_CONN     # Updated with the return value from on_connect()
    ...
    if ret_val:
        ...
        if not isinstance(user_id, int) or len(subscript_mutex) <= user_id:
            ...
            if client_sbscr.is_message_flush(topic):
                ...
                if not user_msg[0] == '{' or not  user_msg[-1] == '}':
                    process_log.add("Error", f"MQTT message is not in JSON format (Missing curly brackets)")
...
def process_using_policies(status, topic_info, topic, json_msg, prep_dir, watch_dir, bwatch_dir, err_dir, blobs_dir):
    ...
    for policy in policies:
        ...
        is_apply = 1  # 1 value means if returned true
        if "condition" in policy_inner:
            # Get the condition if the policy applies
            is_apply = mapping_policy.process_if_code(status, policy_inner, policy_id, "condition", json_msg)
            if is_apply == -1:
...
def process_policy(status, policy_inner, policy_id, json_msg, prep_dir, watch_dir, bwatch_dir, err_dir, blobs_dir):
    ...
    if not ret_val:
        if policy_info.startswith("bring "):
            # Process the BRING command
            retrieve_info = policy_info.split()
            if len(retrieve_info) < 2:
                status.add_error(f"Wrong 'bring' command: '{policy_info}' to retrieve DBMS name in policy {policy_id}")
                ret_val = process_status.Wrong_policy_structure
            else:
                ret_val, dbms_name = utils_json.pull_info(status, [json_msg], retrieve_info[1:], None, 0)
        else:
            dbms_name = policy_info

        dbms_name = dbms_name.lower()

        if not ret_val:
            ret_val, policy_info = mapping_policy.get_policy_info(status, policy_inner, policy_id, "table", True)
            if not ret_val:
                if policy_info.startswith("bring "):
                    # Process the BRING command
                    retrieve_info = policy_info.split()
                    if len(retrieve_info) < 2:
                        status.add_error(f"Wrong 'bring' command: '{policy_info}' to retrieve table name in policy {policy_id}")
                        ret_val = process_status.Wrong_policy_structure
                    else:
                        ret_val, table_name = utils_json.pull_info(status, [json_msg], retrieve_info[1:], None, 0)
                else:
            ...
            if not ret_val:
                ...
                ret_val, data_source, insert_list = mapping_policy.apply_policy_schema(status, dbms_name, table_name, policy_inner, policy_id, json_msg, True, blobs_dir)
                ...
                for entry in list_object:
                    ...
                    if local_broker:
                        ...
                        try:
                            ...
                            if delivery_info.rc == mqtt.MQTT_ERR_SUCCESS:
                                ret_val = process_status.SUCCESS
                            else:
...
# ------------------------------------------------------------------------------------------------------------
def connect_to_broker(operation_type, conditions:dict, user_id:int, thread_name):
    ...
    if user_id:
        user_data = user_id
        subscriptions[user_data].set_connect_rc( mqtt.MQTT_ERR_NO_CONN )     # Updated with the return value from on_connect()
    else:
    ...
    if mqtt_broker == "local":
        ...
        client = mqtt.Client(client_id=client_id, userdata=user_data)  # assign conditions to the user data
