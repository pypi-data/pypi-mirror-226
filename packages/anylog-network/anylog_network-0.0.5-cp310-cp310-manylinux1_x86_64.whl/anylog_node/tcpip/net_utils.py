...
# =======================================================================================================================
# Test for socket not available
# =======================================================================================================================
def test_soc_available(soc, message):
    soc_info = soc.getsockname()
    proc = subprocess.Popen(["netstat -an | grep %u" % soc_info[1]], stdout=subprocess.PIPE, shell=True)
    (out, err) = proc.communicate()
    if not len(out):
        info_string = "%s: %s:%u not available" % (message, soc_info[0], soc_info[1])
        utils_print.output(info_string, True)
...
def get_source_addr(dest_type, dest_ip):
    ...
    if active_connections_[server_type][0]:
...
def is_connection_used(ip_port):
    ...
    for entry in active_connections_:
        if ip_port in entry:        # entry is an array of 2 connections: internal external
            return True
    return False
