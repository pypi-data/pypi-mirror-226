...
from anylog_node.blockchain.al_auth import is_user_authentication, validate_user
...
class ChunkedHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def authenticate_request(self, status, rest_type, command):
        ...
        if ret_val:
            if is_user_authentication():
                ...
                ret_val = validate_user(status, key, False)
    ...
    def process_user_certificate(self, status, rest_type, command):
        ...
        if ret_val:
            ...
            ret_code = permissions.authenticate_rest_message(status, public_key)
            if ret_code:
                ret_val = False
                ...
                if messages_list:
                    ...
                    for message in messages_list:
                        ...
                        try:
                            ...
                            self.handle_exception(status, "POST", errno, value, stack_trace)
                            ret_val = process_status.REST_call_err
                            utils_print.output_box("Failed to process message: %s" % msg_str)
        ...
        if not command:
            ...
            if not self.is_correct_method(status, http_method, command):
                ...
                if ret_val != process_status.SUCCESS and ret_val < process_status.NON_ERROR_RET_VALUE:
                    ...
                    write_ret_value = self.local_table_query(status, j_handle, with_wait, nodes_count)
    ...
    # =======================================================================================================================
    def local_table_query(self, status, j_handle, with_wait, nodes_count):
        ...
        if not ret_val:
            try:
                ret_val = member_cmd.query_local_dbms(status, None, logical_dbms, table_name, conditions, sql_command, nodes_count)
            except:
...
def put_params_from_header(status, al_headers):
    ...
    dbms_name = get_value_from_headers(al_headers, "dbms")
    if not dbms_name:
        ret_val = process_status.Missing_dbms_name
        status.keep_error("Missing 'dbms' name in REST PUT command")
    else:
        table_name = get_value_from_headers(al_headers, "table")
        if not table_name:
