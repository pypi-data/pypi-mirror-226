...
class PSQL:
    ...
    def get_conn_from_pool(self, status):
        try:
            ...
            connection = None
            status.add_error("Postgres failed to get connection with database %s from the pool" % self.dbms_name)
    ...
    def ret_conn_to_pool(self, status, connection):
        try:
            ...
            status.add_error("Postgres failed to return connection with database %s to the pool" % self.dbms_name)
            ret_val = False
        else:
            ret_val = True
        return ret_val
    ...
    def fetch_rows(self, status: process_status, db_cursor, output_prefix: str, fetch_size: int,
                   title_list: list, type_list:list):
