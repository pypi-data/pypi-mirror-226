...
dbms_stat = {}  # Dbms insert statistics
mutex_stat = threading.Lock()  # mutex to update the statistics
...
# =======================================
def insert_rows(status, dbms_name, list_inserts):
...
def process_sql_from_file(status, dbms_name, table_name, file_path):
    ...
    if db_cursor == None:
        ...
        update_insert_stat(dbms_name, table_name, rows_counter)  # Update stat on inserts
...
# =======================================================================================================================
def delete_tsd_row(status, row_id):

    sql_delete = "delete from tsd_info where file_id = '%s'" % (row_id)
...
# ==================================================================
# Update stats - rows inserted for each dbms
# ==================================================================
def update_insert_stat(dbms_name, table_name, rows_count):
    # Get the dictionary of the specific dbms
    if dbms_name in dbms_stat.keys():
        dbms_info = dbms_stat[dbms_name]
    else:
        # take a global mutex to get or create the dbms dictionary
        mutex_stat.acquire()

        if dbms_name in dbms_stat.keys():
            # Get the dbms dictionary (with all the tables
            dbms_info = dbms_stat[dbms_name]
        else:
            # create the dictionary for the dbms
            dbms_info = [threading.Lock(),
                         {}]  # Create a mutex for the specific database and a dictionary for the database tables
            dbms_stat[dbms_name] = dbms_info

        mutex_stat.release()

    # Get the dictionary for the specific table
    tables_stat = dbms_info[1]
    if table_name in tables_stat.keys():
        tables_info = tables_stat[table_name]
    else:
        # take a mutex to get or create the table info
        tables_mutex_stat = dbms_info[0]
        tables_mutex_stat.acquire()

        if table_name in tables_stat.keys():
            # Get the table info
            tables_info = tables_stat[table_name]
        else:
            # create an array to hold init time + insert time + counter as a f(table)
            tables_info = [utils_columns.get_current_time_in_sec(), utils_columns.get_current_time_in_sec(), 0]
            tables_stat[table_name] = tables_info

        tables_mutex_stat.release()

    tables_info[1] = utils_columns.get_current_time_in_sec()
    tables_info[2] += rows_count


# ==================================================================
# get rows inserted for each dbms or for all databases
# ==================================================================
def get_insert_stat(is_detailed):
    info_array = [[]]  # include info per every database
    total_rows = 0
    total_rows_per_sec = 0
    index = 0

    current_time = utils_columns.get_current_time_in_sec()
    for dbms_name, dbms_info in dbms_stat.items():

        table_dict = dbms_info[1]

        for table_name, table_info in table_dict.items():

            start_time = table_info[0]
            end_time = table_info[1]
            rows_count = table_info[2]

            if current_time > start_time:
                total_rows += rows_count
                if current_time == start_time:
                    rows_per_sec = rows_count
                else:
                    rows_per_sec = rows_count / (current_time - start_time)
                total_rows_per_sec += rows_per_sec
                if is_detailed:
                    info_array[index].append(dbms_name)
                    info_array[index].append(table_name)
                    info_array[index].append(utils_columns.seconds_to_date(start_time, "%Y-%m-%d %H:%M:%S"))
                    info_array[index].append(utils_columns.seconds_to_date(end_time, "%Y-%m-%d %H:%M:%S"))
                    info_array[index].append(str(rows_count))
                    info_array[index].append(str(round(rows_per_sec, 2)))
                    info_array.append([])
                    index += 1

    info_array[index].append("Total")
    info_array[index].append(None)
    info_array[index].append(None)
    info_array[index].append(None)
    info_array[index].append(str(total_rows))
    info_array[index].append(str(round(total_rows_per_sec, 2)))

    info_str = utils_print.output_nested_lists(info_array, "\r\nNode Inserts",
                                               ["DBMS", "Table", "First Inserts", "Last Inserts", "Total Inserted",
                                                "Inserts/Sec"], True)
