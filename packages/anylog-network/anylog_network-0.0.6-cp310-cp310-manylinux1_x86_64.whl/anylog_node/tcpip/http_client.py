...
def do_PUT(status, cmd_words):
    ...
    if not url:
        status.add_error("REST PUT error, URL not provided")
        return process_status.REST_call_err
...
def do_VIEW(status, cmd_words):
    ...
    if not url:
        status.add_error("REST VIEW error, URL not provided")
        return process_status.REST_call_err
...
def do_POST(status, cmd_words):
    ...
    if not url:
        status.add_error("REST POST error, URL not provided")
        return process_status.REST_call_err
    ...
    response = post_request(status, url, headers)
...
def do_GET(status, cmd_words, offset):
    ...
    if not url:
        status.add_error("REST GET error, URL not provided")
        return process_status.REST_call_err
    ...
    response = get_request(status, url, headers, 10)
...
# =======================================================================================================================
def post_request(status, url, data):
    r = None
