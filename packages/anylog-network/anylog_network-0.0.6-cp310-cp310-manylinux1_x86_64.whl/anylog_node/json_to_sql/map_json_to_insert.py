...
def map_json_file_to_insert(status: process_status, tsd_name, tsd_id, dbms_name, table_name, insert_size: int, json_file, sql_dir, instructions):
    ...
    if instructions == "0":
        ...
        if not instruct:
            err_msg = "Failed to retrieve mapping instructions using the key: '%s' for file: '%s'" % (instructions, json_file)
            ret_val = False
    ...
    if ret_val:
        ...
        if not os.path.isfile(json_file):
            err_msg = "Failed to generate INSERT statement - Path to JSON file not recognized: " + json_file
            ret_val = False
        else:
            ...
            if not os.path.isdir(sql_dir):
                err_msg = "Failed to generate INSERT statement - Invalid SQL dir: %s" % sql_dir
                ret_val = False
            else:
            ...
            if ret_code:
                ...
                if ret_val:
                    ...
                    if apply_policy:
                        ...
                        ret_code, data_source, json_data = mapping_policy.apply_policy_schema(status, dbms_name, table_name, policy_inner, policy_inner["id"], json_data, False, None)
                        if ret_code:
...
def insert_sql_to_table(status, sql_file, dbms_name, table_name, par_name, insert_size, column_names, insert_columns):
    ...
    if ret_val:
        ...
        ret_val = db_info.insert_rows(status, dbms_name, insert_list)
