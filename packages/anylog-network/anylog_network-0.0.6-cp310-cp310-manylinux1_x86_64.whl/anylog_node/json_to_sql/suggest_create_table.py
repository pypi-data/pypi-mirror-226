...
from anylog_node.generic.utils_data import *
...
# ==================================================================
#
# Itterate through dictionary and figure out the data type of each column. 
...
def set_column_name_type(status, columns, column_name, column_value):
    ...
    if isinstance(column_name, str) and len(column_name) and column_value:
...
def resolve_data_type(one_type, second_type):
    ...
    if count_chars:
        ...
        # alpahabetically organize such that only one combination is tested
...
def get_column_type_by_value(value):
    ...
    if isinstance(value, bool):
        ...
        if len(str(value).split(".")[-1]) >= 1 and len(str(value).split(".")[-1]) <= 5:
            data_type = 'DECIMAL'
        else:
            data_type = 'FLOAT'
    elif isinstance(value, str):
        if utils_data.check_uuid(value) is True:
            data_type = 'UUID'
