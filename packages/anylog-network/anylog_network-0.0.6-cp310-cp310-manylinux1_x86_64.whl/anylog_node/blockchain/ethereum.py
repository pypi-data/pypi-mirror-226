...
class Ethereum(gateway.BlockchainNode):
    ...
    def set_sender_account(self, status):
        ...
        '''
        try:
            self.senderAccount = self.connection.eth.account.privateKeyToAccount(self.private_key)
        except:
            errno, value = sys.exc_info()[:2]
    ...
    def get_balance(self):
        ...
        try:
            wei = self.connection.eth.getBalance( self.public_key )
        except:
    ...
    def is_connected(self):
        ...
        if self.connected:
            ...
            try:
                valid_connect = self.connection.isConnected()
            except:
                errno, value = sys.exc_info()[:2]
                err_msg = "Ethereum isConnected failed: {0} : {1}".format(str(errno), str(value))
