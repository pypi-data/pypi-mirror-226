...
import anylog_node.cmd.permissions as permissions
...
# import anylog_node.dbms.cluster_info as cursor_info
...
import anylog_node.members.alpublisher as alpublisher
import anylog_node.members.aldistributor as aldistributor
import anylog_node.members.alarchiver as alarchiver
import anylog_node.members.alconsumer as alconsumer
...
from anylog_node.dbms.dbms import select_dbms, connect_dbms, get_real_dbms_name



code_version = "AnyLog Code Vesion: 1.232108"    # git log -1 --oneline --decorate
license_public_str_ = """-----BEGIN PUBLIC KEY-----\n\
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDdqlseiuWwc5+JR0N7KxVIHg8l\n\
TZUHg8ImTysvzYoANLDB2Zj9TB/P1K9uWjLKsqUpJSIXnsFZ/MKRcRc3F4PxaM84\n\
hnD52eMWC5sccTu3gXu80sdco02FJzvZ/AW516rZu/dGBXZ0igYJKfxSkk01Ci/p\n\
H1g4WsVvnjTlzn78EQIDAQAB\n-----END PUBLIC KEY-----\n"""
...
activation_license_ = True      # If compiled with cython, license is needed
expiration_ = ""                # License expiration date
license_type_ = ""              # beta or commercial
licensed_company_ = ""          # Company holding the license

license_msg_ = "The usage of this software is subject to an active an license agreement with AnyLog."
...
def config_servers(status, json_object, policy_id, trace):
    ...
    if (ip and port) or (local_ip and local_port):
        ...
        ret_val = start_tcp_threads(ip, main_port, local_ip, second_port, is_bind, url_bind, 6, trace)
        if ret_val:
            return ret_val
    ...
    if rest_port:
        ...
        ret_val = start_rest_threads(external_ip, external_port, internal_ip, internal_port, is_bind, url_bind, None)
        if ret_val:
            return ret_val
    ...
    if broker_port:
        ...
        ret_val = start_broker_threads(external_ip, external_port, internal_ip, internal_port, is_bind, url_bind, 6, trace)
        if not ret_val:
...
def blockchain_test(status, cmd_words, blockchain_file):
    ...
    if not ret_val:
        if test_type == 1:
            # test valif blockchain structure
        ...
        if offset:
            ...
            message = params.get_node_name() + " %s" % message
        ...
        if was_modified or force_load:
            ...
            if not ret_val:
                if not clusters:
                    ...
                    status.add_error("Failed to retrieve 'cluster' info from local blockchain file or no cluster info")
                    ret_val = process_status.Needed_policy_not_available
                else:
                    if trace:
...
def blockchain_get(status, cmd_words, blockchain_file, return_data):
    ...
    if words_count >= (offset + 1):
        with_bring = True
        bring_type = utils_json.get_bring_type(status, cmd_words[offset])
        if bring_type == -1:
            ret_val = process_status.ERR_command_struct
    ...
    if not ret_val:
        ...
        if utils_json.is_bring("table", bring_type):
            ...
            if json_str:
                output_str = set_json_as_table(json_str, bring_type)
...
# =======================================================================================================================
def set_json_as_table(json_struct, bring_type):
...
def get_command_ip_port(status, cmd_words):
    ...
    dest_keys = ""
    ip_port_values = []
    if cmd_words[2][0] != '(':  # one Ip and one port without parenthesis
        ...
        # add IP + Port + Table name + DBMS name (Table Name and DBMS Name are provided for Include tables"
        ip_port_values.append((ip, port, "", "", True))

    else:
        # format in parenthesis
        ips_ports = re.sub('\s+', ' ', cmd_words[2][1:-1])  # remove extra spaces
        ips_ports = ips_ports.strip()  # remove leading and trailing spaces

        if ips_ports.startswith('dbms') or ips_ports.startswith("table") or ips_ports.startswith("blockchain get "):
            dest_keys = ips_ports  # keys on the blockchain representing destinations
        else:
            ip_port_list = ips_ports.split(',')  # get an array with IPs and ports (ignore the brackets)
            ...
            for x in range(entries):
                if ip_port_list[x] != "":
                    ...
                    for one_ip_port in main_list:
                        ...
                        if ip == "subset":
                            status.set_subset(port)    # The port is a bool like: subset=true
                            counter_instruct += 1
                            ip = ""
                            continue
                        if ip == "timeout":
                            if port and port.isdecimal():
                                status.set_timeout(int(port))      # The port serves as the seconds: timeout=30
                            counter_instruct += 1
                            ip = ""
                            continue
...
# =======================================================================================================================
def print_command(line_number, commnad, ret_val):
...
def transfer_file(status, ip_port_list, file_source, file_dest, flag, trace, caller_code, consider_err):
    ...
    for ip_port in ip_port_list:  # send message to all servers on the list
        ...
        ret_val, auth_str = al_auth.get_transfer_str(status, net_utils.get_external_ip_port())
        if ret_val:
            break

        if not net_client.file_send(ip, port_val, buff_size, file_source, file_dest, flag, trace, consider_err, auth_str):
            if consider_err:
                error_msg = "Failed to send file to server: " + file_source
                status.add_keep_error(error_msg)
...
def resolve_destination(status, dest_names, is_select, remote_dbms, remote_table, include_tables):
    ...
    if dest_names != "":
        ...
        if dbms_table.find(',') != -1:
            ...
            if  counter_entries >= 2:
                for index, entry in enumerate(reversed(dest_keys)):
                    if entry.startswith("subset="):
                        if len(entry) > 7:
                            status.set_subset(entry[7:])  # The port serves as the seconds: timeout=30
                            del dest_keys[counter_entries - index - 1]
                            continue
                    if entry.startswith("timeout="):
                        if len(entry) > 8:
                            status.set_subset(entry[8:])  # The port serves as the seconds: timeout=30
                            del dest_keys[counter_entries - index - 1]
                            continue
    ...
    if operators:
        ...
        for entry in operators:
            ...
            # add IP + Port + Table name + DBMS name (Table Name and DBMS Name are provided for Include tables"
            ip_port_values.append((entry.ip, entry.port, entry.dbms, entry.table, entry.active))
    else:
...
def get_servers_by_cmd(status, command_str, ip_port_values ):
    ...
    timeout = 0
    for _ in range (2):
        if len(sub_str) > 3 and sub_str[-2] == "=":
            if sub_str[-3].replace(',','') == "timeout":
                if sub_str[-1].isnumeric():
                    timeout = int(sub_str[-1])
                sub_str = sub_str [:-3]
            elif sub_str[-3].replace(',','') == "subset":
                if sub_str[-1] == "true":
                    subset = "true"
                sub_str = sub_str [:-3]
        ...
        if not err_val and len(ip_port_string):
            ...
            for entry in result_list:
                ...
                if index and index < (len(entry) - 1):
                    ip_port_values.append((entry[0:index], entry[index + 1:], "", "", True))
# =======================================================================================================================
# Process REDO Query
...
# counter - a sequential job counter
# =======================================================================================================================
    ...
    for ip_port in ip_port_list:  # send message to all servers on the list
        ...
        if dbms_name:
            ...
            if message.startswith("sql "):
                message = utils_sql.update_dbms_table_names(dbms_name, ip_port[3], message)
            elif message.startswith("info table "):
                table_name = ip_port[3]
                if table_name:
                    message = "info table " + dbms_name + " " + table_name + " " + message.rsplit(' ', 2)[-1]
        ...
        if not ip_port[4]:
            # This is the case in a query without a node supporting a cluster (and subset is set to True in the query destination)
        ...
        if ret_val != process_status.SUCCESS:
            if ret_val == process_status.NETWORK_CONNECTION_FAILED:
                # Failed to create a socket
                err_message = "Connection Failed"
            elif ret_val == process_status.Operator_not_accessible:
            ...
            if unique_job_id:
                ...
                if status.is_subset():
                    ...
                    integrate_print_reply(status, None, unique_job_id, j_instance, receiver_id, ret_val, False, "Not responding")
                    ret_val = process_status.SUCCESS
            elif not status.is_rest_wait():
                # Message to stdout
                if status.is_subset():
                    # Error to one node
                    utils_print.output("\n\r[Message to Node %s:%s] [%s]" % (ip, port, err_message), True)
                    ret_val = process_status.SUCCESS
            else:
                break
...
def deliver_rows(status, io_buff_in, j_instance, receiver_id, par_id, is_pass_through, dbms_cursor, rest_call, io_stream):
    ...
    if is_pass_through:
        if format_type == "table":
            if not ret_val:
                ret_val = output_manager.finalize(status, io_buff_in, fetch_counter, 0, True, False)
...
def query_summary(status, io_buff_in, j_instance, io_stream):
    ...
    if not ret_val:
        ...
        ret_val = output_manager.finalize(status, io_buff_in, rows_transferred, operator_time, False, True)
...
# =======================================================================================================================
def query_row_by_row(status, dbms_cursor, io_buff_in, conditions, sql_time, sql_command, nodes_count):
    global query_mode
    ...
    if message:
        ...
        if al_auth.is_node_encryption():
            ...
            if public_str:
                encrypted_key, f_object = al_auth.setup_encryption(status, public_str)
    ...
    while get_next:  # read each row - when the rows is read, it is copied to one or more data blocks - see message message_header.copy_data() below.
        ...
        # test for a ;imit on processing time or data volume
        if max_time:
        ...
        if message:  # prepare a reply
            ...
            if f_object:
                # Symetric encryption
                data_encoded = al_auth.symetric_encryption(status, f_object, rows_data)
                if not data_encoded:
    ...
    if message:
        ...
        if not ret_val or ret_val > process_status.NON_ERROR_RET_VALUE:
            ret_val = output_manager.finalize(status, io_buff_in, fetch_counter, dbms_time, False, True)
...
# =======================================================================================================================
def process_query_sequence(status, io_buff_in, cmd_words, logical_dbms, table_name, conditions, sql_command, nodes_count):
    ...
    if j_handle.is_rest_caller():
        ...
        ret_val = query_local_dbms(status, io_buff_in, logical_dbms, table_name, conditions, sql_command, nodes_count)
...
# =======================================================================================================================
def query_local_dbms(status, io_buff_in, logical_dbms, table_name, conditions, sql_command, nodes_count):
    ...
    if not db_info.process_sql_stmt(status, anylog_cursor, sql_command):
        ...
        if db_info.is_row_by_row(logical_dbms):
            ret_val = query_row_by_row(status, anylog_cursor, io_buff_in, conditions, sql_time, sql_command, nodes_count)
        else:
...
# =======================================================================================================================
# Get the query and params from a test file and re-execute the query.
# Compare the new results to the test query run.
# Example: test case where source = !test_dir/output_test.out and output = stdout
# or
# Get the query and params from all the files in the source dir and re-execute the query for each.
# Compare the new results to the test query run.
# Example: test suite where source = !test_dir/output_test.out and inform = stdout and subdir = true
# =======================================================================================================================
def test_cases(status, io_buff_in, cmd_words, trace):

    offset = get_command_offset(cmd_words)

    keywords = {"source": ("str", True, False, True),     # The path to the file or directory
                "inform": ("str", False, False, False),   # Destination of output - examples: stdout and inform = stdout@!qa_node and inform = dbms.qa.testing@!qa_node
                "subdir": ("bool", False, False, True),   # include sub-dir
                "time" : ("bool", False, False, True),    # Add time comparison
                "dest" : ("str", False, False, True) # Destination Operator nodes
                }

    test_type = cmd_words[offset + 1]       # Test case or test suite
    ...
    dest = '(' + interpreter.get_one_value_or_default(conditions, "dest", "") + ')' # Get destination nodes

    if test_type == "case":
        # Only a single file is tested
        files_list = [source]
        ret_val = process_status.SUCCESS

    else:
        # All the files in the suite are tested
        src_dir, name_prefix, type_prefix = utils_io.extract_path_name_type(source)
        inc_sub_dir = interpreter.get_one_value_or_default(conditions, "subdir", False)

        if name_prefix and name_prefix[-1] == '*':
            name_prefix = name_prefix[:-1]

        files_list = []
        ret_val = utils_io.search_files_in_dir(status, files_list, src_dir, name_prefix, type_prefix, True, inc_sub_dir)


    if ret_val:
        return [ret_val, None]

    time_compare = interpreter.get_one_value(conditions, "time")       # Returns True if time is considered in the comparison
    if time_compare:
        # Consider execution time in the comparison
        options = " and option = time"          # transformed to option on the query
    else:
        options = ""

    # Get destination for test output
    # Dest can be stdout (default) or server@ip:port
    inform = interpreter.get_all_values(conditions, "inform")
    if inform:
        # example: and inform = stdout and inform = stdout@!qa_node and inform = dbms.qa.testing@!qa_node
        for entry in inform:
            options += " and option = %s" % entry       # Transformed to option on the query call


    for file_name in files_list:

        output_header = utils_io.get_header_info(status, file_name)        # Ge the info from the header of a test file

        if not output_header or not len(output_header):
            ret_val = process_status.Err_test_file_struct
        else:
            dir_name, s_file_name, file_type = utils_io.extract_path_name_type(file_name)

            new_file_name = "%srun_of_%s.%s" % (dir_name, s_file_name, file_type)

            utils_io.delete_file(new_file_name, False)  # delete tje file of the previous run
            ...
            # Test that the keys are represented in the header
            for key in keys_needed:
                if not key in output_header:
                    status.add_error("The '%s' info is missing in the file header: %s" % (key, file_name))
                    ret_val = process_status.Err_test_file_struct
                    break

            query_options = ""              # optional instructions for the query execution
            optional_keys = ["Include",
                             "Extend"]
            for key in optional_keys:
                if key in output_header:
                    value = output_header[key]
                    query_options += " and %s = %s" % (key.lower(), value)

            if not ret_val:
                sql_query = "run client %s sql %s format = %s and test = true %s%s and file = %s and source = %s \"%s\"" % \
                            (dest, output_header["DBMS"], output_header["Format"],\
                            options, query_options, new_file_name, file_name, output_header["Command"])

                ret_val = process_cmd(status, sql_query, False, None, None, io_buff_in)

    return [ret_val, None]

# =======================================================================================================================
# Compare query or process output to a trusted output
# Example: analyze output where file = !test_dir/main_query.out and source = !test_dir/tested_query.out and option = time
# =======================================================================================================================
def analyze_output(status, io_buff_in, cmd_words, trace):
    #                              Must     Add      Is
    #                              exists   Counter  Unique

    offset = get_command_offset(cmd_words)

    keywords = {"file": ("str", True, False, True),
                "source": ("str", True, False, True),
                "option" : ("str", False, False, False),
                }
    ...
    file_name, source, options = interpreter.get_multiple_values(conditions, ["file", "source", "option"], None)

    result, compare_message = utils_io.analyze_file(status, source, file_name, options)

    post_process_command(status, io_buff_in, ret_val, False, False, compare_message, None)

    # utils_print.output(compare_message, True)
    ...
    if not ret_val:
        ...
        if db_info.is_dbms(logical_dbms):
            if src_command:
                ...
                if (not ret_val or ret_val == process_status.Empty_data_set) and message and al_auth.is_node_authentication():
                    ...
                    if not permissions.permission_process(status, 0, ip_port, public_key, command, logical_dbms, select_table_name):
                        ...
                        if not message:
                            ...
                            if index == -1:
                                ...
                                ret_val = process_query_sequence(status, io_buff_in, query_words, logical_dbms,
                                                                 select_table_name, conditions, src_command, 1)
                        else:
                            ...
                            if not ret_val:
                                if workers_pool != None and workers_pool.is_active():
                                    ...
                                    ret_val = process_query_sequence(status, io_buff_in, query_words, logical_dbms,
                                                                     select_table_name, conditions, sql_command, 1)
    ...
    if not ret_val:
        ...
        if counter_threads:
            ...
            for index, entry in enumerate(sql_queries):
                ...
                task.set_cmd(process_query_sequence, [cmd_words, logical_dbms, sql_tables[index], conditions, entry, 1])
...
def _process_id(status, io_buff_in, cmd_words, trace):
    ...
    return ret_val
# =======================================================================================================================
# Create a private and public key for a node:
# id create keys for node where password = abc
# =======================================================================================================================
def create_node_keys(status, io_buff_in, cmd_words, trace):

    if utils_data.test_words(cmd_words, 5, ["where", "password", "="]):
        password = cmd_words[8]
        ret_val = al_auth.create_node_id(status, password, False)
    else:
        ret_val = process_status.ERR_command_struct
    return [ret_val, None]
# =======================================================================================================================
# Create a private and public key for a user:
# id create keys where password = abc  <- return keys to the user on stdout
# id create keys where password = abc  and keys_file = !usb_path/my_keys <- write keys to a file
# =======================================================================================================================
def create_user_keys(status, io_buff_in, cmd_words, trace):

    ret_val = process_status.ERR_command_struct
    if utils_data.test_words(cmd_words, 3, ["where", "password", "="]):
        words_count = len(cmd_words)
        password = params.get_value_if_available(cmd_words[6])
        if words_count == 7:
            # return the keys to the user
            private_str, public_str = al_auth.create_keys(password)
            if not private_str or not public_str:
                ret_val = process_status.Failed_to_generate_keys
            else:
                utils_print.output("\r\n%s\r\n%s" % (public_str, private_str), True)
                ret_val = process_status.SUCCESS
        elif words_count == 11 and utils_data.test_words(cmd_words, 7, ["and", "keys_file", "="]):
            keys_file = params.get_value_if_available(cmd_words[10])
            ret_val = al_auth.create_keys_to_file(status, password, keys_file)

    return [ret_val, None]
# =======================================================================================================================
# Sign a JSON policy using the private key
# Example: 'id sign [JSON Policy] where key = [private key] and Password = [password]'
# Or: id sign [JSON Policy] where Password = [password]
# =======================================================================================================================
def sign_json_policy(status, io_buff_in, cmd_words, trace):

    offset = get_command_offset(cmd_words)

    script = params.get_value_if_available(cmd_words[2 + offset])

    keywords = {
        "password": ("str", True, False, True),  # Command to validate
        "key": ("str", False, False, True),  # Command to validate
    }
    ...
    private_str = interpreter.get_one_value_or_default(conditions, "key", None)  # Must have a value
    if not private_str:
        # user did not provide a private key - get private key from node
        private_str = al_auth.get_node_private_key(status)

    if not ret_val:
        if not script:
            status.add_error("Policy assigned to '%s' is not provided" % cmd_words[2 + offset])
            ret_val = process_status.Needed_policy_not_available
        elif not private_str:
            status.add_error("Private key not provided")
            ret_val = process_status.Private_key_not_available
        else:
            ret_val, signed_script = sign_param(status, script, private_str, password)

            if not ret_val and signed_script:
                if offset:
                    # assign to the variable provided
                    params.add_param(cmd_words[0], signed_script)
                else:
                    # Change the Policy provided
                    if len(cmd_words[2]) == 1 or cmd_words[2][0] != '!':
                        # The policy has to be provided by a variable
                        status.add_error("The policy is not provided by an AnyLog variable")
                        ret_val = process_status.Wrong_policy_structure
                    else:
                        params.add_param(cmd_words[2][1:], signed_script)

    return [ret_val, None]

# =======================================================================================================================
# Validate that the policy was signed using the private key associated with the public key on the policy.
# =======================================================================================================================
def verify_signature(status, io_buff_in, cmd_words, trace):

    offset = get_command_offset(cmd_words)
    script = params.get_value_if_available(cmd_words[2 + offset])
    ret_code = authenticate_str(status, script)
    if ret_code == False:
        outstr = "Failed"
    else:
    ...
    if offset:
        params.add_param(cmd_words[0], str(outstr))
    else:
        utils_print.output(outstr, True)

    return [process_status.SUCCESS, None]

# =======================================================================================================================
# Encrypt a message using the public key of the receiver
# EXAMPLE: id encrypt !message !public_key
# =======================================================================================================================
def encrypt_message(status, io_buff_in, cmd_words, trace):

    ret_val = process_status.SUCCESS
    offset = get_command_offset(cmd_words)
    message = params.get_value_if_available(cmd_words[2 + offset])
    public_key = params.get_value_if_available(cmd_words[3 + offset])

    outstr = al_auth.encrypt(status, public_key, message)

    if not outstr:
        status.add_error("Encryption using public key failed")
        ret_val = process_status.ERR_command_struct
    else:
        if offset:
            params.add_param(cmd_words[0], str(outstr))
        else:
            params.add_param(cmd_words[2][1:], str(outstr))

    return [ret_val, None]
# =======================================================================================================================
# Decrypt a message using the private key of the receiver
#  example: id decript !message where key = !private_key and password = !my_password
#           id decript !message where password = !my_password
# =======================================================================================================================
def decrypy_message(status, io_buff_in, cmd_words, trace):
    ...
    if words_count == (7 + offset) and utils_data.test_words(cmd_words, 3 + offset, ["where", "password", "="]):
        private_key = al_auth.get_node_private_key(status)
        password =  params.get_value_if_available(cmd_words[6 + offset])
    elif words_count == (11 + offset) and utils_data.test_words(cmd_words, 3 + offset, ["where", "key", "="])\
                                      and utils_data.test_words(cmd_words, 7 + offset, ["and", "password", "="]):
        private_key = params.get_value_if_available(cmd_words[6 + offset])
        password =  params.get_value_if_available(cmd_words[10 + offset])
    else:
        return [process_status.ERR_command_struct, None]

    outstr = al_auth.decrypt(status, private_key, password, message)

    if not outstr:
        status.add_error("Decryption using private key failed")
        ret_val = process_status.ERR_command_struct
    else:
        if offset:
            params.add_param(cmd_words[0], str(outstr))
            ...
            # Change the Policy provided
            if len(cmd_words[2]) == 1 or cmd_words[2][0] != '!':
                # The message has to be provided by a variable
                status.add_error("The message is not provided by an AnyLog variable")
                ret_val = process_status.Wrong_policy_structure
            else:
                utils_print.output(outstr, True)
...
# -----------------------------------------------
# validate authorization for a member to use a process
# Example: id validate where key = !public_key and command = !command
# id validate where key = !public_key and command = "select" and dbms = "lsl_demo" and table = "ping_sensor"
# -----------------------------------------------
def is_authorized(status, io_buff_in, cmd_words, trace):
    ...
    keywords = {
                "command": ("str", True, False, True),  # Command to validate
                "key": ("str", True, False, True),  # The Public Key
                "dbms": ("str", False, False, True),  # dbms to validate
                "table": ("str", False, False, True),  # table to validate
                }
    ...
    ret_code = test_permission(status, conditions)
    if ret_code == False:
        outstr = "Failed"
    else:
        outstr = "Verified"
    ...
    if offset:
        params.add_param(cmd_words[0], outstr)
    else:
        utils_print.output(outstr, True)
...
# -----------------------------------------------
# Add a user to the a local list of users that can connect to this node
# Example: id add user where name = ori and password = 123 and expiration = 2 minutes
# -----------------------------------------------
def add_user(status, io_buff_in, cmd_words, trace):

    #                               Must     Add      Is
    #                              exists   Counter  Unique

    keywords = {
        "name": ("str", True, False, True),  # user name
        "password": ("str", True, False, True),  # user password
        "type": ("str", False, False, True),  # user type: admin or user
        "expiration": ("int.time", False, False, True),
    }
    ...
    if cmd_words[3] == "where":
        ...
        if not ret_val:
            user_name = interpreter.get_one_value(conditions, "name")
            password = interpreter.get_one_value(conditions, "password")
            user_type = interpreter.get_one_value_or_default(conditions,"type", "user")
            expiration = interpreter.get_one_value_or_default(conditions,"expiration", 0)

            ret_val = al_auth.add_local_user(status, user_name, user_type, password, expiration)
    else:
        ret_val = process_status.ERR_command_struct
...
# -----------------------------------------------
# Add a user to the a local list of users that can connect to this node
# Example: id remove user where name = john and password = abc
# -----------------------------------------------
def remove_user(status, io_buff_in, cmd_words, trace):

    if utils_data.test_words(cmd_words, 3 , ["where", "name", "="]):
        user_name = cmd_words[6]
        ret_val = al_auth.remove_local_user(status, user_name)
    else:
        ret_val = process_status.ERR_command_struct
...
# Example: id update user password where name = ori and old = 123456 and new = iugsek88ekA
# -----------------------------------------------
def update_password(status, io_buff_in, cmd_words, trace):

    keywords = {
        "name": ("str", True, False, True),  # user name
        "old": ("str", True, False, True),  # user password
        "new": ("str", False, False, True),
    }
    ...
    if cmd_words[4] == "where":
        ...
        if not ret_val:
            user_name = interpreter.get_one_value(conditions, "name")
            old_password = interpreter.get_one_value(conditions, "old")
            new_password = interpreter.get_one_value(conditions, "new")
            ret_val = al_auth.update_local_user_password(status, user_name, old_password, new_password)
    else:
        ret_val = process_status.ERR_command_struct

    return [ret_val, None]

def build_my_keys(status, io_buff_in, cmd_words, trace):
    ret_val = al_auth.build_keys(status)
    return [ret_val, None]

# =======================================================================================================================
# Generate a Certificate Authority
# Create a private and public key for the certificate authority
# =======================================================================================================================
def generate_certificate_auth(status, io_buff_in, cmd_words, trace):

    if len(cmd_words) == 4:
        # id generate certificate request
        conditions = None  # no password
        ret_val = process_status.SUCCESS
    elif len(cmd_words) >= 8 and cmd_words[4] == "where":
        #                               Must     Add      Is
        #                               exists   Counter  Unique
        keywords = {"password": ("str", False, False, True),
                    "country": ("str", False, False, True),
                    "state": ("str", False, False, True),
                    "locality": ("str", False, False, True),
                    "org": ("str", False, False, True),
    ...
    if not ret_val:
        ret_val =  al_auth.generate_certificate_authority(status, conditions)
    return [ret_val, None]

# =======================================================================================================================
# Generate a Certificate Request for a node in the network
# id generate certificate request
# id generate certificate request where password = abc
# =======================================================================================================================
def generate_certificate_request(status, io_buff_in, cmd_words, trace):
    ...
    if len(cmd_words) == 4:
        ...
        keywords = {"password": ("str", False, False, True),
                    ...
                    "locality": ("str", False, False, True),
                    "org": ("str", False, False, True),
                    "alt_names": ("str", False, False, True),
                    "hostname": ("str", False, False, True),
                    "ip": ("str", False, False, True)
                    }
        ret_val, counter, conditions = interpreter.get_dict_from_words(status, cmd_words, 5 , 0, keywords, False)
    ...
    if not ret_val:
        ret_val = al_auth.generate_certificate_request(status, conditions)
    return [ret_val, None]

# =======================================================================================================================
# Sign a certificate request by the certificate authority
# id sign certificate request where ca_org = "AnyLog Inc" and server_org = postman
# =======================================================================================================================
def sign_certificate_request(status, io_buff_in, cmd_words, trace):

    #                               Must     Add      Is
    #                               exists   Counter  Unique

    keywords = {"ca_org": ("str", True, False, True),      # the org name given to the CA
                "server_org": ("str", True, False, True),  # the org name given to the server
    }
    ret_val, counter, conditions = interpreter.get_dict_from_words(status, cmd_words, 5, 0, keywords, False)
    if not ret_val:
        if cmd_words[4] != "where":
            ...
            ret_val = al_auth.sign_certificate_request(status, conditions)

    return [ret_val, None]

# =======================================================================================================================
# Authenticate - Map the string to a JSON struct and call authenticate
# =======================================================================================================================
def authenticate_str(status, script):
    json_struct = utils_json.str_to_json(script)
    if not json_struct or not isinstance(json_struct, dict):
        status.add_error("JSON script format error")
        ret_val = False
    else:
        ret_val = authenticate(status, json_struct)
...
# =======================================================================================================================
def authenticate(status, json_struct):
    copy_struct = copy.deepcopy(json_struct)  # making a copy as the signature is removed
    key = next(iter(copy_struct.keys()))
    policy_terms = copy_struct[key]
    if not policy_terms or not isinstance(policy_terms, dict):
        status.add_error("Wrong format to the JSON script")
        ret_val = False
    else:
        if not "signature" in policy_terms or not "public_key" in policy_terms:
            if "name" in policy_terms:
                policy_info = f" and name: '{policy_terms['name']}'"
            else:
                policy_info = " (no policy 'name')"

            if "id" in policy_terms:
                policy_info += f" and id: '{policy_terms['id']}'"
            else:
                policy_info += " (no policy 'id')"

            status.add_error(f"Signature and public key are missing in Policy type: '{key}' {policy_info}")
            ret_val = False
        else:
            try:
                signature = policy_terms["signature"]

                public_str = al_auth.str_values_to_public_str(policy_terms["public_key"])

                # delete signature from the policy as the signature is nit available when policy is signed
                del policy_terms["signature"]

                if "ledger" in policy_terms:
                    # ledger status was added after the signature
                    del policy_terms["ledger"]

                message = utils_json.to_string(copy_struct)
                ret_val = al_auth.verify(public_str, signature, message)
            except:
                ret_val = False
    return ret_val
# =======================================================================================================================
# Sign a JSON policy which is addressed by a parameter
# =======================================================================================================================
def sign_param(status, word_val, private_str, password):

    if word_val[0] == '[' and word_val[-1] == ']':
        # Get the JSON from the list
        script = params.get_value_if_available(word_val[1:-1])
    else:
        script = params.get_value_if_available(word_val)

    script = params.json_str_replace_key_with_value(script)
    json_struct = utils_json.str_to_json(script)
    if not json_struct or not isinstance(json_struct, dict):
        status.add_error("Wrong format to the JSON script")
        ret_val = process_status.ERR_wrong_json_structure
        ...
        ret_val, signed_script = sign_policy(status, json_struct, private_str, password)

    return [ret_val, signed_script]


# =======================================================================================================================
# Sign a JSON policy
# =======================================================================================================================
def sign_policy(status, source_policy, private_str, password):

    policy = copy.deepcopy(source_policy)

    key = next(iter(policy.keys()))
    policy_terms = policy[key]
    signed_script = None

    if not policy_terms or not isinstance(policy_terms, dict):
        ret_val = process_status.ERR_wrong_json_structure
    else:
        if "signature" in policy_terms:
            status.add_error("Duplicate signatures on the same policy")
            ret_val = process_status.Duplicate_signatures
        else:
            # get the public key using the private key signing
            public_key = al_auth.public_from_private(private_str, password)

            if public_key == "":
                ret_val = process_status.Wrong_password
            else:
                # skip: '-----BEGIN PUBLIC KEY-----\n' '\n' '\n' '-----END PUBLIC KEY-----\n'
                policy_terms["public_key"] = al_auth.get_public_key_chars(public_key)

                # Update the policy to include all needed values (including ID and Time)
                ret_val = policies.add_json_id_date(policy)  # if there is no ID to the JSON, add ID
                if not ret_val:

                    message = utils_json.to_string(policy)
                    signature = al_auth.sign(status, private_str, password, message)
                    if not signature:
                        ret_val = process_status.Failed_message_sign
                        status.add_error(f"Failed to sign a policy of type '{key}'")
                    else:
                        policy_terms["signature"] = signature
                        signed_script = utils_json.to_string(policy)

    return [ret_val, signed_script]


# =======================================================================================================================
# Return a private key or a public key
# Examples:
# 1. get public key
# 2. get public key using keys_file = my_file
# 3. get private key
# 4. get private key using keys_file = my_file
# =======================================================================================================================
def get_key_by_cmd_line(status, cmd_words, offset):
    key_type = cmd_words[offset + 1]
    words_count = len(cmd_words)
    ret_val = process_status.SUCCESS
    key_string = ""

    if words_count > (offset + 3):
        if cmd_words[offset + 3] == "where" and words_count > (offset + 4):
            keywords = {"password": ("str", False, False, True),
                        "keys_file": ("str", False, False, True)  # optional output file for the keys
                        }
            ret_val, counter, conditions = interpreter.get_dict_from_words(status, cmd_words, 4 + offset, 0, keywords,
                                                                           False)
            if not ret_val:
                keys_file = interpreter.get_one_value(conditions,
                                                      "keys_file")  # A file that stores the private and public keys
        else:
            ret_val = process_status.ERR_command_struct
    ...
    if not ret_val:
        if words_count == (offset + 3):
            if key_type == "public":
                key_string = al_auth.get_node_public_key(status)  # the node id is the public key
            else:
                key_string = al_auth.get_node_private_key(status)
        elif keys_file:
            ...
            if not dir_name:
                ...
                if not file_type:
                    keys_file += ".pem"
                ret_val, pf_name = utils_io.make_path_file_name(status, dir_name, keys_file, "")
            else:
                pf_name = dir_name + file_name
                if not file_type:
                    pf_name += ".pem"
            if not ret_val:
                if key_type == "public":
                    key_string = al_auth.read_pem_file(status, pf_name, False)
                else:
                    key_string = al_auth.read_pem_file(status, pf_name, True)
    return [ret_val, key_string]

# =======================================================================================================================
# Test permissions - given a public key of a member and the process to do
# Example: id validate where key = !public_key and command = "select" and dbms = "lsl_demo" and table = "ping_sensor"
# =======================================================================================================================
def test_permission(status, conditions):
    public_key = interpreter.get_one_value(conditions, "key")
    command = interpreter.get_one_value(conditions, "command")

    if not public_key:
        status.add_error("Missing member's public key for validation")
        return False

    public_string = al_auth.get_public_key_chars(public_key)

    if not command:
        status.add_error("Missing command for validation")
        return False

    dbms_name = interpreter.get_one_value(conditions, "dbms")
    table_name = interpreter.get_one_value(conditions, "table")

    ret_val = permissions.permission_process(status, 0, "NA", public_string, command, dbms_name, table_name)
...
def _exit(status, io_buff_in, cmd_words, trace):
    ...
    elif words_count == 3 and process_name == "mqtt" and cmd_words[2].isdecimal():
        client_id = int(cmd_words[2])
    ...
    elif words_count == 3 and process_name == "scheduler" and cmd_words[2].isdecimal():
        # exit specific scheduler
        task_scheduler.set_not_active(int(cmd_words[2]))
...
def _python(status, io_buff_in, cmd_words, trace):
    ...
    if words_count == 2 + offset:
        ...
        pre_string = utils_data.get_str_from_array(cmd_words, offset + 1, 0)
...
def _echo(status, io_buff_in, cmd_words, trace):
    ...
    if cmd_words[1] == "message":  # a tcp sql message
        ...
        if unique_job_id:  # Source node is True if the message is a reply
            ...
            integrate_print_reply(status, mem_view, unique_job_id, None, 0, 0, False, None)
        else:
...
# =======================================================================================================================
def integrate_print_reply(status, io_buff_in, unique_job_id, job_instance, r_id, n_ret_val, json_format, message):
    '''
    ...
    if is_last:
        ...
        if j_instance.is_job_active() and j_instance.get_unique_job_id() == unique_job_id:
            j_handle = j_instance.get_job_handle()
            is_done = j_instance.count_nodes_replied()  # Count the number of nodes replied and return TRUE is all replied
            # Flag that the job is completed
            if is_done:
                ...
                if assignment:
                    ...
                    offset = 0
                    if j_instance.get_nodes_participating() == 1:
...
def get_processes_stat(status, io_buff_in, cmd_words, trace):
    ...
    test_active = {
        #                   Get is acrive | Comment
        "TCP"  :        (net_utils.is_active_connection(0), tcpip_server.get_info()),
        "REST" :        (net_utils.is_active_connection(1), http_server.get_info()),
        "Operator" :    (aloperator.is_running, aloperator.get_info),
        "Publisher" :   (alpublisher.is_running, None),
        "Blockchain Sync": (bsync.is_running, bsync.get_info),
        "Scheduler" :   (task_scheduler.is_running, task_scheduler.get_info),
        "Distributor":  (aldistributor.is_distr_running, None),
        "Blobs Archiver":     (alarchiver.is_arch_running, None),
        "Consumer" :    (alconsumer.is_consumer_running, alconsumer.get_info),
        "MQTT":         (mqtt_client.is_running, None),
        "Message Broker": (net_utils.is_active_connection(2), message_server.get_info()),
        "SMTP":         (utils_output.is_smtp_running, utils_output.get_smtp_info),
        "Streamer":     (streaming_data.is_running, streaming_data.get_info),
        "Query Pool":   (is_query_pool_active, get_query_pool_info),
        "Kafka Consumer": (al_kafka.is_running, al_kafka.get_info),
    }
    ...
    if not ret_val:
        ...
        for entry, methods in test_active.items():
            get_state = methods[0]     # A method or a variable to determine status
            if callable(get_state):
                is_active = get_state()     # Function
            else:
                is_active = get_state       # param
            if is_active:
                value = 'Running'
            else:
                value = 'Not declared'
            ...
            if reply_format == "json":
                ...
                reply_dict[entry] = process_info
            else:
                processes.append((entry, value, info_str))
...
# Example: run mqtt client where broker = "mqtt.eclipse.org" and (topic = lsl_data.ping_sensor.$SYS/#.0 and topic = 3)
# run mqtt client where broker = "mqtt.eclipse.org" and topic = (name = $SYS/# and dbms = lsl_demo and table =ping_sensot and qos = 2)
# =======================================================================================================================
def _run_mqtt_client(status, io_buff_in, cmd_words, trace):
    ...
    if ret_val:
        return ret_val
    ret_val = interpreter.add_value_if_missing(status, conditions, "bwatch_dir", "!bwatch_dir")
    if ret_val:
        return ret_val
    ret_val = interpreter.add_value_if_missing(status, conditions, "blobs_dir", "!blobs_dir")
    if ret_val:
        return ret_val
...
def start_tcp_threads(url, port, url_internal, port_internal, is_bind, url_bind,  workers_count, trace):
    ...
    port_bind = port if port else port_internal     # Always show port

    ret_val = net_utils.add_connection(0, url, port, url_internal, port_internal, url_bind,  port_bind)
    if ret_val:
        return ret_val
    ...
    t = threading.Thread(target=tcpip_server.tcp_server, args=(url_internal, port_internal, is_bind, workers_count, trace))
    t.start()
    # t.join()
...
def _execute_from(status, io_buff_in, cmd_words, trace):
    ...
    bring_type = utils_json.get_bring_type(status, cmd_words[offset + 2])
    if bring_type == -1:
        return process_status.ERR_command_struct
    ...
    if not ret_val:
        if utils_json.is_bring("table", bring_type):
            # Change the JSON format to table format
            if dynamic_string:
                dynamic_string = set_json_as_table(dynamic_string, bring_type)
...
def blockchain_insert(status, io_buff_in, cmd_words, trace, func_params):
    ...
    ret_val = blockchain_insert_all(status, mem_view, policy, is_local, None, master, platform)
...
# Validate policy structure + update policy in all platforms
# -----------------------------------------------------------------------------------------------------
def blockchain_insert_all(status, mem_view, policy, is_local, blockchain_file, master, platform):
    '''
    status - The process status object
    mem_view - buffer for outgoing messages
    policy - the policy to update
...
def _run_operator(status, io_buff_in, cmd_words, trace):
    ...
    keywords = {"watch_dir": ("dir", False, False, True),
                ...
                "company": ("str", False, False, True),
                "delete_json": ("bool", False, False, True),
                "delete_sql": ("bool", False, False, True),
                "compress_json": ("bool", False, False, True),
                "compress_sql": ("bool", False, False, True),
                "move_json": ("bool", False, False, True),
                ...
                "archive": ("bool", False, False, True),       # move data to archive
    ...
    # test that watch_dir is defined
    ret_val = interpreter.test_add_value(status, conditions, "watch_dir", "!watch_dir", "dir")  # add value if not in conditions.
    if ret_val:
        return ret_val

    # test that err_dir is defined
    ret_val = interpreter.test_add_value(status, conditions, "err_dir", "!err_dir", "dir")
    if ret_val:
        return ret_val

    # test that bkup_dir is defined
    ret_val = interpreter.test_add_value(status, conditions, "bkup_dir", "!bkup_dir", "dir")
    if ret_val:
        return ret_val

    # test that archive_dir is defined
    ret_val = interpreter.test_add_value(status, conditions, "archive_dir", "!archive_dir", "dir")
    if ret_val:
        return ret_val

    # test that distr_dir is defined
    ret_val = interpreter.test_add_value(status, conditions, "distr_dir", "!distr_dir", "dir")
    if ret_val:
        return ret_val
...
# =======================================================================================================================
#  Run Publisher process. It transfers files from the watch directory to the Operator
# Example: run publisher where limit_tables = (lsl_demo.ping_sensor, lsl_demo.percentagecpu_sensor) and dbms_name = file_name[0] and table_name = file_name[4]
# =======================================================================================================================
def _run_publisher(status, io_buff_in, cmd_words, trace):
    #                          Must     Add      Is
    #                          exists   Counter  Unique
    keywords = {"watch_dir": ("dir", False, False, True),
                "err_dir": ("dir", False, False, True),
                "bkup_dir": ("dir", False, False, True),
                "archive_dir": ("dir", False, False, True),
                "file_type": ("str", False, False, False),
                "master_node": ("ip.port", False, False, True),
                "dbms_name": ("int.from.file_name", False, False, True),
                "table_name": ("int.from.file_name", False, False, True),  # File, dbms
                "source": ("int.from.file_name", False, False, True),
                # section in the file name with the ID of source (meter, device, etc)
                "hash": ("int.from.file_name", False, True, True),  # section in the file name with the hash value
                "instructions": ("int.from.file_name", False, False, True),  # mapping instructions JSON to table struct
                "delete_json": ("bool", False, False, True),
                "delete_sql": ("bool", False, False, True),
                "compress_json": ("bool", False, False, True),
                "compress_sql": ("bool", False, False, True),
                "move_json": ("bool", False, False, True),
                "move_sql": ("bool", False, False, True),
                "limit_tables": ("str", False, False, False),
                "company": ("str", False, False, True),
                }

    if alpublisher.is_running:
        status.add_error("Duplicate call to initiate Publisher process")
        return process_status.Process_already_running
    ...
    ret_val = interpreter.add_value_if_missing(status, conditions, "blockchain_file", "!blockchain_file")
    if ret_val:
        return ret_val

    if "limit_tables" in conditions.keys():
        # validate that there are operators supporting these table
        blockchain_file = interpreter.get_one_value(conditions, "blockchain_file")
        ret_val = blockchain.validate_operators_support(status, conditions["limit_tables"], blockchain_file)
        if ret_val:
            return ret_val

    if not "file_type" in conditions.keys():
        # add the default - SQL and JSON
        interpreter.add_value(conditions, "file_type", "json")
        interpreter.add_value(conditions, "file_type", "sql")

    t = threading.Thread(target=alpublisher.run_publisher, args=("dummy", conditions), name="main_publisher")
    t.start()
...
# =======================================================================================================================
#  Run HA process. It transfers files to the cluster member nodes
#  Example: run data distributor
# =======================================================================================================================
def _run_distributor(status, io_buff_in, cmd_words, trace):
    #                                Must     Add      Is
    #                                exists   Counter  Unique
    keywords = {
                "distr_dir": ("str", False, False, True),
                "archive_dir": ("str", False, False, True),
               }

    if aldistributor.is_distr_running():
        status.add_error("Duplicate call to initiate distributor process")
        return process_status.Process_already_running
    ...
    distr_dir = interpreter.get_one_value_or_default(conditions, "distr_dir", "!distr_dir")
    ret_val = utils_io.test_dir_exists_and_writeable(status, distr_dir, True)
    if ret_val:
        return ret_val

    archive_dir = interpreter.get_one_value_or_default(conditions, "archive_dir", "!archive_dir")
    ret_val = utils_io.test_dir_exists_and_writeable(status, archive_dir, True)
    if ret_val:
        return ret_val

    t = threading.Thread(target=aldistributor.data_distributor, args=("dummy", conditions), name="main_distributor")
    t.start()
...
# =======================================================================================================================
#  Run consumer process. It is part of the HA process to provide a complete data set of the cluster on the current node.
#  Example: run data consumer where start_date = -30d
# =======================================================================================================================
def _run_consumer(status, io_buff_in, cmd_words, trace):
    #                                Must     Add      Is
    #                                exists   Counter  Unique
    keywords = {
                "start_date": ("date", True, False, True),
                "end_date":   ("date", False, False, True),
                "mode":       ("str", False, False, True),      # default is "active", suspend does not pull data
               }

    if alconsumer.is_consumer_running():
        status.add_error("Duplicate call to initiate HA consumer process")
        return process_status.Process_already_running
    ...
    t = threading.Thread(target=alconsumer.data_consumer, args=("dummy", conditions), name="main_consumer")
    t.start()
...
def _process_task(status, io_buff_in, cmd_words, trace):
    ...
    return ret_val
# =======================================================================================================================
# Puase processing for the specified duration (in seconds)
# Example sleep 10
# sleep 10 or file update !blockchain_file
# =======================================================================================================================
def _sleep(status, io_buff_in, cmd_words, trace):
    words_count = len(cmd_words)
    if words_count == 1:
        return process_status.ERR_command_struct

    seconds = params.get_value_if_available(cmd_words[1])
    ...
    elif len(cmd_words) == 6 and utils_data.test_words(cmd_words, 2 ,  ["or", "file", "update"]):
        # wait for the file to be changed - or max time
        file_name = params.get_value_if_available(cmd_words[5])
        file_time = utils_io.get_file_modified_time(file_name)
        try:
            for i in range (sleep_time):
                time.sleep(1)
                new_time =  utils_io.get_file_modified_time(file_name)
                if new_time and new_time != file_time:
                    break
        except:
...
# =======================================================================================================================
def process_cmd(status, command, print_cmd, source_ip, source_port, io_buffer_in):
    '''
    ...
    if not error_code:
        ...
        if not activation_license_:
            # Only set license allowed or exit
            if len(sub_str) >= 2 and (sub_str[1] == "license" or sub_str[1] == "connections" or sub_str[1] == "status" or sub_str[0] == "exit"):
                pass    # Allow get or set license command, get status or get connections.
            else:
                utils_print.output(license_msg_ + "\r\nStatus: Missing Valid AnyLog License Key", True)  # print error message
                return process_status.Wrong_license_key
        ...
        for x in range(first_word, y):  # go over the words in the command line
            ...
            if cmd_sub_string in commands.keys():
                ...
                if cmd_sub_string != "job" and (source_ip or is_rest_caller) and al_auth.is_node_authentication():
                    # Use the public key to validate permissions
                    public_key = status.get_pub_key()
                    if not permissions.permission_process(status, 0, f"{source_ip}:{source_port}", public_key, cmd_sub_string, "", ""):
    ...
    if not error_code:
        ...
        if status.get_err_value() != error_code:
            ...
            if not is_message and not first_word:  # if not assignment
                # if status.get_error_dest() == "stdout": # (main thread and TCP thread and Repeat thread)
                if not is_rest_caller:
                    utils_print.output(error_msg, True)  # print error message
            elif echo_queue:
                queue = echo_queue  # Get the method as the user may Null the method
                if queue:
                    queue.add_msg("Error executing: [%s] Message: [%s]" % (' '.join(sub_str), error_msg))
...
# =======================================================================================================================
def network_get_info(status, io_buff_in, al_cmd, cmd_name, nodes_types, dbms_name, table_name, trace):
    '''
        ...
        if not nodes_list:
            ...
            for index, entry in enumerate(nodes_list):
                ...
                if ip and port:
                    ...
                    if node_val:
                        if cmd_name == "Status":
                            aggregate_table.add_entry("status", ip_port, "  V")  # Node replied
                        else:
                            # Remove the [From Node 10.0.0.78:7848] prefix
                            index = node_val.find(']', 1)
                            if index > 0 and index < (len(node_val) - 2):
...
def test_network(status, io_buff_in, cmd_words, trace):
    ...
    ret_val, reply = network_get_info(status, io_buff_in, al_cmd, cmd_name, nodes_types, dbms_name, table_name, trace)
    return [ret_val, reply]
            ...
            if peer_reply and len(peer_reply) == 1:
                ...
                if node_databases and isinstance(node_databases, dict):
                    for dbms_name in node_databases:
                        ...
                        aggregate_table.add_entry(operator[2], dbms_name, "    V")    # The value as f( node id and dbms name )
            ...
            if peer_reply and len(peer_reply) == 1:
                ...
                if node_partitions and isinstance(node_partitions, dict):
                    for dbms_table_name, partition_info in node_partitions.items():
                        ...
                        aggregate_table.add_entry(operator[2], partition_details, "    V")    # The value as f( node id and dbms name )
...
def test_cluster_setup(status, io_buff_in, cmd_words, trace):
    ...
    info_table.append(("Distributor", "Running" if aldistributor.is_distr_running() else "Disabled"))
    info_table.append(("Consumer", "Running" if alconsumer.is_consumer_running() else "Disabled"))
...
def test_node(status, io_buff_in, cmd_words, trace):
    ...
    if not dest_ip or not dest_port:
        utils_print.output("No TCP IP and Port in use\r\n", False)
    else:
        command = f"run client {dest_ip}:{dest_port} get status"
        utils_print.output(command + "\r\n", False)
        ret_val = process_cmd(status, command, False, None, None, io_buff_in)

    time.sleep(1)  # Wait for the reply
    ...
    # TODO: IT takes IP and PORT from TCP - Note rest - Make change
    if not rest_ip_port:
        utils_print.output("No REST IP and Port in use\r\n", False)
    else:
        command = "rest get where url = %s and type = info and command = \"get status where format = json\" and User-Agent = AnyLog/1.23" % rest_ip_port
        utils_print.output(command + '\r\n', False)
        ret_val = process_cmd(status, command, False, None, None, io_buff_in)
...
# Example create:  set license main_keys 123 2023-04-20
# Keys file is once created using: id create keys  where password = 123 and keys_file = license_keys
# To create a license key:
#       set license where key_file = license_keys and password = 123 and expiration = 2023-04-12 and company = Socnoc and type = beta
# Get the private key: get private key where keys_file = main_keys
# Example:
# ------------------------------------------
def set_license(status, io_buff_in, cmd_words, trace):
    '''
    To create a license key:
    set license where key_file = [name of a PEM file] and password = [private key password] and expiration = [%D-%m-%y] and company = [company name] and type = [commercial or beta]

    To activate a license key:
    set license where activation_key = [activation key]
    '''

    global activation_license_
    global expiration_
    global license_type_
    global licensed_company_
    global license_public_str_
    ...
    keywords = {
        ...
        #                    exists   Counter  Unique
        # To generate a key
        "key_file": ("str", False,    True,   True),        # The PEM file with the private key
        "password": ("str", False,    True,   True),        # Password used to protect the key in the PEM file
        "expiration": ("str", False,   True,   True),        # %D-%m-%y format
        "company": ("str",  False,     True,   True),        # Company name woith the license
        "type":    ("str", False,      True,   True),        #  commercial or beta
        # To activate a key
        "activation_key": ("str", False, True, True),  # commercial or beta
    }
    ...
    if not ret_val:
        activation_key = interpreter.get_one_value(conditions, "activation_key")

        if activation_key and counter == 1:
            #  validate a license key
            if len(activation_key) <= 268:     # 256 is the signature + 10 is the date YYYY-MM-DD + 1 char is the license type + followed by company name
                status.add_error("Wrong format of license key")
                ret_val = process_status.ERR_process_failure
            else:
                ...
                company = activation_key[267:]
                if not utils_columns.validate_date_string(expiration, "%Y-%m-%d"):
                    utils_print.output("Wrong license date format", True)
                    ret_val = process_status.Wrong_license_key
                else:
                    source_str = expiration + license_type + company
                    is_valid = al_auth.verify(license_public_str_, signature, source_str)
                    if is_valid:
                        current_time = utils_columns.get_current_utc_time()
                        if expiration < current_time[:10]:
                            ret_val = process_status.Wrong_license_key
                        else:
                            expiration_ = expiration
                            license_type_ = "Commercial" if license_type == 'c' else 'Beta'
                            licensed_company_ = company
                            activation_license_ = True
                            ret_val, license_msg = get_license(status, io_buff_in, cmd_words, trace)    # Get and print the license message
                            utils_print.output(license_msg, True)
                    else:
                        ret_val = process_status.Wrong_license_key

        elif not activation_key and counter == 5:
            # generate the license key
            # Example: set license where
            key_file = interpreter.get_one_value(conditions, "key_file")
            password = interpreter.get_one_value(conditions, "password")
            expiration = interpreter.get_one_value(conditions, "expiration")
            company = interpreter.get_one_value(conditions, "company")
            license_type = interpreter.get_one_value(conditions, "type")

            cmd = f"get private key where keys_file = {key_file}".split()
            ret_val, private_str = get_key(status, io_buff_in, cmd, trace)
            if not ret_val:
                if not utils_columns.validate_date_string(expiration, "%Y-%m-%d") or len(expiration) != 10:
                    utils_print.output("Wrong license date format", True)
                    ret_val = process_status.ERR_command_struct
                elif license_type != "commercial" and license_type != "beta":
                    utils_print.output("Wrong license type ('commercial' or 'beta' is required", True)
                    ret_val = process_status.ERR_command_struct
                elif not len(company):
                    utils_print.output("Missing licensed company name", True)
                    ret_val = process_status.ERR_command_struct
                else:
                    license_type = 'b' if license_type == 'beta' else 'c'
                    source_str = expiration + license_type + company

                    signature = al_auth.sign(status, private_str, password, source_str)
                    if not signature:
                        utils_print.output("Failed to sign license with PEM file and password provided", True)
                    else:
                        utils_print.output(f"{signature}{source_str}", True)

        else:
            ret_val = process_status.ERR_command_struct


    return [ret_val, signature]
# ------------------------------------------
...
def get_node_status(status, io_buff_in, cmd_words, trace):
    ...
    ret_val = process_status.SUCCESS
    reply = params.get_node_name() + " running"
...
# get operator stat
# ------------------------------------------
def get_operator(status, io_buff_in, cmd_words, trace):
    ...
    words_count = len(cmd_words)
    if words_count >= 5 + offset:
        # show operator format = json/table/generic
        print_format = get_reply_format(status, cmd_words, words_count - 3 )
        if print_format:
            words_count -= 3        # Adjust command to ignore the format instruction
    else:
        print_format = None

    if words_count == 2 + offset:
        query_type = "info"         # Generic info
    elif words_count == 3 + offset:
        query_type = cmd_words[2 + offset]         # Statistics oon data load
    else:
        ret_val = process_status.ERR_command_struct

    if not ret_val:
        reply = aloperator.show_info(status, query_type, print_format)  # statistics
...
# Provide the info on the operator
# ------------------------------------------
def get_publisher(status, io_buff_in, cmd_words, trace):
    if alpublisher.is_running:
        reply = "Publisher process is running"
    else:
        reply = "Publisher is not running"
    reply += alpublisher.show_info()  # statistics

    return [process_status.SUCCESS, reply]
# ------------------------------------------
# Provide the info on the Distributor
# ------------------------------------------
def get_distributor(status, io_buff_in, cmd_words, trace):
    if aldistributor.is_distr_running():
        reply = "HA: Data Distribution Process is running"
    else:
        reply = "HA: Data Distribution Process is not running"
    reply += aldistributor.show_distr_info()  # statistics
    return [process_status.SUCCESS, reply]

# ------------------------------------------
# Provide the info on the Consumer
# ------------------------------------------
def get_consumer(status, io_buff_in, cmd_words, trace):
    if alconsumer.is_consumer_running():
        reply = "HA: Data Consumer Process\r\n"
        reply += alconsumer.get_info(status)
        reply += metadata.show_tsd_status("all")
    else:
        reply = "HA: Data Consumer Process is not running"
...
def get_msg_client_broker(status, io_buff_in, cmd_words, trace):
    ...
    return [ret_val, reply]
# ------------------------------------------------
# show Inserts on ths node
# ------------------------------------------------
def get_insert_info(status, io_buff_in, cmd_words, trace):
    reply = db_info.get_insert_stat(True)
    return [process_status.SUCCESS, reply]
# ------------------------------------------
# Get Node ID
# ------------------------------------------
def get_node_id(status, io_buff_in, cmd_words, trace):

    reply = al_auth.get_node_id(status)
    if reply:
        ret_val = process_status.SUCCESS
    else:
        if not al_auth.is_installed():
            status.add_error("Library cryptography not installed")
            ret_val = process_status.Failed_to_import_lib
        else:
            status.add_keep_error("Failed to retrieve node id")
            ret_val = process_status.ERR_process_failure

    return [ret_val, reply]
# ------------------------------------------
# Get hardware ID - a unique id of the motherboard
# ------------------------------------------
def get_hardware_id(status, io_buff_in, cmd_words, trace):

    reply = al_auth.get_hardware_id()
...
def get_virtual_tables(status, io_buff_in, cmd_words, trace):
    ...
    if not ret_val:
        ...
        if with_info:
            ...
            if al_auth.is_node_authentication():
                ret_val, public_key_str = al_auth.get_signatory_public_key(status)
                if not ret_val and public_key_str:
                    for entry in info_list:
                        dbms_name = entry[1]
                        table_name = entry[2]
                        ret_val, permissions_list = permissions.permissions_by_public_key(status, public_key_str)
                        if not permissions.permission_process(status, 0, None, public_key_str, None, dbms_name, table_name):
                            entry[5] = False
...
# get private key or get public key
# get private string or get public string

# Get the output as a string:
# get private key or get public key

# ------------------------------------------
def get_key(status, io_buff_in, cmd_words, trace):

    offset = get_command_offset(cmd_words)

    if cmd_words[2+ offset] == "key" or cmd_words[2+ offset] == "string":

        ret_val, reply = get_key_by_cmd_line(status, cmd_words, offset)
        if not ret_val:
            if not reply:
                ret_val = process_status.File_read_failed
            else:
                if cmd_words[2+offset] == "string":
                    # Remove the BEGIN and END headers
                    reply = al_auth.get_public_key_chars(reply)
    else:
        ret_val = process_status.ERR_command_struct
        reply = ""
...
# ------------------------------------------
# Get permissions for the current node or a member policy
# Command: get permissions
# or - get permissions for a public key:
# Command: get member permissions where [attribute_name] = [attribute value]
# Command: get member permissions where public_key = value
# Command: get member permissions where name = value
# ------------------------------------------
def get_member_permissions(status, io_buff_in, cmd_words, trace):
    ...
    if words_count == (3 + offset):
        signatory_name = al_auth.get_signatory_name()
        if signatory_name:
            title = f"Permissions for Current Node with Signatory Permissions ({signatory_name})"
            ret_val, public_key = al_auth.get_signatory_public_key(status)
        else:
            title = "Permissions for Current Node"
            ret_val, public_key = get_key_by_cmd_line(status, ["get", "public", "key"], offset)

        if public_key:
            key_list = [public_key]

    elif words_count == (7 + offset) and cmd_words[3 + offset] == "where" and cmd_words[5 + offset] == "=":
        attr_name = params.get_value_if_available(cmd_words[4 + offset])
        attr_val = params.get_value_if_available(cmd_words[6 + offset])

        cmd = ["blockchain", "get", "member", "where", attr_name, "=", attr_val, "bring", "[member][public_key]", "separator", "=", " "]
        ret_val, members = blockchain_get(status, cmd, "", True)
        if not ret_val:
            if not members or not len(members):
                ret_val = process_status.Missing_public_key
            else:
                key_list = members.split()
                if attr_name == "public_key":
                    title = "Permissions for Member by public key:"
                else:
                    title = "Permissions for Member with attribute: %s = %s" % (attr_name, attr_val)
    ...
    if not ret_val:
        if not public_key:
            ...
            for public_key in key_list:
                if public_key.startswith("-----BEGIN"):
                    public_key_str = al_auth.get_public_key_chars(public_key)
                else:
                    public_key_str = public_key
                ret_val, permissions_list = permissions.permissions_by_public_key(status, public_key_str)
                if not ret_val:
                    attr_names = ["Name", "ID", "Databases", "Tables", "Enable", "Disable"]
                    reply = utils_print.print_dict_as_table("permissions", permissions_list, None, attr_names, True, "   ", title + "\r\n" + public_key)

    return [ret_val, reply, "table"]
# =======================================================================================================================
# Get one or more the permissions policies
# Examples:
# get permissions
# get permissions where [attr name] = [attr value]
# =======================================================================================================================
def get_permissions(status, io_buff_in, cmd_words, trace):

    reply = ""
    offset = get_command_offset(cmd_words)
    cmd_array = ["blockchain", "get", "permissions"]
    if len (cmd_words) >  (offset + 2):
        cmd_array += cmd_words[2:]      # add the where condition
    ret_val, permissions_list = blockchain_get(status, cmd_array, "", True)
    if not ret_val and isinstance(permissions_list, list) and len(permissions_list):
        attr_names = ["Name", "ID", "Databases", "Tables", "Enable", "Disable"]
        reply = utils_print.print_dict_as_table("permissions", permissions_list, None, attr_names, True, "   ", None)
    return [ret_val, reply, "table"]
...
# ------------------------------------------
def get_authentication(status, io_buff_in, cmd_words, trace):

    if len(cmd_words) == 2:
        ret_val = process_status.SUCCESS
        reply = al_auth.get_authentication()
    else:
        ret_val = process_status.ERR_command_struct
        reply = ""
...
# Returns on/off to determine if active
# ------------------------------------------
def get_encryption(status, io_buff_in, cmd_words, trace):

    reply = al_auth.get_encryption()
    return [process_status.SUCCESS, reply]
# ------------------------------------------
# get compression
# Returns on/off to determine if active
...
def get_directory_size(status, io_buff_in, cmd_words, trace):
    ...
    return [ret_val, reply]
# ----------------------------------------------------------
# Return the list of users listed on the node using the command:
# id add user ...
# ----------------------------------------------------------
def get_users(status, io_buff_in, cmd_words, trace):

    ret_val, reply = al_auth.get_users_list(status)
    return [ret_val, reply]

# ------------------------------------------------
# TSD sync status of the particular node:
# Info of the tsd tables or on a particular tsd table,
# The sync process supports the HA processes.
# Example: get tsd sync status
# Example: get tsd sync status where table = tsd_128
# The info is based on messages between the peers supporting the cluster
# ------------------------------------------------
def get_ha_node_stat(status, io_buff_in, cmd_words, trace):

    ret_val = process_status.SUCCESS
    if not alconsumer.is_consumer_running():
        ...
        if not ret_val:
            reply = metadata.show_tsd_status(tsd_table)
...
def get_version(status, io_buff_in, cmd_words, trace):
    ...
    global code_version
    return [process_status.SUCCESS, code_version]

# ------------------------------------------------
# Get the software version
# ------------------------------------------------
def get_license(status, io_buff_in, cmd_words, trace):
    global activation_license_
    global expiration_
    global license_msg_
    global license_type_
    ...
    msg = license_msg_
    if not activation_license_:
        msg += "\r\nStatus: Activation license is missing"
    else:
        if license_type_:
            msg += f"\r\nLicense Type: {license_type_}"
        if licensed_company_:
            msg += f"\r\nLicensed Company: {licensed_company_}"
        if expiration_:
            msg += f"\r\nExpiration Date: {expiration_}"
    return [process_status.SUCCESS, msg]
# ------------------------------------------------
...
# set consumer mode = suspend / active
# --------------------------------------------------------------
def set_consumer_mode(status, io_buff_in, cmd_words, trace):

    mode = cmd_words[4]
    if cmd_words[3] == '=' and (mode == "suspend" or mode == "active"):
        alconsumer.set_mode(mode)
        ret_val = process_status.SUCCESS
    else:
        ret_val = process_status.ERR_command_struct
    return ret_val
...
def set_query_pool(status, io_buff_in, cmd_words, trace):
    ...
    return ret_val
# --------------------------------------------------------------
# Provide a password to protect sensitive information (like private keys and users passwords) that are kept on the node.
# Example: set local password = abc
# --------------------------------------------------------------
def set_local_password(status, io_buff_in, cmd_words, trace):

    if cmd_words[3] == '=':
        password = cmd_words[4]
        ret_val = al_auth.set_aut_passwrd(status, password)
    else:
        ret_val = process_status.ERR_command_struct

    return ret_val
# --------------------------------------------------------------
# Provide a private key and a password to sign default messages and policies
# Replace the private key and password assigned to the node.
# Example: set signatory where key = !private_key and password = 78426a# and name = root
# --------------------------------------------------------------
def set_signatory(status, io_buff_in, cmd_words, trace):
    ...
    if cmd_words[2] != "where":
        ...
        #                               Must     Add      Is
        #                               exists   Counter  Unique
        keywords = {"key":          ("str", True, False, True),
                    "password":     ("str", True, False, True),
                    "name":         ("str", True, False, True),     # A name returned in the command "get signatory"
                    }
        ...
        private_key = interpreter.get_one_value(conditions, "key")
        password = interpreter.get_one_value(conditions, "password")
        name = interpreter.get_one_value(conditions, "name")
        if name.lower() == "node":
            status.add_error("Wrog signatory name")     # Can be any name other than "Node"
            return process_status.ERR_command_struct

        ret_val = al_auth.set_signatory(private_key, password, name)


    return ret_val

# --------------------------------------------------------------
#  'usage': "set private password = [password] [in file]",
#  'example': "set private password = private_password",
#   set private password = demo1 in file
# --------------------------------------------------------------
def set_private_key_password(status, io_buff_in, cmd_words, trace):
    ...
    if cmd_words[3] != '=':
        ...
        if words_count == 5:
            in_file = False
        elif words_count == 7 and utils_data.test_words(cmd_words, 5, ["in", "file"]):
            in_file = True
        else:
            ret_val = process_status.ERR_command_struct

        if not ret_val:
            ret_val = al_auth.set_prk_passwrd(status, password, in_file)
    return ret_val
# --------------------------------------------------------------
# Set the user / node authentication
# 3 options:
# set authentication on/off
# set node authentication on/off
# set node authentication on/off
# --------------------------------------------------------------
def set_authentication(status, io_buff_in, cmd_words, trace):

    ret_val = process_status.SUCCESS
    words_count = len(cmd_words)

    if words_count == 3:
        # set authentication for node + user
        if cmd_words[2] == 'on':
            ret_val = al_auth.set_user_authentication(status, True)  # May fail to load library
            if not ret_val:
                al_auth.set_node_authentication(True)
        elif cmd_words[2] == 'off':
            al_auth.set_node_authentication(False)
            al_auth.set_user_authentication(status, False)
        else:
            ret_val = process_status.ERR_command_struct
...
# --------------------------------------------------------------
# Enable / disable encryption of messages
# set encryption on off
# --------------------------------------------------------------
def set_encryption(status, io_buff_in, cmd_words, trace):
    ...
    if not ret_val:
        al_auth.set_encryption(encryption)
...
_reset_methods = {
        ...
        "operator stat": {'command': aloperator.reset_stat,
                 'key_only': True,
                 'help': {
                     'usage': "reset operator stat",
                     'example': "reset operator stat",
                     'text': "reset the operator statistics and timer.",
                     'keywords' : ["profiling"],
                    }
                 },
        ...
        "signatory": {'command': al_auth.reset_signatory,
                        'key_only': True,
                        'help': {
                           'usage': "reset signatory",
                           'example': "reset signatory",
                           'text': "Revert the signatory to be the loccal node (using the node's private key).",
                           'link' : 'blob/master/authentication.md#revert-to-the-node-as-the-signatory',
...
_set_methods = {
        ...
        'data distribution': {
            'command': alpublisher._distribute_data,
            'words_min' : 7,
            'help': {
                'usage': 'set data distribution where dbms = [dbms_name] and table = [table_name] and dest = [ip:port]',
                'example': 'set data distribution where dbms = lsl_demo and table = ping_sensor and dest = 10.12.32.148:2048\n'
                ...
                'text': 'Distribute data by a publisher to the specified operators.\n'
                        'If "remove" is set to "true", the existing distribution for the table is removed.',
                'link' : 'blob/master/anylog%20commands.md#set-data-distribution',
                'keywords' : ["streaming", "data", "configuration", "background processes"],
            },
            'trace': 0,
        },
        ...
        "license": {'command': set_license,
               'words_min': 3,
               'help': {
                   'usage': "set license where activation_key =[license activation key]",
                   'example': "set locense where activation_key = 12343445679",
                   'text': "Provide the license key.",
      ...
      "debug": {'command': set_debug_options,
        ...
        "local password": {'command': set_local_password,
                        'words_count' : 5,
                        'help': {
                           'usage': "set local password = [password]",
                           'example': "set local password = my_password",
                           'text': "Provide a password to protect sensitive information that is kept on the node (like private keys and users passwords).",
                           'link': 'blob/master/authentication.md#the-local-password',
                            'keywords' : ["secure network"],
                            }
                       },
        "signatory": {'command': set_signatory,
                       'words_count': 14,
                       'help': {
                           'usage': "set signatory where key = [private key] and password = [password] and name = [signatory name]",
                           'example': "set signatory where key = !private_key and password = 78426a# and name = root",
                           'text': "Set the default signatory according to the info provided.",
                           'link': "blob/master/authentication.md#assign-a-signatory",
                           'keywords' : ["secure network"],
                            }
                       },

        "private password": {'command': set_private_key_password,
                       'words_min': 5,
                       'help': {
                           'usage': "set private password = [password] [in file]",
                           'example': "set private password = private_password",
                           'text': "Store the password protecting the node's private key on the node. The password is protected by the local password.",
                           'link': 'blob/master/authentication.md#the-private-password',
                           'keywords' : ["secure network"],
                            }
                       },
        "authentication": {'command': set_authentication,
                       'help': {
                           'usage': "set authentication [on/off]",
                           'example': "set authentication on",
                           'text': "Enables \ disables user authentication and node authentication.",
                           'link': 'blob/master/authentication.md#the-private-password',
                           'keywords' : ["secure network"],
                            }
                       },
        "user authentication": {'command': set_authentication,
                       'help': {
                           'usage': "set user authentication [on/off]",
                           'example': "set user authentication on",
                           'text': "Enable\disables user authentication.",
                           'link': 'blob/master/authentication.md#users-authentication',
                           'keywords' : ["secure network"],
                           }
                       },
        "node authentication": {'command': set_authentication,
                       'help': {
                           'usage': "set node authentication [on/off]",
                           'example': "set node authentication on",
                           'text': "Enable\disable node authentication.",
                           'link': 'blob/master/authentication.md#node-authentication',
                           'keywords' : ["secure network"],
                            }
                       },
        "encryption": {'command': set_encryption,
                        'words_count' : 3,
                        'help': {
                           'usage': "set encryption [on/off]",
                           'example': "set encryption on",
                           'text': "Enable\disable encryption over TCP.",
                           'keywords' : ["secure network"],
                            }
                        },
        ...
        "buffer threshold": {'command': streaming_data.set_thresholds,
                            'help': {
                                ...
                                'keywords' : ["data", "configuration", "streaming"],
                                }
                            },
    ...
    "consumer mode": {'command': set_consumer_mode,
                    'words_count' : 5,
                    'help': {
                         'usage': "set consumer mode = [status]",
                         'example': "set consumer mode = active\n"
                                    "set consume mode = suspend",
                         'text': "Resume / Suspend the process of cluster synchronization with peer operators.",
                        'keywords' : ["high availability"],
                        }
                    },
...
_test_methods = {
        ...
        'case': {
            ...
            'words_min': 6,
            'help': {
                'usage': 'test case where source = [file path and name] and inform = [output destination]',
                'example': 'test case where source = !test_dir/main_query.out and inform = stdout',
                'text': 'Read the command and params from the source file and execute the command. Compare test output with the source file and provide the comparison results to the declared output.',
                'link': 'blob/master/test%20suites.md#the-test-case-command',
                'keywords' : ["test suite"],
            },
            'trace': 0,
        },
        'suite': {
            ...
            'words_min': 6,
            'help': {
                'usage': 'test suite where source = [path to files and file type] and inform = [output destination] and subdir = [true/false]',
                'example': 'test suite where source = !test_dir/*.out and subdir = true',
                'text': 'Execute the "test case" command on each file which is in the source path and satisfies the file type.',
                'link': 'blob/master/test%20suites.md#the-test-suite-command',
                'keywords' : ["test suite"],
            },
            'trace': 0,
        },
...
_get_methods = {
        ...
        "publisher distribution": {'command': alpublisher.get_distribution,
                      'words_count': 3,
                      'help': {
                          'usage': "get publisher distribution",
                          'example': "get publisher distribution",
                          'text': "Get the destination of data distributed from this node",
                          'link' : 'blob/master/anylog%20commands.md#set-data-distribution',
                          'keywords' : ["streaming", "data", "profiling"],
                        }
                      },
    ...
    "os process": {'command': utils_monitor.get_os_process,
        ...
        "node id": {'command': get_node_id,
                'key_only' : True,
                'help': {
                     'usage': "get node id",
                     'example': "get node id",
                     'text': "Get the public key of the node.",
                     'keywords' : ["node info", "secure network"],
                    }
                 },

        "hardware id": {'command': get_hardware_id,
             'help': {
                 'key_only' : True,
                 'usage': "get hardware id",
                 'example': "get hardware id",
                 'text': "Get the ID of the motherboard.",
                 'keywords' : ["node info"],
                }
             },
        ...
        "public": {'command': get_key,
                'words_min' : 3,
                'help': {
                    'usage': "get public [key/string] where keys_file = [file path and name]",
                    'example': "get public key\n"
                               "get public string where keys_file = !pem_dir/server-acme-inc-public-key",
                    'text': "Get the public key of the node, or get the public key from a PEM file.",
                    'link' : "blob/master/authentication.md#retrieving-the-keys",
                    'keywords' : ["secure network"],
                    }
                },
        "private": {'command': get_key,
                'words_min' : 3,
                'help': {
                    'usage': "get private [key/string] where keys_file = [file path and name]",
                    'example': "get private key\n"
                               "get private key where keys_file = roy",
                    'text': "Get the private key of the node, or get the private key from a PEM file.",
                    'link' : "blob/master/authentication.md#retrieving-the-keys",
                    'keywords' : ["secure network"],
                    }
               },
        "member permissions": {'command': get_member_permissions,
                'help': {
                    'usage': "get member permissions\n"
                             "get member permissions where [attribute name] = [attribute value]",
                    'example': "get member permissions\n"
                               "get member permissions where public_key = !public_key\n"
                               "get member permissions where name = value",
                    'text': "Get permissions of the current node or permissions assigned to a member policy in the network identified by the attribute name value pair.",
                    'link' : "blob/master/authentication.md#permission-group",
                    'keywords' : ["secure network"],
                    }
                },
        ...
        "authentication": {'command': get_authentication,
            'words_count' : 2,
            'help': {
                 'usage': "get authentication",
                 'example': "get authentication",
                 'text': "Get authentication ON/OFF status.",
                 'link': 'blob/master/authentication.md#users-authentication',
                 'keywords' : ["secure network"],
                }
            },

        "encryption": {'command': get_encryption,
            'words_count' : 2,
            'help': {
                'usage': "get encryption",
                'example': "get encryption",
                'text': "Get encryption ON/OFF status.",
                'keywords' : ["secure network"],
                }
            },
    ...
    "databases": {'command': get_databases,
        ...
        "operator": {'command': get_operator,
                     'words_min': 2,
                     'help': {
                         'usage': "get operator [options]",
        ...
        "data nodes": {'command': get_data_nodes,
                    'help': {
                        'usage': "get data nodes where company = [company name] and dbms = [dbms name] and table = [table name]",
        ...
        "local ingestion": {'command': aloperator.get_local_ingestion,
                       'key_only': True,
                       'help': {
                           'usage': "get local ingestion",
                           'example': "get local ingestion",
                           'text': "List tables updated on this node which are not replication of data.",
                           'keywords': ["high availability"],
                       }
                       },

        "publisher": {'command': get_publisher,
                 'key_only': True,
                 'help': {
                     'usage': "get publisher",
                     'example': "get publisher",
                     'text': "Information on the Publisher processes.",
                     'keywords' : ["node info", "configuration", "background processes"],
                    }
                 },
        "distributor": {'command': get_distributor,
                  'key_only': True,
                  'help': {
                      'usage': "get distributor",
                      'example': "get distributor",
                      'text': "Information on the Distributor processes.",
                      'keywords' : ["node info", "configuration", "background processes", "high availability"],
                  }
                  },
        "consumer": {'command': get_consumer,
                  'key_only': True,
                  'help': {
                      'usage': "get consumer",
                      'example': "get consumer",
                      'text': "Information on the Consumer processes.",
                      'keywords' : ["node info", "configuration", "background processes", "high availability"],
                  }
                  },
    ...
    "streaming": {'command': get_streaming_info,
        ...
        "signatory": {'command': al_auth.get_signatory,
                   'key_only': True,
                   'help': {
                       'usage': "get signatory",
                       'example': "get signatory",
                       'text': "Return the name of the default signatory.",
                       'link': "blob/master/authentication.md#get-the-signatory-name",
                       'keywords' : ["secure network"],
                   }
                   },
    ...
    "msg": {'command': get_msg_client_broker,
        ...
        "inserts": {'command': get_insert_info,
                     'key_only': True,
                     'help': {
                         'usage': "get inserts",
                         'example': "get inserts",
                         'text': "Statistics on SQL Inserts of data to the local databases.",
                         'keywords' : ["dbms", "node info"],
                        }
                     },
        ...
        "license": {'command': get_license,
                'key_only': True,
                'help': {
                    'usage': "get license",
                    'example': "get license",
                    'text': "Return AnyLog license info.",
                    'keywords': ["node info", "configuration"],
                }
                },
        ...
        "ha node status": {'command': get_ha_node_stat,
                     'help': {
                         'usage': "get ha node status where table = [TSD table name]",
                         'example': "get ha node status\n"
                                    "get ha node status where table = tsd_128",
                         'text': "Provide the HA synchronization status on this node.",
    ...
    "files": {'command': get_files_list,
        ...
        "users": {'command': get_users,
                    ...
                    'help': {
                        'usage': "get users",
                        'example': "get users",
                        'text': "List the users added by the command: 'id add user ...'.",
                        'keywords' : ["secure network"],
                    }
                    },
...
_id_methods = {
        "create keys for node": {'command': create_node_keys,
                    'words_count' : 9,
                    'help': {
                        'usage': "id create keys for node where password = [password]",
                        'example': "id create keys for node where password = my_password",
                        'text': "Create a public and a private key for the node.",
                        'link' : "blob/master/authentication.md#creating-keys-for-a-node-in-the-network",
                        'keywords' : ["secure network"],
                        }
                    },
        "create keys": {'command': create_user_keys,
                    'words_min' : 7,
                    'help': {
                         'usage': "id create keys where password = [password] and keys_file = [file path and name]",
                         'example': "id create keys for node where password = my_password\n"
                                    "id create keys for node where password = my_password and keys_file = !usb_path/my_keys",
                         'text': "Create a public and a private key for the user.",
                         'link': "blob/master/authentication.md#creating-a-private-and-public-key-for-a-user",
                         'keywords' : ["secure network"],
                         }
                    },

        "sign": {'command': sign_json_policy,
                    'words_min': 7,
                    'help': {
                        'usage': "id sign [JSON Policy] where key = [private key] and password = [password]\n"
                                 "id sign [JSON Policy] where password = [password]",
                        'example': "id sign !json_script where password = !my_password\n"
                                   "id sign !json_script where key = !my_key and password = !my_password",
                        'text': "Sign a policy and add add the public key and the signature to the policy.",
                        'link': "blob/master/authentication.md#signing-a-policy",
                        'keywords' : ["secure network"],
                        }
                    },

        "authenticate": {'command': verify_signature,
                    'words_count': 3,
                    'help': {
                        'usage': "id authenticate [JSON Policy]",
                        'example': "id authenticate !json_script",
                        'text': "Authenticate the policy by validating that the policy was signed using the private key associated with the public key on the policy.",
                        'link': "blob/master/authentication.md#authenticate-signature",
                        'keywords' : ["secure network"],
                        }
                    },
        "encrypt": {'command': encrypt_message,
                     'words_count': 4,
                     'help': {
                         'usage': "id encrypt [message text] [public key]",
                         'example': "id encrypt !message !public_key",
                         'text': "Encrypt a message using the public key of the receiver",
                         'link': "blob/master/authentication.md#encrypt-and-decrypt-messages",
                         'keywords' : ["secure network"],
                        }
                     },
        "decrypt": {'command': decrypy_message,
                'words_min': 7,
                'help': {
                    'usage': "id decrypt [message text] where key = [private key] and password = [password]",
                    'example': "id decript !message where key = !private_key and password = !my_password\n"
                               "id decript !message where password = !my_password",
                    'text': "Decrypt a message using the private key. If the private key is not provided, the private key of the receiving node is used.",
                    'link': "blob/master/authentication.md#encrypt-and-decrypt-messages",
                    'keywords' : ["secure network"],
                    }
                },

        "validate": {'command': is_authorized,
                     'words_min': 10,
                     'help': {
                         'usage': "id validate where key = [public_key] and command = [command text] and table = [table name] and dbms = [dbms name]",
                         'example': "id validate where key = !public_key and command = copy\n"
                                    "id validate where key = !public_key and command = sql and dbms = lsl_demo and table = ping_sensor",
                         'text': "Validate that the public key provided is associated with a policy that authorize the specified command",
                         'link': "blob/master/authentication.md#validate-permitted-command",
                         'keywords' : ["secure network"],
                        }
                     },

        "add user": {'command': add_user,
                'words_min': 11,
                'help': {
                    'usage': "id add user where name = [user name] and password = [password] and expiration = [expiration time]",
                    'example': "id add user where name = ori and password = 123 and expiration = 2 minutes",
                    'text': "Add a user to the a local list of users that can connect to this node",
                    'link': "blob/master/authentication.md#add-users",
                    'keywords' : ["secure network"],
                    }
                },

        "remove user": {'command': remove_user,
                 'words_count': 7,
                 'help': {
                     'usage': "id remove user where name = [user name]",
                     'example': "id remove user where name = john",
                     'text': "Remove a user from the a local list of users that can connect to this node",
                     'link': "blob/master/authentication.md#remove-users",
                     'keywords' : ["secure network"],
                    }
                 },

        "update user password": {'command': update_password,
                    'words_count':16,
                    'help': {
                        'usage': "id update user password where name = [user name] and old = [old password] and new = [new password]",
                        'example': "id update user password where name = ori and old = 123456 and new = iugsek88ekA",
                        'text': "Update a passord for an existing user",
                        'link': "blob/master/authentication.md#update-password",
                        'keywords' : ["secure network"],
                    }
                    },
        "generate certificate authority": {'command': generate_certificate_auth,
                             'words_min': 3,
                             'help': {
                                 'usage': "id generate certificate authority\n"
                                          "id generate certificate authority where password = [password]",
                                 'example': "id generate certificate authority",
                                 'text': "Generate a private key and a public key for the certificate authority.\n"
                                         "The keys are written to the PEM directory and the private key is optionally encrypted.",
                                 'link': "blob/master/authentication.md#setup-the-ca",
                                 'keywords' : ["secure network"],
                                }
                             },
        "generate certificate request": {'command': generate_certificate_request,
                'words_min': 3,
                'help': {
                    'usage': "id generate certificate request\n"
                             "id generate certificate request where password = [password]",
                    'example': "id generate certificate request",
                    'text': "Generate a certificate request for a node in the network",
                    'link': "blob/master/authentication.md#generating-a-certificate-request",
                    'keywords' : ["secure network"],
                    }
                },
        "sign certificate request": {'command': sign_certificate_request,
                'words_count': 12,
                'help': {
                    'usage': "id sign certificate request",
                    'example': "id sign certificate request",
                    'text': "Sign a certificate request for a node in the network",
                    'link': "blob/master/authentication.md#signing-a-certificate-request",
                    'keywords' : ["secure network"],
                    }
                },
...
commands = {
    ...
    'analyze output': {
        'command': analyze_output,
        'words_min' : 10,
        'help': {'usage': 'analyze output where file = [file path and name] and source = [file path and name] and option = time',
                 'example': 'analyze output where file = !test_dir/main_query.out and source = !test_dir/tested_query.out and option = time',
                 'text': 'Compare the query output to the output in the source file',
                 'link': 'blob/master/test%20suites.md#the-analyze-output-command',
                 'keywords' : ["test suite"],
                 },
        'trace': 0,
    },
    ...
    'rest': {
        ...
        'help': {'usage': 'rest [operation] where url=[url] and [option] = [value] and [option] = [value] ...',
                 'example': 'rest get where url = http://10.0.0.159:2049 and type = info and details = "get status"\n'
                            'rest get where url = https://ipinfo.io/json\n'
    ...
    'run mqtt client': {
        'command': _run_mqtt_client,
        'help': {'usage': 'run mqtt client where broker = [url] and dbms = [dbms name] and topic = [topic]',
                 'example': 'run mqtt client where broker = "driver.cloudmqtt.com" and port = 18975 and user = mqwdtklv and password = uRimssLO4dIo and topic = (name = test and dbms = "bring [metadata][company]" and table = "bring [metadata][machine_name] _ [metadata][serial_number]" and column.timestamp.timestamp = "bring [ts]" and column.value.int = "bring [value]")',
    ...
    'run client': {
        ...
        'help': {'usage': 'run client (IPs and Ports) [AnyLog command]',
                 ...
                 'link' : 'blob/master/getting%20started.md#sending-messages-to-peers-in-the-network',
                 'keywords' : ["configuration", "cli"],
                 },
        'trace': 0,
    },
    ...
    'sleep': {
        'command': _sleep,
        'help': {'usage': 'sleep [sleep time in seconds] [or file update] [file name]',
                 'example': 'sleep 30\n'
                 'sleep 10 or file update !blockchain_file',
                 'text': 'Pause processing of the thread for the specified duration (value in seconds), optionaly wait for a file update (if earlier than the sleep time).',
                 'keywords' : ["cli", "script"],
                 },
        'trace': 0,
    },
    ...
    'run operator': {
        ...
        'help': {'usage': 'run operator where [option] = [value] and [option] = [value] ...',
                 ...
                 'text': 'Monitors new data added to the watch directory and load the new data to a local database\n'
                         ...
                         'watch_dir - The directory monitored by the Operator. Files placed on the Watch directory are processed by the Operator.\n'
                         'bkup_dir - The directory location to store JSON and SQL files that were processed successfully.\n'
                         'error_dir - The directory location to store files containing data that failed processing.\n'
                         'archive - True/False to move JSON files to archive.\n'
                         ...
                         'move_json - True moves the JSON file to the \'bkup\' dir if processing is successful.\n'
                         'move_sql -  True moves the SQL file to the \'bkup\' dir if processing is successful. The file deleted if move_sql is false.\n'
    ...
    'run publisher': {
        'command': _run_publisher,
        'help': {'usage': 'run publisher [options]',
                 'example': 'run publisher where dbms_name = file_name[0] and table_name = file_name[3] and delete_json = true and delete_sql = true',
                 'text': 'Monitors new data added to the watch directory and distributes the new data to an Operator.\n'
                         'Options:\n'
                         'watch_dir - The directory monitored by the Publisher. Files placed on the Watch directory are processed by the Publisher.\n'
                         'bkup_dir - The directory location to store JSON and SQL files that were processed successfully.\n'
                         'error_dir - The directory location to store files containing data that failed processing.\n'
                         'delete_json - True/False for deletion of the JSON file if processing is successful.\n'
                         ...
                         'master_node - The IP and Port of a Master Node (if a master node is used).',
                 'link': 'blob/master/background%20processes.md#publisher-process',
                 'keywords' : ["configuration", "background processes"],
                 },
        'trace': 0,
    },
    ...
    'run data distributor': {
        'command': _run_distributor,
        'help': {'usage': 'run data distributor where distr_dir = [data directory location] and archive_dir = [archive directory location]',
                 'example': 'run data distributor',
                 'text': 'Transfer data files to members of the cluster.',
                 'link': 'blob/master/background%20processes.md#invoking-the-data-distributor-process',
                 'keywords' : ["configuration", "background processes"],
                },
        'trace': 0,
    },
    ...
    'run data consumer': {
        'command': _run_consumer,
        'help': {'usage': 'run data consumer where start_date = [date] and end_date = [date] and mode = [mode of operation]',
                 'example': 'run data consumer where start_date = -30d',
                 'text': 'Pull the source file from the Operators of the cluster such that the cluster data set on this node is complete.',
                 'link': 'blob/master/background%20processes.md#invoking-the-data-consumer-process',
                 'keywords' : ["configuration", "background processes"],
                 },
        'trace': 0,
    },
