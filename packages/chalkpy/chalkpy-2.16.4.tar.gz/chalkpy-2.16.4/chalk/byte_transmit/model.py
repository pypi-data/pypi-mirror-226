...
class ByteBaseModel(ByteSerializable):
    ...
    def produce_byte_items_map(
        cls, start_idx: int, buffer: BytesIO, byte_serializable_items_map: Dict[str, Union[bytes, ByteSerializable]]
    ):
        curr_idx = start_idx
        byte_items_map = {
...
class ByteDict(ByteBaseModel):
    def __init__(self, *args, **kwargs):
        for arg in args:
            if not isinstance(arg, Mapping):
                raise ValueError("Need all args (non-kwargs) in constructor to be dict-like objects")
            for k in arg:
                self[k] = arg[k]
        for k, v in kwargs.items():
            self[k] = v
    ...
    def __setitem__(self, key: str, value: Union[bytes, ByteSerializable]):
        if not isinstance(key, str):
            raise ValueError("Need for keys to be strings!")
        if not isinstance(value, bytes):
            raise ValueError("Need for values to all be bytes or byte-serializable!")
