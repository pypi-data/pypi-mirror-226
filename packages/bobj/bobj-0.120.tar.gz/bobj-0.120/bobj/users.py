...
class UserManagement:
    ...
    def get_user_id(self, user_name): #3
       ...
       if user_id is None:
           print (f"No such user {user_name} found in repositery")
       return user_id
...
class Audit:
    ...
    def inactive_users(self, days): #Not Working
         from datetime import datetime, timedelta
         
         days_ago = datetime.now() - timedelta(days=days)
         days_ago = days_ago.isoformat() + ".000Z"  # Add fractional seconds if needed
         url = UserManagement.base_url + LIST_USERS_ENDPOINT
         params = {'updated': f'{days_ago}'}  # Removed trailing comma
         response = requests.get(url, headers=UserManagement.headers, params=params)
