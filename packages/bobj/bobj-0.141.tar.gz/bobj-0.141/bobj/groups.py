...
class GroupManagement:
    ...
    def add_users_to_group(self, group_name, user_name): #8
    ...
    def remove_users_from_group(self, group_name, user_name): # 9 
        user_id = self.users.get_user_id(user_name)
        url = self.base_url + sup.USERS_IN_GROUP.format(group_id=self.get_group_id(group_name))
        user_data = GroupSupport.create_xml_body("string", str(user_id))
    ...
    def modify_user_group_details(self, *, group_name, new_name=None, description=None):
        group_id = self.get_group_id(group_name)
        url = self.base_url + sup.GROUP_DETAILS_ENDPOINT.format(group_id=group_id)

        attributes = []
        if new_name:
            attributes.append(('name', 'string', new_name))
        if description:
            attributes.append(('description', 'string', description))

        body = self.create_xml_body(attributes)
        self.headers['Content-Type'] = 'application/xml'  # Ensure the correct Content-Type
...
class GroupSupport:
    ...
    @staticmethod
    def create_xml_body(attributes=None, element_type=None, element_value=None):
        attr_elements = ''
        
        if attributes:
            for attr_name, attr_type, attr_value in attributes:
                attr_elements += f'<attr name="{attr_name}" type="{attr_type}">{attr_value}</attr>'
        elif element_type and element_value:
            attr_elements = f'<attr name="id" type="{element_type}">{element_value}</attr>'
        else:
            raise ValueError("Either attributes or element_type and element_value must be provided")
        
        attrs_element = f'<attrs>{attr_elements}</attrs>'
