...
class Audit:
    ...
    def get_inactive_users(self, days):
        query = f"SELECT top 50000 {sup.id}, {sup.last_login}, {sup.created_at}, {sup.name}, {sup.fullname} FROM {sup.system_db} WHERE ({sup.kind}='User') AND {sup.last_login} < current_date - {days}"
