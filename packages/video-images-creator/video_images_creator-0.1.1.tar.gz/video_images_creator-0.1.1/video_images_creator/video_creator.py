...
def create_left_to_right_movement(left_image_file_path, right_image_file_path, current_index_, left_screen_name, right_screen_name):
    ...
    bg_img = cv2.imread('video_images_creator/background.png')
...
def build_screen(screen_file, combine_file_name, x_coordinate, sceen_name, text_to_be_added):
    ...
    # Load the images
    background = cv2.imread('video_images_creator/background.png')
    overlay = cv2.imread(screen_file)
    mobile = cv2.imread('video_images_creator/features/mobile_2.png') 
    ...
    if text_to_be_added:
        ...
        font_path = 'video_images_creator/features/Rubik-Medium.ttf'
        font_size = 47
...
def run_ffmpeg():
    os.system(f"source ~/.bash_profile && ffmpeg -framerate 60 -i images/frame_%d.jpg -c:v libx264 -crf 18 -pix_fmt yuv420p -r 60 output.mp4")
