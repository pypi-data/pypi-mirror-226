...
def electric_meter_data_from_sml_message(message: bytes) -> ElectricMeterReadDTO:
    """Get ElectricMeterData from SML message.

    Args:
        message: The SML message.

    Returns:
        The ElectricMeterData object.

    Raises:
        ValueError: If the message is valid.
    """
    ...
    try:
        ...
        key = exc.args[0]
        raise ValueError(f"OBIS code {key} not found in message") from exc
