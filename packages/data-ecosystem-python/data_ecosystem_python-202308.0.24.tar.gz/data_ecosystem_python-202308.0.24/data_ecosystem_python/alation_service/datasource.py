...
class DataSource:
    ...
    @staticmethod
    def fetch_datasources(edc_alation_api_token, edc_alation_base_url, alation_datasource_id):
        """ 
        ...
        with tracer.start_as_current_span("fetch_datasources"):
            ...
            try:
                ...
                if collection_set_results is not None:
                    ...
                    collection_set_results_json = collection_set_results.json()
                    schema_custom_fields = collection_set_results_json[0].get(
                        "custom_fields")
                ...
                try:
                    ...
                    for offset in range(0, total_records, limit):
                        ...
                        # Create the API URL
                        api_url = f"{edc_alation_base_url}/integration/v2/datasource/"
                ...
                expanded_json = []
                for existing_datasource_item in merged_data_json:
                    new_item = existing_datasource_item.copy()  # start with existing fields

                    datasource_steward_list = None  # Initialize the variable here
                    for custom_field in new_item['custom_fields']:
                        # handle stewards
                        if custom_field['field_name'] == "Steward":
                            datasource_steward_list = custom_field['value']

                            # Create a list of unique otype and oid combinations from datasource_steward_list
                            combinations = {(item['otype'], item['oid'])
                                            for item in schema_steward_list + datasource_steward_list}

                            # Add items from schema_steward_list to datasource_steward_list if not present
                            for combined_item in combinations:
                                search_otype = combined_item[0]
                                search_oid = combined_item[1]
                                found_datasource_items = [datasource_item for datasource_item in datasource_steward_list if datasource_item.get(
                                    'otype') == search_otype and datasource_item.get('oid') == search_oid]

                                if not found_datasource_items:
                                    for schema_item in schema_steward_list:  # corrected to schema_steward_list
                                        ...
                                        datasource_steward_list.append(
                                            item_copy)
                                else:
                                    # The item exists in datasource_steward_list, set is_inherited to empty string
                                    found_datasource_items[0]["is_inherited"] = ""

                            # Modify 'Steward' field_name to 'Steward_Initial'
                            custom_field['field_name'] = 'Steward_Initial'

                    # Add new 'Steward' attribute with value "merged" if datasource_steward_list is not None
                    if datasource_steward_list is not None:
                        new_item['custom_fields'].append(
                            {'field_name': 'Steward', 'value': datasource_steward_list})

                    # Promote custom fields to the datasource level
                    for field in existing_datasource_item['custom_fields']:
                        # add custom fields
                ...
                df_datasources = json_normalize(expanded_json)
    ...
    @staticmethod
    def fetch_datasource_schemas(alation_headers, edc_alation_base_url, alation_datasource_id):
        """
