...
try:
    from data_ecosystem_python.app_startup import create_api, create_app
...
from data_ecosystem_python.alation_service import (
...
from data_ecosystem_python.az_key_vault_service import (
...
from data_ecosystem_python.cdc_security_service import (
    security_core as pade_security_core,
)
from data_ecosystem_python.posit_service import (
...
auth_parser.add_argument(
    'code', type=str, help='Code parameter', location='args')
auth_parser.add_argument(
    'state', type=str, help='State parameter', location='args')

auth_form_parser = reqparse.RequestParser()
auth_form_parser.add_argument(
    'code', type=str, help='Code parameter', location='form')
auth_form_parser.add_argument(
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if id_token is None:
                ...
                flask_response.set_cookie(
                    'id_token', id_token, path='/', secure=secure, httponly=False, samesite='Lax')
                flask_response.set_cookie("redirect_attempted", '',
                                          expires=0, secure=secure, samesite='Strict')
...
def get_login_redirect_response():
    ...
    try:
        ...
        response.set_cookie("redirect_attempted", '',
                            expires=0, secure=secure, samesite='Strict')
        return response
...
class AuthCallback(Resource):
    ...
    def post(self):
        ...
        redirect_url = next(
            (url for url in urls if url and 'cdc_security/callback' not in url), None)
    ...
    def get(self, schema_id):
        ...
        with tracer.start_as_current_span("metadata_json_file_download"):
            try:
                ...
                manifest_json_file = schema.download_manifest_json(
                    schema_id, config)
    ...
    def post(self):
        ...
        with tracer.start_as_current_span("metadata_excel_file_upload"):
            ...
            try:
                ...
                content_result, authorized_tables_count, unauthorized_table_count = schema.upload_manifest_excel(
    ...
    def post(self):
        ...
        with tracer.start_as_current_span("metadata_json_file_upload"):
            ...
            try:
                ...
                repository_path = config.get("repository_path")
                environment = config.get("environment")
                json_data_definition_file_path = schema.get_json_data_definition_file_path(
                    repository_path, environment)

                authenticated_user_id = request.cookies.get('user_id')

                content_result, authorized_tables_count, unauthorized_table_count = schema.upload_manifest_json(
