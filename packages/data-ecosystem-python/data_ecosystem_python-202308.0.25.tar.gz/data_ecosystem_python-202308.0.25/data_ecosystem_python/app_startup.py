...
# Importing necessary modules from data_ecosystem_python package
# These modules seem to be related to environment metadata, tracing and logging.
from data_ecosystem_python.cdc_self_service import (
...
def change_to_flask_directory():
    ...
    if base_directory != 'data_ecosystem_python':
        # path to the directory you want to change to
        new_directory = os.path.join(
            current_directory, 'data_ecosystem_python/data_ecosystem_python')

        # change to new directory
        os.chdir(new_directory)
        print(f"Directory changed to {os.getcwd()}")
    else:
        print("Current directory is already 'data_ecosystem_python'")

   # Create the path to the child directory
    child_directory = os.path.join(
        current_directory, 'data_ecosystem_python')
...
def create_app():
    ...
    API_PATH = "/data-ecosystem-services/data_ecosystem_python"
    ...
    try:
        with tracer.start_as_current_span("create_app"):
            ...
            try:
                importlib.import_module("data_ecosystem_python")
                logger.info("data_ecosystem_python is module in pythonpath")
            except ImportError:
                logger.warning(
                    "data_ecosystem_python is not a module in pythonpath")
            ...
            if RUNNING_IN_WSL is True:
                ...
                sys.path.append(os.path.abspath(
                    __file__ + "/../../../data_ecosystem_python/"
                ))
                logger.info(f"RUNNING_IN_WSL: {RUNNING_IN_WSL}")
                logger.info(
                    os.path.abspath(
