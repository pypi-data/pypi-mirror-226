...
import data_ecosystem_python.cdc_self_service.pipeline_metadata as davt_pl_metadata
import data_ecosystem_python.cdc_self_service.dataset_metadata as davt_ds_metadata
import data_ecosystem_python.cdc_self_service.environment_metadata as davt_env_metadata
import data_ecosystem_python.cdc_self_service.logging_metadata as davt_log_metadata
import data_ecosystem_python.cdc_tech_environment_service.job_core as davt_job_core
import data_ecosystem_python.cdc_tech_environment_service.repo_core as davt_repo_core
import data_ecosystem_python.cdc_tech_environment_service.dataset_crud as davt_ds_crud
import data_ecosystem_python.cdc_tech_environment_service.dataset_core as davt_ds_core
import data_ecosystem_python.cdc_tech_environment_service.environment_file as davt_env_file
...
class JobMetaData:
    ...
    def install_cluster_library(config) -> str:
        ...
        data_ecosystem_python_version = config["data_ecosystem_python_version"]
        data_ecosystem_python_version = data_ecosystem_python_version .replace(
            "v", "")
        library_folder = f"{davt_root_project_url}dist/"
        library_file = f"{library_folder}{data_ecosystem_python_version}-py3-none-any.whl"
        obj_repo = davt_repo_core.RepoCore()
        results = obj_repo.install_cluster_library(
    ...
    def get_standard_parameters(environment: str, dbutils, spark,
        ...
        if not running_local:
            try:
                ...
                dbutils.widgets.dropdown("job_name", default_job_name, [
    ...
    def run_publish_config(obj_env_metadata: davt_env_metadata.EnvironmentMetaData, config: dict, spark,
        ...
        config_mount_path = "".join(
            ['/dbfs', ingress_mount.rstrip('/'), '/davt/translationFoodnet.json'])
    ...
    def run_data_processing(obj_env_metadata: davt_env_metadata.EnvironmentMetaData, config, spark, dbutils,
        ...
        # setup database
        davt_database_folder = obj_env_metadata.setup_databricks_configuration(
            config, spark)
            ...
            if execute_flag is None:
                ...
                if execute_flag == 'skip_execute':
                    ...
                    if pipeline_type == "databricks_sql":
                        ...
                        config_pipeline = obj_pipeline.get_execute_pipeline_parameters(
                            config, config_pipeline)
                        ...
                        davt_parquet_folder = config["davt_database_folder"].replace(
                            "/delta", "/" + export_format)
                        davt_parquet_path = davt_parquet_folder.rstrip(
                            "/") + ("/") + query_name + "." + export_format
                        ...
                        df_export.repartition(1).write.format(export_format).mode(
                            "overwrite").save(davt_parquet_path)
