...
class EquipmentShell(Equipments):
    ...
    @staticmethod
    def get_ram_state(command):
        percent_ram_value = randint(1, 100)
        return percent_ram_value < 50

    @staticmethod
    def get_cpu_state(command):
        percent_cpu_value = randint(1, 100)
        return percent_cpu_value < 50
        ...
        try:
            with ftplib.FTP() as connection:
                ...
                content["updated_recently"] = EquipmentShell.was_updated_recently(
                    ...
                    frequency=frequency)
                content["ram_state"] = EquipmentShell.get_ram_state(command="")
                content["cpu_state"] = EquipmentShell.get_cpu_state(command="")
