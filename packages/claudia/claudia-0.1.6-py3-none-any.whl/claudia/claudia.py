...
def launch_command_run_e2e_tests(lib, client_type, network, tag, feature, invalidate_cache):
    try:
        ...
        setup_e2e_env(lib, client_type, network, tag, feature, invalidate_cache)
        ...
        command = get_launch_command_e2e_tests(lib, tag, feature)
        subprocess.call(command, shell=True)
        teardown_e2e_env(lib)
    except Exception as e:
        set_to_project_root_wd()
        raise e
...
def launch_command_e2e_features():
    click.echo(get_launch_command_e2e_features_message())


def get_launch_command_e2e_features_message():
    set_to_project_root_wd()
...
def setup_e2e_env(lib, client_type, network, tag, feature, invalidate_cache):
    if lib == 'py':
...
def get_launch_command_e2e_tests(lib, tag, feature):
    if lib == 'py':
        if feature == "all":
