...
class EntityCache(BaseCache):
    ...
    def get(self, **kwargs):
        ...
        Указываемые поля должны совпадать с полями в searching_key.
        Если требуется выборка по полям не входящим в searching_key, необходимо использовать .filter(...).first().
        """
        id_field_name = self._model._meta.pk.name

        if id_field_name in kwargs and id_field_name not in self._searching_key and PK in self._searching_key:
            kwargs[PK] = kwargs[id_field_name]

            kwargs.pop(id_field_name)
        elif PK in kwargs and PK not in self._searching_key and id_field_name in self._searching_key:
            kwargs[id_field_name] = kwargs[PK]

            kwargs.pop(PK)

        fields_count = len(kwargs)

        if len(self._searching_key) != fields_count:
            raise ValueError(SEARCHING_KEY_SIZE_NOT_EQUAL_DEFAULT_SEARCHING_KEY_SIZE_ERROR)

        if not set(self._searching_key).issubset(kwargs):
            raise ValueError(SEARCHING_KEY_NOT_EQUAL_DEFAULT_SEARCHING_KEY_ERROR)

        key = tuple([kwargs.get(field) for field in self._searching_key])

        entity_item = self.get_by_key(
            key=key,
            strict_mode=True,
        )
    ...
    def values_list(
        ...
        **kwargs,
        ...
        fields_getter = attrgetter(*parameters)
