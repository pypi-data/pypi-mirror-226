...
class HopscotchDict(MutableMapping[Hashable, Any]):
    ...
    def _clear_neighbor(self, lookup_idx: int, nbhd_idx: int) -> None:
        ...
        :param lookup_idx: The index in _nbhds
        :param nbhd_idx: The neighbor in the neighborhood of
                         lookup_idx to set unoccupied
        """
        ...
        self._nbhds[lookup_idx] &= ~(1 << nbhd_idx)
        ...
        while lookup_idx < self._size:
            ...
            if entry_expected_idx != lookup_idx and lookup_idx == target_idx:
                ...
                if expected_nbr is not None:
                    ...
                    self._set_lookup_index_info(expected_nbr, data=data_idx)
                    self._set_lookup_index_info(lookup_idx, data=self.FREE_ENTRY)
                    self._set_neighbor(entry_expected_idx, exp_opn_nbhd_idx)
                    self._clear_neighbor(entry_expected_idx, exp_tgt_nbhd_idx)
                    return
            ...
            for displacement in range(disp_range):
                ...
                if displaceable_nbr is not None:
                    ...
                    self._set_lookup_index_info(open_idx, data=data_idx)
                    self._set_lookup_index_info(displaceable_nbr, data=self.FREE_ENTRY)

                    disp_nbhd_idx = (displaceable_nbr - disp_root) % self._size
                    open_nbhd_idx = (open_idx - disp_root) % self._size
                    self._set_neighbor(disp_root, open_nbhd_idx)
                    self._clear_neighbor(disp_root, disp_nbhd_idx)
    ...
    def _resize(self, new_size: int) -> None:
        ...
        for data_idx, key in enumerate(self._keys):
            expected_lookup_idx = abs(hash(key)) % new_size

            open_nbr_idx = self._get_open_neighbor(expected_lookup_idx)
            if open_nbr_idx is None:
                ...
                open_nbr_idx = cast(int, self._get_open_neighbor(expected_lookup_idx))
            nbhd_idx = (open_nbr_idx - expected_lookup_idx) % new_size
            self._set_neighbor(expected_lookup_idx, nbhd_idx)
            self._set_lookup_index_info(open_nbr_idx, data=data_idx)

    def _set_lookup_index_info(
        self, lookup_idx: int, data: Optional[int] = None, nbhd: Optional[int] = None
    ) -> None:
        """
        Update the given index of _indices/_nbhds with new information

        :param lookup_idx: Index in _indices/_nbhds to update
        :param data: New index into _keys/_values, or None to keep same
        :param nbhd: New neighborhood information, or None to keep same
        """
        if lookup_idx < 0:
            raise ValueError("Indexes cannot be negative")
        elif lookup_idx >= self._size:
        ...
        if data is not None:
            self._indices[lookup_idx] = data

        if nbhd is not None:
            self._nbhds[lookup_idx] = nbhd

    def _set_neighbor(self, lookup_idx: int, nbhd_idx: int) -> None:
        """
        Set the given neighbor for the given index as occupied,
        with the index 0 representing the given index

        :param lookup_idx: The index in _nbhds
        :param nbhd_idx: The neighbor in the neighborhood to set
                         occupied
        """
        if lookup_idx < 0 or nbhd_idx < 0:
            raise ValueError("Indexes cannot be negative")
            ...
            raise ValueError("Trying to set neighbor outside neighborhood")
    ...
    def __delitem__(self, key: Hashable) -> None:
        ...
        if data_idx is None:
            ...
            if data_idx != self._count - 1:
                ...
                self._set_lookup_index_info(cast(int, tail_lookup_idx), data=data_idx)
