...
class GridDrawArgs:
    ...
    margin: int = 0
    draw_scale_matrix: mm.TransformMatrixProvider = None
...
def draw_to_surface(
        ...
        grid_color: Color = None,
        margin: int = 0,
        draw_transform_matrix: np.array = None,
        center_guide_color: Color = None,
    ...
    if grid_color is not None:
        _draw_grid(
            ...
            margin=margin,
            draw_transform_matrix=draw_transform_matrix
        )
...
def _draw_grid(
    ...
    if grid_draw_type == GridDrawType.BOXES:
        _draw_grid_boxes(surface=surf, grid=grid, margin=margin, grid_color=grid_color, transform_matrix=draw_transform_matrix)
    elif grid_draw_type == GridDrawType.LINES:
        _draw_grid_lines(surface=surf, grid=grid, margin=margin, grid_color=grid_color, draw_transform_matrix=draw_transform_matrix)
...
def _draw_outlined_grids(surface: pygame.Surface,
    ...
    if draw_transform_matrix is not None:
...
def _scaled_rect_points(rects: List[Rectangle],
    ...
    scaled = mm.point_transform_3d(points,
                                lh_matrix=transform_matrix)
...
def _draw_highlighted_grids(surface: pygame.Surface,
    ...
    scaled_rects = _scaled_rect_points(
        ...
        transform_matrix=draw_scale_matrix
    )
    ...
    for y in range(0, grid.nRows):
        for x in range(0, grid.nColumns):
            grid_box_rect = grid_box_rectangle_at_pos(
                ...
                margin=margin)
            points = help.scaled_points_of_a_rect(grid_box_rect, transform_matrix)[:, :2]
...
def _draw_grid_lines(surface: pygame.Surface,
    ...
    if draw_transform_matrix is None:
        draw_transform_matrix = np.identity(4)
...
def get_mouse_grid_pos(grid: RectGrid,
    ...
    draw_scale_matrix_inv = np.linalg.inv(draw_transform_matrix)
    ...
    mouse_over_grid_coords = help.viewport_point_on_plane(mouse_game_area_coord, game_area_rect, draw_transform_matrix, margin=0)
