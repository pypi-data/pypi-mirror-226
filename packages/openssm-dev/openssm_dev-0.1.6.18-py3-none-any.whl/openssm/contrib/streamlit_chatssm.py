...
class ChatSSMComponent:
    ...
    def run(self):
        ...
        if _knowledge_src_pardir_path := st.text_input(label='Knowledge Source Parent Directory Path (Local|S3|GCS|GDrive)',
            ...
            if to_reload := _knowledge_src_pardir_path != self.knowledge_src_pardir_path:
                # set new Knowledge Source Parent Directory Path
                self.knowledge_src_pardir_path: str = _knowledge_src_pardir_path
                # re-initilize SSM & re-load Knowledge Source
