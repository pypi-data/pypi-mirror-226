...
def framing(input_path,  duration_intv_sec = 10, max_row_width = 4,):
    ...
    try:
        ...
        while start < frame_len:
            end = start + interval_in_frame if start + interval_in_frame < frame_len else frame_len
    ...
    for row in range(math.ceil(imgs.shape[0] / max_row_width)):
        e = s + max_row_width if s + max_row_width < imgs.shape[0] else imgs.shape[0]
        curr = imgs[s:e, :, :, :]
        if max_row_width - (e - s) > 0:
            padding = np.zeros(shape=(max_row_width - (e - s), *curr.shape[1:]), dtype=np.uint8)
...
def FrameCapture(input_path,  duration_intv_sec = 600, max_row_width = 3,):
    ...
    if os.path.isfile(input_path):
        ...
        for ext in accepted_video_extension:
            all_files.extend(glob.glob(input_path + f'/**/*{ext}', recursive=True))
            all_files.extend(glob.glob(input_path + f'/**/*{ext.upper()}', recursive=True))
...
if __name__ == '__main__':
    ...
    FrameCapture(input_path, duration_intv_sec=duration_intv_sec, max_row_width=max_row_width)