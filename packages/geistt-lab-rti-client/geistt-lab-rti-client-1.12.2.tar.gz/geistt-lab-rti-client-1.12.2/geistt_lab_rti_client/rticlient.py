...
class DispatcherMainLoop:
    ...
    def read(self, sock, read_callback, check_callback):
        try:
            while self.web_socket_app.sock.connected:
                read_sockets, _, _ = select.select([sock], [], [], self.__timeout())
                if read_sockets:
                    if not read_callback():
                        break
                    self.idle = time.time() - self.called
                else:
                    self.__call_main_loop()
    ...
    def connect(self, sslopt=None, http_proxy_host=None, http_proxy_port=None):
        ...
        if self.main_loop:
            self.ws.run_forever(sslopt=sslopt, http_proxy_host=http_proxy_host, http_proxy_port=http_proxy_port,
                                dispatcher=DispatcherMainLoop(self.ws, self.main_loop, self.idle_time))
        else:
            self.ws.run_forever(sslopt=sslopt, http_proxy_host=http_proxy_host, http_proxy_port=http_proxy_port)
...
class RTIClient(Emitter):
    ...
    def __init__(self, application: str = "Python",
                 ...
                 wait: bool = False,
                 main_loop: Callable = None, main_loop_idle_time: float = 0.0, create_rti_thread: bool = True):
        super().__init__()
        ...
        self.create_rti_thread = create_rti_thread
        ...
        socket = FixedSocketClusterClient(url, main_loop, main_loop_idle_time)
        socket.setdelay(2)
        ...
        socket.setBasicListener(self.__on_connect, self.__on_disconnect, self.__on_connection_error)
        socket.setAuthenticationListener(self.__on_set_auth, self.__on_auth)
