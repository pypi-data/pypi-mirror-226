...
def init(name=None):
    ...
    # Navigate to the runtime directory relative to the script's location
    ...
    available_frameworks = [d for d in os.listdir(framework_dir) if os.path.isdir(os.path.join(framework_dir, d)) and not d.startswith('__')]
...
def makealive():
    ...
    # Detect runtimes
    for potential_runtime in os.listdir(runtime_dir):
        config_path = os.path.join(runtime_dir, potential_runtime, 'config.py')
        if os.path.exists(config_path):
            ...
            if detect_function:
                main_path = os.path.join(runtime_dir, potential_runtime, 'main.py')
    ...
    framework_dir = os.path.join(runtime_dir, runtime, 'frameworks')
    for potential_framework in os.listdir(framework_dir):
        config_path = os.path.join(framework_dir, potential_framework, 'config.py')
        if os.path.exists(config_path):
            ...
            if detect_function:
                main_path = os.path.join(framework_dir, potential_framework, 'main.py')
                sys.path.insert(0, os.path.dirname(main_path))
                detect_result = detect_function()
                sys.path.pop(0)
                if detect_result:
