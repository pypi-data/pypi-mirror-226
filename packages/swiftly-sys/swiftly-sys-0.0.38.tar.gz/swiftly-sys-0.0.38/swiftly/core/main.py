...
def makeapp(name=None):
    ...
    if len(execute) > 1:
        ...
        if choice == "run makeapp":
            ...
            # Custom style to display squares instead of circles
            custom_style = Style([
                ('checkbox', ' bg:#FF9D00 '),  # Background color for the checkbox
                ('checkbox.selected', ' bg:#FF9D00 ')  # Background color when selected
            ])
