...
def init(name=None):
    ...
    # Convert spaces and dashes in the name to underscores
    if validate_project_name(name) != True:
        pass
    else:
        raise NameError(validate_project_name(name))

    # Get the directory of the currently executing file
    current_dir = os.path.dirname(os.path.abspath(__file__))

    # Navigate one directory up and then to the runtime directory
    ...
    # Create a directory with the usable name
    ...
    clireturn(f"{name}<=====>" + ",".join([runtime] + [f"{runtime}-{framework}" for framework in frameworks]))
