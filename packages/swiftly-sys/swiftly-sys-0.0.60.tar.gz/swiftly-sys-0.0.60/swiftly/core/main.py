...
def makealive():
    ...
    for rt in runtimes:
        try:
            module_name = f"swiftly.runtime.{rt}.config"
            config_module = importlib.import_module(module_name)
            detect_function = getattr(config_module, "detect", None)
            if detect_function:
                result = detect_function()
                if result:  # If the detect function returns True
                    runtime = rt
                    break
        except ImportError:
            continue
    ...
    # Detect the frameworks for the detected runtime
    frameworks = []
    framework_dir = os.path.join(runtime_dir, runtime, 'frameworks')
    available_frameworks = [d for d in os.listdir(framework_dir) if os.path.isdir(os.path.join(framework_dir, d)) and not d.startswith('__')]
    ...
    for framework in available_frameworks:
        try:
            module_name = f"swiftly.runtime.{runtime}.frameworks.{framework}.config"
            config_module = importlib.import_module(module_name)
            detect_function = getattr(config_module, "detect", None)
            if detect_function:
                result = detect_function()
