...
def init(name=None):
    ...
    # Adjust keybindings for multi-select
    kb = KeyBindings()

    @kb.add('enter')
    def _(event):
        checkbox = event.app.layout.current_control
        checkbox.current_item.checked = not checkbox.current_item.checked

    @kb.add('s')
    def _(event):
        event.app.exit(result=event.app.layout.current_control.checked_values)
