...
def create_table_if_not_exists(username):
    ...
    cursor.execute(f"SELECT to_regclass('{username}');")
    if cursor.fetchone()[0]:
        print(f"Table '{username}' already exists.")
    ...
    try:
        ...
        logger.info(f"Table '{username}' created successfully.")
    except Exception as e:
        logger.info(f"An error occurred while creating the '{username}' table: {e}")
...
def insert_into_db(username, filename, metadata, embeddings, embeddings_text):
    ...
    insert_query = f"""
        INSERT INTO {username} (filename, metadata, embeddings, embeddings_text)
        VALUES (%s, %s, %s, %s);
    """
    ...
    try:
        ...
        logger.info(f"Record inserted successfully into '{username}' table.")
    except Exception as e:
        logger.info(f"An error occurred while inserting the record into the '{username}' table: {e}")
        raise
