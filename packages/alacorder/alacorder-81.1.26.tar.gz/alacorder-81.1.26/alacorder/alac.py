...
class AlacourtDriver:
    ...
    with path to downlaod directory, then call login() before searching.
    """
        ...
        try:
            ...
            blue_tables = pl.read_excel(
                ...
                [
                    ...
                    pl.col("MinReleaseDate").str.to_date(
                        "%Y-%m-%d", strict=False
                    ),
...
class Cases:
    ...
    def fees(self, debug: bool = False) -> pl.DataFrame:
        ...
        with console.status("Parsing fee sheets…"):
            df = (
                self.archive
                .select("CaseNumber", "AllPagesText")
                .select(
                    [
                        pl.col("CaseNumber"),
                        pl.col("AllPagesText")
    ...
    def filing_charges(self, debug: bool = False) -> pl.DataFrame:
        ...
        with console.status("Parsing filing charges…"):
            df = (
                ...
                .with_columns(
                    pl.col("Description").str.replace(r"\\", "\\\\")
                )
                # fix CFR cites
                .with_columns(
                    pl.when(
    ...
    def disposition_charges(self, debug: bool = False) -> pl.DataFrame:
        ...
        with console.status("Parsing disposition charges…"):
            df = (
                self.archive
                .select("AllPagesText", "CaseNumber")
                .select(
                    [
                        pl.col("CaseNumber"),
                        pl.col("AllPagesText")
    ...
    def settings(self, debug: bool = False) -> pl.DataFrame:
        ...
        with console.status("Parsing settings…"):
            df = (
                ...
                .filter(
                    pl.col("Settings").str.contains(r"^DOB|SSN").is_not()
                )
        ...
        with console.status("Parsing attorneys tables…"):
            df = (
                ...
                .with_columns(
                    ...
                    .then(
                        pl.col("Name").str.replace(r"^DEFENDER ", "").str.strip()
                    )
...
def get_bond_amount(text: str) -> float:
...
def get_bondsman_process_return(text: str) -> str:
