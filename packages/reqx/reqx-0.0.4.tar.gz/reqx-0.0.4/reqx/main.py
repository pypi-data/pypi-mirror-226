...
async def backoff(fn: callable, *args, m: int = 20, b: int = 2, max_retries: int = 8, **kwargs) -> any:
    ...
    logger.debug(f'Backoff: {args = }')
    logger.debug(f'Backoff: {kwargs = }')
    logger.debug(f'Backoff: {_kwargs = }')
    logger.debug(f'Backoff: {m = }')
    logger.debug(f'Backoff: {b = }')
    logger.debug(f'Backoff: {max_retries = }')
...
def build_requests(method: str, urls: list[str], debug: int, **kwargs) -> Generator[Request]:
    """
    ...
    for url in urls:
        ...
        for k, v in _kwargs.items():
            if k in REQUEST_DYNAMIC:
                curr = helper(v)
                tmp = [{**x, k: y} for x in tmp for y in curr]
...
def send(method: str, urls: list[str], debug: int = 0, **kwargs) -> Generator[partial]:
    """
