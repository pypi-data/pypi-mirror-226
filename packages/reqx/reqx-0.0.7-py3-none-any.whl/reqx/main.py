...
def send(method: str, urls: list[str | URL], debug: int = 0, **kwargs) -> Generator[partial]:
    ...
    return (partial(fn, r=r) for r in _build_requests(method, urls, debug, **kwargs))
...
def _build_requests(method: str, urls: list[str | URL], debug: int, **kwargs) -> Generator[Request]:
    """
    ...
    def expand(d: dict) -> list[dict]:
        ...
        for k, v in d.items():
            if isinstance(v, (list, range, set, tuple)):
                res = [d | {k: vv} for d in res for vv in v]
        return res
    ...
    for url in urls:
        ...
        for k, v in _kwargs.items():
            tmp = [x | {k: y} for x in tmp for y in (expand(v) if k in REQUEST_DYNAMIC else [v])]
        data.extend(tmp)
