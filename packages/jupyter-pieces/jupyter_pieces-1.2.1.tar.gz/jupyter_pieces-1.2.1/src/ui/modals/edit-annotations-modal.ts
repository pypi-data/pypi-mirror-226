...
import { Annotation, AnnotationTypeEnum } from '../../PiecesSDK/core';
...
export class AnnotationsModal {
  ...
  open() {
    ...
    modalHeader.innerText = 'Annotations';
    modalContent.appendChild(modalHeader);

    const annotationForm = document.createElement('div');
    modalContent.appendChild(annotationForm);
    ...
    modalContainer.appendChild(annotationModal);
    ...
    const handleWindowHide = (event: any) => {
      if (
        event.target !== annotationModal &&
        !annotationModal.contains(event.target)
      ) {
        clicks++;
        if (clicks >= 2) {
  ...
  annotationWidget = (
    ...
    saveBtn.style.color = theme ? 'white' : 'black';
