    ...
    def IS_DEBUG(self, value):
        ...
        if isinstance(logger.filter, set):
            if not self._IS_DEBUG:
                logger.filter.add('DEBUG')
            elif self._IS_DEBUG:
                if 'DEBUG' in logger.filter:
                    logger.filter.remove('DEBUG')
    ...
    def IS_REQUEST_LOGGED(self, value):
        ...
        if isinstance(logger.filter, set):
            if not self._IS_REQUEST_LOGGED:
                logger.filter.add('HTTP')
                logger.filter.add('WEBSOCKET')
            elif self._IS_REQUEST_LOGGED:
                if 'HTTP' in logger.filter:
                    logger.filter.remove('HTTP')
                if 'WEBSOCKET' in logger.filter:
                    logger.filter.remove('WEBSOCKET')
