...
class App:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if scope['type'] == 'lifespan':
            ...
            if message['type'] == 'lifespan.startup':
                ...
                if self.process.name == 'MainProcess':
                    logger.starting('The server started, took {:.6f} seconds'.format(startTime), 'The server started, took \033[34m{:.6f}\033[0m seconds'.format(startTime))
                else:
                    logger.starting(f'The worker {os.getpid()} started, took ' + '{:.6f} seconds'.format(startTime), f'The worker \033[34m{os.getpid()}\033[0m started, took ' + '\033[34m{:.6f}\033[0m seconds'.format(startTime))
        ...
        if scope['type'] in [ 'http', 'https' ]:
            ...
            logger.http(f'{request.ip} accessed {request.method} {request.fullPath}, returned {response.status}, took ' + '{:.6f}'.format(diffTime) + ' seconds', f'{request.ip} accessed \033[36m{request.method} {request.fullPath}\033[0m, returned \033[34m{response.status}\033[0m, took ' + '\033[34m{:.6f}\033[0m'.format(diffTime) + ' seconds')
        ...
        if scope['type'] in [ 'websocket', 'websockets' ]:
            try:
                ...
                if (await receive())['type'] == 'websocket.connect':
                    logger.websocket(f'{request.ip} connected {request.path}', f'{request.ip} connected \033[36m{request.path}\033[0m')
                    ...
                    while True:
                        ...
                        if message['type'] == 'websocket.receive':
                            ...
                            task.cancel()
                            logger.websocket(f'{request.ip} disconnected {request.path}', f'{request.ip} disconnected \033[36m{request.path}\033[0m')
                            break
