...
class Symbol(sympy.core.symbol.Symbol):
    ...
    def PartialDerivative(cls, what, by, *args, **kwargs):
        ...
        kwargs.setdefault(
            "latex",
            rf"\frac{{\partial {{{sympy.latex(what)}}} }}{{\partial {{{sympy.latex(by)}}} }}",
        )
        if by is z:
    ...
    def Covariance(cls, s1, s2, *args, **kwargs):
        ...
        kwargs.setdefault(
            "latex",
            rf"\overline{{ {{{sympy.latex(s1)}}}' {{{sympy.latex(s2)}}}' }}",
        )
    ...
    def difference(self, *args, along=None, **kwargs):
        ...
        if not args:
            args = [f"Δ{self}"]
        kwargs.setdefault("title", f"{self.title} difference")
        ...
        if isinstance(along, type(self)):
            ...
            kwargs.setdefault(
                "latex", rf"\Delta_{{{sympy.latex(along)}}}{{{self}}}"
            )
...
Θ = θ = potential_temperature = Symbol(
    ...
    description="potential temperature, defined as the temperature a volume of gas has after adiabatically changing its pressure to a reference pressure (typically 1000 hPa)",
    positive=True,
)
dθdz = (
...
# 🔨 Pressure
p = pressure = Symbol("p", unit="Pa", positive=True)
...
for symbol, _names in NAMES.items():
    if str(symbol) not in _names and str(symbol).isidentifier():
        warnings.warn(
            f"Symbol {symbol} a string representation unlike "
...
def _sympy_latex(*args, **kwargs):
    # TODO: or should we globally override SymPy's LaTeX printer (if that's possible)?
    return sympy.latex(*args, **{**dict(mul_symbol=r" \cdot "), **kwargs})
...
def _symbol_entry(symbol):
    ...
    :math:`{_sympy_latex(symbol)}`   :math:`\left[{unitfmt}\right]`
...
def formatted_list_of_equation_functions(_locals):
    ...
    return f"\n\n".join(
        [
            f":any:`{name}`  →  :math:`{_sympy_latex(fun.equation)}`"
...
{(2*chr(10)).join([_symbol_entry(symbol) for symbol, names in sorted(NAMES.items(), key=str)])}
...
def from_sympy(
    ...
    def decorator(decorated_fun):
        ...
        def multiarg_wrapper(**given_kwargs):
            """
        ...
        for (name, param), symbol in zip(
            ...
            argtypes = [f":math:`{_sympy_latex(symbol)}`"]
            with contextlib.suppress(AttributeError):
                argtypes.append(
                    r":math:`\left[{}\right]`".format(
        ...
        .. math::
            ...
            {_sympy_latex(equation)}

        Args:

{textwrap.indent(chr(10).join(argdoc).strip(),2*'    ')}

        :return: :math:`{_sympy_latex(result_symbol)}` {unitfmt(result_symbol)} - {get_symbol_doc(result_symbol)}
