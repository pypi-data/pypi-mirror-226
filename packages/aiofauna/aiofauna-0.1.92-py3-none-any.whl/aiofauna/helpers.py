...
def _(response: bytes) -> Response:
    return Response(status=200, body=response, content_type="application/octet-sse_stream")
...
def sse_stream(generator: typing.Callable[P, typing.AsyncGenerator]) -> Response:
	"""
	Decorator to convert a generator into an SSE sse_stream.
	"""
	async def wrapper(*args:P.args, **kwargs:P.kwargs):
		async for item in generator(*args, **kwargs):
			yield to_json(item).encode("utf-8") + b"\n\n"
	return Response(body=wrapper(), content_type="text/event-sse_stream")
