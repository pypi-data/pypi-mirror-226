"""DAQ that runs in subprocess.

Should be able to read 4 counters with roughly 1000 Hz
...
class SubprocessDaq(DaqLabjack):
    """Daq with fast sampling."""

    def __init__(self, output_folder: Path) -> None:
        super().__init__()
        self.output_folder: Path = output_folder
        self.daq_file: Path = self.output_folder / "daq.csv"
        ...
        self.timeout_start: datetime.datetime | None = None
    ...
    def wait_for_counters_to_increase(self) -> None:
        ...
        print(f"Waiting on counters to increase: {counter_names}")
        ...
        print("All counters have increased.")
                ...
                if not self.is_ready_per_counter[name]:
                    ...
                    if self.increases_per_counter[name] > self.n_increases:
                        ...
                        if np.all(is_ready):
                            ...
                            print(f"Still waiting on {missing_counters}")

    def run(self) -> None:
        """Main method to call. Samples daq with 1000 Hz."""
        assert isinstance(self.main_counter, str)
        print("Scanning all counters from now on.")
        while True:
            row, current_time, current_reads = self.sample()
            self.write(row, current_reads)
            self.update(current_time, current_reads)
        ...
        if self.count == 0:
            ...
            for name, val in current_reads.items():
                if self.last_reads[name] != val:
                    ...
                    self.timeout_start = current_time
                else:
    ...
    def wait_for_counters_to_stop(self, seconds: float = 5) -> None:
        elapsed = 0
        print("Waiting for all counters to stop.")
        while elapsed < seconds:
            row, current_time, current_reads = self.sample()
            self.write(row, current_reads)
            self.update(current_time, current_reads)
            elapsed = (current_time - self.timeout_start).total_seconds()
        print("All counters have stopped.")


if __name__ == "__main__":
    daq = SubprocessDaq(Path("/home/pennartz/Mathis/gitlab/trigger_count/results"))
    daq.add_counter("widefield_camera", "DIO0")
    # daq.add_counter("vitals_monitor", "DIO1")
    # daq.add_counter("left_eye_camera", "DIO2")
    # daq.add_counter("right_eye_camera", "DIO3")
    daq.set_main_counter("widefield_camera")
    daq.wait_for_counters_to_increase()
    daq.wait_for_counters_to_stop()
