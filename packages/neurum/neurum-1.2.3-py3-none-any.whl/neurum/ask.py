...
def ask(prompt: str, api_key : str, internet : bool=False, reference_image : bool=False):
    if api_key=="vanshshah1029384756":
        if internet==False and reference_image==False:
          ...
          while retry < max_retries:
            ...
            bard=Bard("aAgDyFtcCeFdE5bHLQqsyasOhtmHvLmp2HHfQLomec_ROxN4wGQEY9MdrfhRYAdiD7V7Ew.")
            ans=bard.get_answer(prompt)['content']
            return ans
        if reference_image==True:
            bard=Bard("aAgDyFtcCeFdE5bHLQqsyasOhtmHvLmp2HHfQLomec_ROxN4wGQEY9MdrfhRYAdiD7V7Ew.")
            img = open(reference_image, 'rb').read()
            ans = bard.ask_about_image(prompt, img)
            return ans
