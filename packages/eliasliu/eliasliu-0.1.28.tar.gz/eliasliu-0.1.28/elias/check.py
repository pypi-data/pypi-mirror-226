...
def integrity_check(table_name,hosts,output='integrity_rate',output_hosts={},output_table_name = 'quality_check_records_integrity'):
    '''
    完备性检查
...
def key_check(table_name,hosts,key,output='key_rate',output_hosts={},output_table_name = 'quality_check_records_key'):
...
def time_check(table_name,hosts,updated,time_rule = '09:00:00',output='time_score',output_hosts={},output_table_name = 'quality_check_records_time'):
...
def same_check(table_name ,hosts,sum_list,count_list,source_table_name,source_hosts,source_sum_list,source_count_list,source_where,output='same_rate',output_hosts={},output_table_name = 'quality_check_records_same'):
...
def validity_check(table_name,hosts,standard_dic,sum_list,count_list,output='validity_score',output_hosts = {},output_table_name='quality_check_records_validity'):   
...
def quality_check(table_name,hosts,sum_list,count_list,key,time_rule,updated,standard_dic,source_table_name,source_hosts,source_sum_list,source_count_list,source_where,output_hosts):

    import pandas as pd

    # 完备性验证：非null值占比
    integrity_rate = integrity_check(table_name,hosts,output='integrity_rate',output_hosts = output_hosts)
    
    # 唯一性验证：主键非重复记录比例
    key_rate = key_check(table_name,hosts,key=key,output='key_rate',output_hosts = output_hosts)
    
    # 及时性验证：更新时间是否满足要求
    time_score = time_check(table_name,hosts,updated,time_rule,output='time_score',output_hosts = output_hosts)
    
    # 一致性验证
    same_rate = same_check(table_name ,hosts,sum_list,count_list,source_table_name,source_hosts,source_sum_list,source_count_list,source_where,output='same_rate',output_hosts = output_hosts)
    
    # 有效性验证
    validity_score = validity_check(table_name, hosts, standard_dic, sum_list, count_list,output='validity_score',output_hosts = output_hosts)
