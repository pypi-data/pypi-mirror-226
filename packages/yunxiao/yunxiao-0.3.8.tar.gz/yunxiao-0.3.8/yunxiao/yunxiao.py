...
class Time:
    ...
    last_week_monday = _this_week_monday_.replace(day=_this_week_monday_.day - 7).strftime("%Y-%m-%d")
    last_week_sunday = _this_week_monday_.replace(day=_this_week_monday_.day - 1).strftime("%Y-%m-%d")
    next_week_monday = _this_week_monday_.replace(day=_this_week_monday_.day + 7).strftime("%Y-%m-%d")
    next_week_sunday = _this_week_monday_.replace(day=_this_week_monday_.day + 13).strftime("%Y-%m-%d")

    # 月
    _next_month_ = _today_.replace(year=_today_.year + 1, month=1) if _today_.month == 12 else _today_.replace(
        month=_today_.month + 1)
    this_month_start = _today_.replace(day=1).date().strftime("%Y-%m-%d")
    this_month_end = (_next_month_.replace(day=1) - timedelta(days=1)).date().strftime("%Y-%m-%d")
...
print(Time.last_week_monday)
print(Time.last_week_sunday)