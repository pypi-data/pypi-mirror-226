...
def _rest_extract(result, arg_dict) -> Dict:
    d = _sempy_extract_message(result, arg_dict)
    response = arg_dict['response']
    token = response.request.headers["authorization"].split("Bearer ")[1]
    ...
    d['url'] = urllib.parse.unquote(response.request.url)
    d['request_id'] = response.headers.get("RequestId", None)
...
log_rest = _sempy_log(_rest_extract)
