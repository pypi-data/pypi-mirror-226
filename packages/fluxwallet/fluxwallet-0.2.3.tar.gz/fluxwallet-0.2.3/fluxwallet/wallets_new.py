...
async def main():
    db = await Db.start()
    # print(await wallets_list())
    # print(await wallet_delete(2, force=True))
    # print(await wallet_empty(1))
    session = db.get_session()
    wk = await WalletKey.from_db_key(9, session)
    print("as dict", wk.as_dict())
    print("balance", wk.balance())
    print("name", wk.name)
    print("public", wk.public())
    print("key", wk.key())
