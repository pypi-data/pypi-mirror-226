...
def move_with_sudo_and_set_permissions(src, dest):
    """Move a file using sudo (if available) and set executable permissions."""
    if is_sudo_available():
        move_cmd = ["sudo", "mv", src, dest]
        chmod_cmd = ["sudo", "chmod", "+x", dest]
    else:
        move_cmd = ["mv", src, dest]
        chmod_cmd = ["chmod", "+x", dest]
    
    try:
        subprocess.check_call(move_cmd)
        subprocess.check_call(chmod_cmd)
    except subprocess.CalledProcessError:
        print("Failed to move the file or set permissions.")
        return False
    return True
...
def update_unigridchain():
    ...
    chain_ver = get_latest_installed_version(
        base_path, _base_unigridchain_name)
...
def enable_udp_port(port):
    ...
    cmd_prefix = ["sudo"] if is_sudo_available() else []

    # Open the port in ufw
    try:
        subprocess.run(cmd_prefix + ['ufw', 'allow', str(port)+'/udp'], check=True)
...
def enable_tcp_port(port):
    ...
    # Open the port in ufw
    try:
        subprocess.run(cmd_prefix + ['ufw', 'allow', str(port)+'/tcp'], check=True)
...
def ensure_ufw_installed():
    ...
    try:
        ...
        try:
            subprocess.run(cmd_prefix + ['apt', 'update'], check=True)
            subprocess.run(cmd_prefix + ['apt', 'install', '-y', 'ufw'], check=True)
