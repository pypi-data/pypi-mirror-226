...
single_gate = ["H", "X", "Y", "Z", "S", "T",
               "SD", "TD", "X2M", "X2P", "Y2M", "Y2P", "M"]
...
class Drawer:
    ...
    def single_gate(self, norm, seq_t, width=1):
        ...
        self.box_width = 0.5
        new_text_options = {"zorder": 3, "ha": "center",
    ...
    def ctrl_gate(self, name, qubit, seq_t):
        ...
        if (name == "CX" or name == "CNOT"):
            ...
            new_options = {"facecolor": "white",
                           "edgecolor": "black", "linewidth": 2}
            target_circ = plt.Circle(
                (seq_t, qubit[1]), radius=circ_rad, **new_options)
            target_v = plt.Line2D(
                (seq_t, seq_t), (qubit[1] - circ_rad, qubit[1] + circ_rad), **options)
            target_h = plt.Line2D(
                (seq_t - circ_rad, seq_t + circ_rad), (qubit[1], qubit[1]), **options)
            ...
            if qubit[1] > qubit[0]:
                line = plt.Line2D(
                    (seq_t, seq_t), (qubit[0], qubit[1] - self.box_length/2.0 - 1.5*self.pad), **options)
                self.ax.add_line(line)
            else:
                line = plt.Line2D(
                    (seq_t, seq_t), (qubit[0], qubit[1] + self.box_length/2.0 + self.pad), **options)
                self.ax.add_line(line)
