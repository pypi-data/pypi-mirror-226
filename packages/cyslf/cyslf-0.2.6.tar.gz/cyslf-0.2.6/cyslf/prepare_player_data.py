...
def _load_existing_player_data(filename: str, division: str):
    ...
    for key, val in column_map.items():
        if key not in existing_players_raw.columns:
            raise ValueError(
                print(f"Coach evals file is missing the \"{key}\" column.")
            )
...
def _load_parent_requests(filename):
    ...
    if "Player First Name (as it is written in your CYS/Sports Connect account)" not in parent_reqs.columns:
        raise ValueError(
            print("Missing first name column in parent requests file. Make sure the column name matches the docs.")
        ) 
    if "Player Last Name" not in parent_reqs.columns:
        raise ValueError(
            print("Missing last name column in parent requests file. Make sure the column name matches the docs.")
        )  
    for key, val in column_map.items():
...
def _load_registration_data(filename):
    ...
    if "latitude" not in registrations_raw.columns or "longitude" not in registrations_raw.columns:
        raise ValueError(
            print("\nERROR: Please use the convert addresses command before preparing player data. See documentation for more details.\n")
        )
    ...
    for key, val in column_map.items():
        if key not in registrations_raw.columns:
            raise ValueError(
                print(f"ERROR: Registration file doesn't contain the \"{key}\" column.")
            )
...
def main():
    ...
    if not args.registration: 
        if args.folder: 
            ...
            raise ValueError(print("Please provide a registration file."))

    if not args.coach_evals: 
        if args.folder:
            args.coach_evals = args.folder + "/coach_evals.csv"
        else:
            raise ValueError(print("Please provide a coach evaluations file."))
        
    if not args.output_file: 
        if args.folder:
            args.output_file = args.folder + "/prepared_player_data.csv"
        else:
            raise ValueError(print("Please provide a valid output file path."))
    ...
    try: 
        ...
        raise ValueError(
            print(f"Invalid registration file path {args.registration}.")
        )
    try:   
        validate_file(args.coach_evals)
    except:
        raise ValueError(print(f"Invalid coach evaluations file path {args.coach_evals}."))
    ...
    if args.replace or not os.path.exists(args.output_file):
        ...
        print(f"\nERROR: Not saving. {args.output_file} already exists. Use -r to replace\n")
