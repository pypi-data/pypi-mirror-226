...
class GeneratePPT(Logger):
    ...
    def __init__(self, config:Config):
        ...
        if config.hl_active:
            ...
            self.hl_pages = [
                HighlightSummary(hl_user=config.hl_user,hl_pswd=config.hl_password,hl_instance=config.hl_instance,
...
#        crit_rule_cols = [col for col in rules_df if col.startswith('rulePattern')]
    ...
    def oss_risk_assessment(self,hl_id,app_no,day_rate):
        ...
        if not oss_df.empty:
            self._ppt.update_table(f'app{app_no}_HL_table_CVEs',oss_df,include_index=False,max_rows=5)
