...
class NbTenant(NetboxBase):
    def __init__(self, komora_obj=None):
        ...
        if komora_obj.isCustomer is True:
            self.custom_fields['komora_is_customer'] = True
    ...
    def find(self):
        ...
        if komora_id:
            try:
                if komora_id in tenant_cache:
                    ...
                    netbox_tenant = self.netbox.connection.tenancy.tenants.get(cf_komora_id=komora_id)
        ...
        if not netbox_tenant:
            try:
                netbox_tenant = self.netbox.connection.tenancy.tenants.get(name__ie=self.name)
    ...
    def create(self):
        try:
            ...
            netbox_tenant = self.netbox.connection.tenancy.tenants.create(params)
