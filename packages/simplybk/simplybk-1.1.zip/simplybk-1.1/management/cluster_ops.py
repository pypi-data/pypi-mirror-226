...
from management import utils, scripts, mgmt_node_ops, storage_node_ops
...
def create_cluster(blk_size, page_size_in_blocks, ha_type, tls,
                   auth_hosts_only, nqn, cli_pass, model_ids):
...
def join_cluster(cluster_ip, cluster_id, role, ifname, data_nics):  # role: ["management", "storage"]
    ...
    try:
        cluster_docker = docker.DockerClient(base_url=f"tcp://{docker_ip}", version="auto")
        if role == 'management':
        ...
        if node_docker.info()["Swarm"]["LocalNodeState"] == "active":
            ...
            time.sleep(3)
        node_docker.swarm.join([docker_ip.split(":")[0] + ":2377"], join_token)
