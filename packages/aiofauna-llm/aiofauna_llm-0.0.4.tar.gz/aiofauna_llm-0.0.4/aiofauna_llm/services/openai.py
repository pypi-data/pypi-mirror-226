...
from ..schemas.typedefs import (F, FunctionCall, FunctionDocument, Vector,
...
class LLMStack:
    ...
    @handle_errors
    async def parse_openai_function(self,  # pylint: disable=dangerous-default-value
        response: dict,
        functions: List[
            Type[F]
    ...
    @handle_errors
    async def function_call(self,  # pylint: disable=dangerous-default-value
        text: str,
        context: Optional[str] = None,
        model: Model = "gpt-3.5-turbo-16k-0613",
        functions: List[
            Type[F]
        ] = FunctionDocument.Metadata.subclasses,
        **kwargs,
    ) -> FunctionCall:
        """
