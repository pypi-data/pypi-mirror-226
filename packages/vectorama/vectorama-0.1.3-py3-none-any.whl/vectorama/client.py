...
class VectoramaConnection(Generic[T]):
    ...
    async def register_schema(self) -> Schema:
        ...
        if existing_response.schemas:
            ...
            server_parsed = ParsedPydanticModel(
                desired_cluster_count=existing_response.schemas[0].desiredClusterCount,
                desired_cluster_size=existing_response.schemas[0].desiredClusterSize,
                chunk_size=existing_response.schemas[0].chunkSize,
                embedding_length=existing_response.schemas[0].embeddingDimension,
                metadatas=frozenset(
                    [
                        MetadataFieldWrapper(raw)
                        for raw in existing_response.schemas[0].fields
                    ]
                ),
            )
            ...
            if pydantic_parsed != server_parsed:
                raise ValueError(
                    f"Existing schema `{existing_response.schemas[0].name}` has fields {server_parsed}, "
                    f"but new schema has fields `{pydantic_parsed}`"
                )

            self.server_schema = existing_response.schemas[0]
        else:
            # Register the schema
    ...
    async def get_stats(self):
        ...
        schemas_response = await self.stub.GetSchemas(
            GetSchemasRequest(names=[self.model.__name__], returnClusters=True)
        )
