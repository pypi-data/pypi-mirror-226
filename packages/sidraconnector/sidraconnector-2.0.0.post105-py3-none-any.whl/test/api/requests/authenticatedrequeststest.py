...
from sidraconnector.sdk.requests.oauthauthorizationrequest import OAuthAuthorizationRequest, OAuthAuthorizationParameters
...
class Test_AuthenticatedRequests(unittest.TestCase):
  ...
  def test_oauth_password_flow_should_call_url_with_access_token(self, mock_requests):
    # Arrange
    parameters = OAuthAuthorizationParameters()
    ...
    mock_requests.get(self.sample_url, text='{"test": "valuue"}')
    # Act
    service.request(self.sample_url)
    # Assert
    assert mock_requests.last_request.url == self.sample_url
    assert mock_requests.last_request.headers['Authorization'] == 'bf840afd71e5e50bccefe6585966495d'
