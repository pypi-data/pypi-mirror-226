...
class WillowGoogleSheetClient:
    ...
    def get_drive_client(self):
        ...
        try:
            creds = ServiceAccountCredentials.from_json_keyfile_name('/home/willow/repo/pythonmail/credentials-spreadsheet.json', scope_spreadsheet)
    ...
    def get_all_data_from_sheet(self,sheet):
        """
        Returns all the records from the sheet.
        
        Returns:
            A list of dictionaries representing the records from the sheet.
        """
        sheet_instance = self.get_sheet(sheet)
        if sheet_instance:
            return sheet_instance.get_all_records()
        else:
            return []
