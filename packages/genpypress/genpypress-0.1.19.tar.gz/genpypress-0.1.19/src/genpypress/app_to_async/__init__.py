...
def to_async(
    ...
    for f in files:
        print(f)
        file_to_async(f, encoding=encoding)
...
def file_to_async(f: pathlib.Path, encoding: str = "utf-8"):
    ...
    new_content = func(content, file_name=f"{f.parent.name}/{f.name}")
