                ...
                if (
                    ...
                    if cast_from_type == "float" and is_db_type_int:
                        ...
                        if datetime_format is not None:
                            column_expr = (
                                f"TO_TIMESTAMP({self.format_name(column_name)}, "
                                f"'{datetime_format}')::DATETIME"
                            )
                        else:
                            column_expr = f"{self.format_name(column_name)}::DATETIME"
                    else:
                        column_expr = (
    ...
    def generate_isin_collection_expression(
        ...
        return f"""
                ARRAY_LENGTH(
                    ARRAY(
                        SELECT X
                        FROM UNNEST([{{", ".join(value_strings)}}]) AS X
                        WHERE X = {self.format_name(column_name)}
                    )
                ) > 0
            """
