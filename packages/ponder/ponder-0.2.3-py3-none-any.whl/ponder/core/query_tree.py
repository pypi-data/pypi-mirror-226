...
class RowWiseFilterPredicates:
    ...
    class PandasQuery(RowWiseFilterPredicate):
        def __init__(
            self,
            query: str,
            ...
            self._column_name_mapper = None

        def generate_predicate(self, conn) -> str:
            return conn.generate_pandas_query_predicate(
                self._query,
                self._local_dict,
                self._global_dict,
                self._input_columns,
        ...
        def update_column_names(self, column_name_mapper, index_column_name_mapper):
            new_input_columns = None
            if self._input_columns:
                new_input_columns = [
                    self.get_new_column_name(
                        col_name, column_name_mapper, index_column_name_mapper
                    )
                    for col_name in self._input_columns
                ]
            ret_val = RowWiseFilterPredicates.PandasQuery(
                self._query, self._local_dict, self._global_dict, new_input_columns
            )
            ret_val._column_name_mapper = column_name_mapper
            return ret_val
...
class QueryTree(object):
    ...
    def add_cumulative_function(
        ...
        if other_col_id is not None:
            ...
            column_types = (
                ["int", "object"] + column_types.tolist()
                if isinstance(column_types, pandas.Series)
                else column_types
            )
...
class DotProductNode(QueryTreeNode):
    def __init__(
        ...
        new_col_type = None
        for col_type in list(self._left_column_types.values) + list(
            self._right_column_types.values
        ):
