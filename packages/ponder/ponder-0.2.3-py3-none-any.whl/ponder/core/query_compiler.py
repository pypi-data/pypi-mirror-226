...
class DBMSQueryCompiler(BaseQueryCompiler):
    ...
    def replace(
        ...
        if "value" in column_df.columns:
            ...
            suffix = "_PONDER_ORIG"
            project_column_types = df._query_tree._root.get_column_types().tolist()
            exp_dict = {}
            for col in changed_columns:
                project_column_types.append(
                    dict(
                        zip(
                            self.columns,
                            df._query_tree._root.get_column_types().tolist(),
                        )
                    )[col]
                )
                inner_exp = (
    ...
    def query(
        ...
        # modin API layer should take care of populating local_dict and global_dict with
        # any variables we need to know about.
        return self.__constructor__(
            self._dataframe.filter_rows(
                condition=RowWiseFilterPredicates.PandasQuery(
                    expr,
                    ({} if local_dict is None else local_dict),
