...
def dynamic_enqueue_tpu_embedding_ragged_tensor_batch(sample_splits: List[_atypes.TensorFuzzingAnnotation[TV_DynamicEnqueueTPUEmbeddingRaggedTensorBatch_T1]], embedding_indices: List[_atypes.TensorFuzzingAnnotation[TV_DynamicEnqueueTPUEmbeddingRaggedTensorBatch_T2]], aggregation_weights: List[_atypes.TensorFuzzingAnnotation[TV_DynamicEnqueueTPUEmbeddingRaggedTensorBatch_T3]], mode_override: _atypes.TensorFuzzingAnnotation[_atypes.String], device_ordinal: _atypes.TensorFuzzingAnnotation[_atypes.Int32], table_ids, combiners=[], max_sequence_lengths=[], num_features=[], name=None):
  r"""TODO: add doc.
...
def dynamic_enqueue_tpu_embedding_ragged_tensor_batch_eager_fallback(sample_splits: List[_atypes.TensorFuzzingAnnotation[TV_DynamicEnqueueTPUEmbeddingRaggedTensorBatch_T1]], embedding_indices: List[_atypes.TensorFuzzingAnnotation[TV_DynamicEnqueueTPUEmbeddingRaggedTensorBatch_T2]], aggregation_weights: List[_atypes.TensorFuzzingAnnotation[TV_DynamicEnqueueTPUEmbeddingRaggedTensorBatch_T3]], mode_override: _atypes.TensorFuzzingAnnotation[_atypes.String], device_ordinal: _atypes.TensorFuzzingAnnotation[_atypes.Int32], table_ids, combiners, max_sequence_lengths, num_features, name, ctx):
  if not isinstance(sample_splits, (list, tuple)):
    raise TypeError(
        "Expected list for 'sample_splits' argument to "
        "'dynamic_enqueue_tpu_embedding_ragged_tensor_batch' Op, not %r." % sample_splits)
