...
class AuthCallback(Resource):
    ...
    def get(self):
        ...
        if auth_code is None:
            ...
            print(msg)
            redirect_attempted = request.cookies.get("redirect_attempted")
            if not redirect_attempted:
                # Mark that we've tried redirecting
                response = get_login_redirect_response()
                response.set_cookie("redirect_attempted",
                                    "1", max_age=10 * 60, secure=True, samesite='Strict')  # 10 minutes
                return response
            else:
        ...
        if id_token and '.' in id_token and id_token.count('.') >= 2:
            ...
            msg = "Invalid id_token after redirect attempt."
            print(msg)
            redirect_attempted = request.cookies.get("redirect_attempted")
            if not redirect_attempted:
                # Mark that we've tried redirecting
                ...
                response.set_cookie('redirect_attempted', '',
                                    expires=0, secure=True, samesite='Strict')
                return response
        ...
        if id_token and '.' in id_token and id_token.count('.') >= 2:
            try:
                ...
                response = make_response(
                    {"message": "Error in decoding id_token: str(ex)"}, 400)
                response.set_cookie('redirect_attempted', '',
                                    expires=0, secure=True, samesite='Strict')
                return response
