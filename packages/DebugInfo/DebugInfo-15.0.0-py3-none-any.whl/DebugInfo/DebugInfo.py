...
class 调试模板:
    ...
    def __init__(self,
        ...
        if 表格列间距 is not None:
            if isinstance(表格列间距, int):
                self.__表格列间距 = 表格列间距
            elif isinstance(表格列间距, list):
                self.__表格列间距 = copy(表格列间距)
            else:
                self.__表格列间距 = 2
    ...
    def 准备表格(self, 对齐控制串: str = None, 列宽控制表: list[int] = None):
        ...
        class 次次级方法类:
            def 修饰行(self, 方法: Callable[[str], str] = None) -> None:
                pass
            ...
            for 列号 in range(总列数):
                ...
                if 列号 < 列数:
                    ...
                    if 本列对齐方式 == 对齐方式.左对齐:
                        ...
                        本列宽度: int
                        if 列号 + 1 < 总列数:
                            本列宽度 = 列起位置[列号 + 1] - 列起位置[列号] - 列间距表[列号]
                        else:
                            本列宽度 = 各列最长字符串显示长度[列号]
    ...
    def __修饰最后一行(self, 方法: Callable[[str], str] = None) -> None:
        if callable(方法):
            if self.__表格:
                ...
                for 序号 in range(len(self.__表格[最后一行的索引])):
                    ...
                    if 换行符的个数 == 0:
                        self.__表格[最后一行的索引][序号] = str(方法(元素)).strip()
                    else:
                        self.__表格[最后一行的索引][序号] = 换行符.join([str(方法(段)).strip() for 段 in 元素.split(换行符)])
