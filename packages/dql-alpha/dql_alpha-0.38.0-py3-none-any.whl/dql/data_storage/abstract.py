...
class AbstractDataStorage(ABC):
    ...
    def update_dataset_row_checksum(
        self, row_id: str, checksum: str, dataset_name: str, dataset_version: int = None
    ) -> None:
        """Updates checksum of specific dataset row in database"""
        dataset = self.get_dataset(dataset_name)
        dr = self.dataset_rows(dataset.id, dataset_version)
        self.execute(
            dr.update()
            .where(dr.c.id == row_id)
            .values(checksum=checksum)  # type: ignore [attr-defined]
        )
