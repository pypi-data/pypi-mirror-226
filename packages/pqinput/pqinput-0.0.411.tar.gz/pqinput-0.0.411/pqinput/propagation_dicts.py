...
def diagonal_sum(n, Opes, label=None):
    mel = {'head':f'm{n}.{n}', 'name':'Sum', 'heir':Opes}
    if label: mel.update({'label':label})
    return mel

'''Off diagonal terms: interactions'''
def offdiagonal_mel(m,n, file, scale=None, laser=None, head=None): # file MgH/mu/mu_Sig0Sig1:: relate to V
...
def offdiagonal_sum(m,n, Opes, label=None): 
