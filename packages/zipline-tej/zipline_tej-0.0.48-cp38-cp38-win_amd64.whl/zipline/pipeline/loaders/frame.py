...
class DataFrameLoader(implements(PipelineLoader)):
    ...
    def load_adjusted_array(self, domain, columns, dates, sids, mask):
        ...
        # 20230729 (by MRC)
        # load_adjusted_array is called with dates on which the user's algo will be shown data,
        # which means we need to return the data that would be known at the **start** of each date.
        # We assume that the latest data known on day N is the data from day (N - 1), so we shift all
        # query dates back by a trading session.
        #        
        # 1.DataFrameLoader的load_adjusted_array原先不會往前平移一期，day N最近已知的資料就是day N當天資料。
        # 2.EquityPricingLoader.load_adjusted_array會往前平移一期，day N最近已知的資料就是day N-1當天資料。
        # 為了一致性這邊也修改為往前平移一期。
        # 3.原先設計沒有平移可能是因為DataFrameLoader提供使用者自行匯入資料，並不會知道使用者提供的資料是在
        # 開盤前就能取得還是開盤後才能取得的。
        # TODO：有更好的改法?
        
        # data = self.baseline[ix_(date_indexer, assets_indexer)]  #20230729 (by MRC)
        data = self.baseline[ix_(date_indexer-1, assets_indexer)]  #20230729 (by MRC)
