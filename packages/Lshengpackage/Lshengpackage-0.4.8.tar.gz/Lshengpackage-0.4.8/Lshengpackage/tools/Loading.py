...
def load(fol_path, target_path, _system, int_x=None, int_y=None, width=0, high=None,
    ...
    while True:
        sleep(0.1)
        iocn = find_image(fol_path, target_path, _system, int_x, int_y, width, high, screenshot,device)
        if iocn is not None:
            sleep(0.1)
            return iocn
...
def load_click(fol_path, target_path, _system, int_x=None, int_y=None, width=0, high=None,
    ...
    """
    sleep(0.1)
    iocn = load(fol_path, target_path, _system, int_x, int_y, width, high, screenshot,device)
    # print(iocn)
    if iocn is not None:
        if _system == 'adb':
            sleep(0.1)
            command().tap_work(int(iocn[0]), int(iocn[1]),device)
        else:
            sleep(0.1)
