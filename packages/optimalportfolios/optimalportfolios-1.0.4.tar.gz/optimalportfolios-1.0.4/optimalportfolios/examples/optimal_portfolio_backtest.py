...
# implementation of rolling portfolios
from optimalportfolios.optimization.qp_solvers import PortfolioObjective
import optimalportfolios.optimization.rolling_portfolios as rlp
...
# 2. compute rolling portfolio weights rebalanced every quarter
...
# 3. given portfolio weights, construct the performance of the portfolio
...
time_period = qis.TimePeriod('31Dec2005', '17Aug2023')  # time period for portfolio reporting

# 4. using portfolio_data run the reporting with strategy factsheet
...
# 5. can create customised reporting using portfolio_data costumfunction
def run_customised_reporting(portfolio_data):
    with sns.axes_style("darkgrid"):
        fig, axs = plt.subplots(3, 1, figsize=(12, 12), tight_layout=True)
    kwargs = dict(x_date_freq='A', frame_alpha=0.75)
...
run_customised_reporting(portfolio_data)
plt.show()
