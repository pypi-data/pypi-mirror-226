...
class AliceBlue(Broker):
    def __init__(self, order_updates=None):
        super(AliceBlue, self).__init__()
        self.order_updates = order_updates

        self.alice = Aliceblue(
            user_id="998325",
            api_key="tDQLs4p96T3sT543QHRD1aj7xx3F9ph6GcYOr84gaDl4jDFZpkgb0IuMGj22qyq5gSVm0MH20VExR17PGKxuLLAcU59KyzpJjkAgTqtk1PC5NMYqVTOh3jZP7YPKDGUZ",
        )
    ...
    def orders(self, tag=None, status=None):
        orders = pd.DataFrame(self.alice.order_data())
        ...
        orders.status = orders.status.replace(
            ["TRIGGER_PENDING", "rejected"], ["TRIGGER PENDING", OrderStatus.rejected]
        )
