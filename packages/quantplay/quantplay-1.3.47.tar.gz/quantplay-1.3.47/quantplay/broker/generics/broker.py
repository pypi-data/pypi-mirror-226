...
class Broker:
    ...
    def convert_to_event(self, order):
        ...
        order["tradingsymbol"] = self.get_symbol(order["tradingsymbol"])
                ...
                if len(orders) > 0:
                    ...
                    for order in orders:
                        ...
                        if order_id not in self.order_log:
                            ...
                            last_log_time = self.order_log[order_id]["update_timestamp"]
    ...
    def stream_order_data(self):
        th = threading.Thread(target=self.stream_order_updates)
        th.start()
