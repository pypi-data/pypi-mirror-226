...
class ACLUser(AbeUser):
    ...
    def add_commitments(self, old_commit: PedersenCommitment, new_commit: PedersenCommitment, new_rand: PedersenRandom) -> PedersenCommitment:
        """Add two Pedersen commitments. Updating randomness stored with the user."""
        # bc_param: BlindedPedersenParam = self.public.bc_param
        self.prand += new_rand
        self.pcommit += new_commit
        # self.pcommit, self.prand = bc_param.add_commitments(old_commit, new_commit, old_rand)
        self.user_attr_commitment = self.pcommit
        return self.pcommit
