...
struct CallbackDrain<D: Drain> {
    inner: D,
    before_hook: RefCell<Box<dyn FnMut() + Send + Sync>>,
    after_hook: RefCell<Box<dyn FnMut() + Send + Sync>>,
}

impl<D: Drain> CallbackDrain<D> {
    fn new(
        inner: D,
        before_hook: impl FnMut() + Send + Sync + 'static,
        after_hook: impl FnMut() + Send + Sync + 'static,
    ) -> Self {
        CallbackDrain {
            inner,
            before_hook: RefCell::new(Box::new(before_hook)),
            after_hook: RefCell::new(Box::new(after_hook)),
        }
    }
}
...
impl<D: Drain> Drain for CallbackDrain<D> {
    ...
    fn log(
        ...
        (&mut *self.before_hook.borrow_mut())();
        let res = self.inner.log(record, values);
        (&mut *self.after_hook.borrow_mut())();
        res
    }
}
