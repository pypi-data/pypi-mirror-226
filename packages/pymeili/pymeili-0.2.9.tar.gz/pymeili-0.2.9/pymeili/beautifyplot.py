...
try:
    ...
    try: # assert if font files are in the directory
        ...
        try: # clone github respository "https://github.com/VVVICTORZHOU/resources.git" into motherpath
            import os
            # go to motherpath
            os.chdir(motherpath)
    ...
    # colorbar顯示
    def colorbar(label=' ', orientation='vertical',shrink=0.95, aspect=10, fraction=0.046, pad=0.04, labelfontsize=16, font=fontpath, color='fg', extend='neither',**kwargs):
        ...
        CB.ax.yaxis.label.set_font_properties(fm.FontProperties(fname = font, size = 20))
        CB.outline.set_linewidth(2)
        for l in CB.ax.yaxis.get_ticklabels():
            l.set_fontproperties(fm.FontProperties(fname = font, size = 15))
    ...
    class initsubplots():
        ...
        def contourf(self, x, y, z, levels=10, cmap='28', contour= True, clabel=True,linewidths=1.5, color='fg', vmin='auto', vmax='auto', **kwargs):
            ...
            if contour == True:
                if self.ndim == 1:
                    ...
                    if clabel == True:
                        CL = CT.clabel(fontsize=8*fontscale, colors=colorlist(color), inline=True)
        ...
        # 繪製colorbar
        def colorbar(self,label=' ', orientation='vertical',shrink=0.95, aspect=10, labelfontsize=16*fontscale, font=fontpath, extend='neither',color='fg',**kwargs):
