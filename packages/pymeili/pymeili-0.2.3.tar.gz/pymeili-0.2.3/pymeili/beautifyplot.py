...
#! UNSOLVE: subplots.colorbar() not working
...
def redirectfontfolder(path):
    ...
    try:
        ...
        # print try to redirect font folder, but we recommend you to install Futura fonts to {motherpath} instead.
        print(bcolors.WARNING+f'Try to redirect font folder, but we recommend you to install Futura fonts to {motherpath} instead.'+bcolors.ENDC)
...
class initsubplots():
    ...
    def contour(self, x, y, z, colors='fg', levels=10, linewidths=2, clabel=True, fontsize=12*fontscale, color='fg', **kwargs):
        if self.ndim == 1:
            ...
            self.levels = levels
    # 繪製等值線填色圖
    def contourf(self, x, y, z, levels=10, cmap='28', contour=True, clabel=True,linewidths=1.5, color='fg', vmin='auto', vmax='auto', **kwargs):
        import numpy as np
        # 傳出Z值和levels值
        global subcontour_vmin, subcontour_vmax, subcontour_levels, CS
        ...
        if contour == True:
            if self.ndim == 1:
                CS = self.ax[self.row].contour(x, y, z, colors=colorlist(color), levels=levels, linewidths=linewidths)
            else:
        ...
        if self.ndim == 1:
            self.ax[self.row].contourf(x, y, z, cmap=cmaplist(cmap), vmin=vmin, vmax=vmax, **kwargs)
        else:
    ...
    def colorbar(self, ticks='auto',label=' ', orientation='vertical',shrink=0.95, aspect=20, labelfontsize=16*fontscale, font=fontpath, extend='neither',color='fg',**kwargs):
        if ticks == 'auto':
            ...
            if self.ndim == 1:
                CS.set_clim(subcontour_vmin, subcontour_vmax)
            else:
                CS.set_clim(subcontour_vmin, subcontour_vmax)
            extend = 'both'
        else:
        ...
        if self.ndim == 1:
            CB = self.ax[self.row].colorbar(orientation=orientation, shrink=shrink, aspect=aspect, label=label, extend=extend, **kwargs)
            CB.ax[self.row].tick_params(labelsize=labelfontsize, labelcolor=colorlist(color), color=colorlist(color))
            CB.ax[self.row].set_ylabel(label, fontproperties=fm.FontProperties(fname=font, size=labelfontsize), color=colorlist(color))
            CB.ax[self.row].set_yticks(ticks,ticks, fontproperties=fm.FontProperties(fname=font, size=labelfontsize))
            CB.ax[self.row].yaxis.set_tick_params(color=colorlist(color), labelcolor=colorlist(color))
            CB.outline.set_color(colorlist(color))
            CB.outline.set_linewidth(2)
        else:
