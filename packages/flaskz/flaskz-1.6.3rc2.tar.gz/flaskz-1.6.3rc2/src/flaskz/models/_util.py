...
def get_db_session():
    ...
    if _has_g_context():  # @2022-07-26 add, make sure work without flask request
        ...
        if session is None:
            session = DBSession()
            setattr(session, '_auto_close', False)
...
def close_db_session():
    """
    Close the cached session in the g.
...
def _auto_close_session(session):
    """
    @2023-08-21: add
    :param session:
    :return:
    """
    # if getattr(session, '_flaskz_db_session', None) is not True:
    if getattr(session, '_auto_close', None) is not False:  # @2023-05-06: add, close non-cached session
        session.close()
