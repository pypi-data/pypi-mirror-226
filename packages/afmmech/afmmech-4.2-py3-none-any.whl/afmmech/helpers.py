...
class dataset:
    ...
    def overwrite_base_format(self):
        self.base_data = copy.copy(self.data)
    def update(self, key, value, add_none_values=False):
        if value is None and not add_none_values:
            return None
        self.data.update({key: value})
    ...
    def process(self, key, func, new_key_name=None, add_none_values=True, *args, **kwargs):
        def safe_func(element, *args, **kwargs):
            try:
                return func(element, *args, **kwargs)
            except Exception as e:
                print(f"An error occurred with element {element}: {e}")
        ...
        if isinstance(key, list):
            ...
            for k, new_name in zip(key, new_key_name):
                self.process(k, safe_func, new_key_name=new_name, add_none_values=add_none_values, *args, **kwargs)
        else:
            if new_key_name is not None:
                self.update(new_key_name, [safe_func(element, *args, **kwargs) for element in tqdm(self.data[key])], add_none_values=add_none_values)
            else:
                self.data[key] = [safe_func(element, *args, **kwargs) for element in tqdm(self.data[key])]
                
    def parallel_process(self, key, func, new_key_name=None, add_none_values=True, n_jobs=-1, *args, **kwargs):
        def safe_func(element, *args, **kwargs):
            try:
                return func(element, *args, **kwargs)
            except Exception as e:
        ...
        if isinstance(key, list):
            if new_key_name is None:
                ...
                new_key_name = [new_key_name]
            for k, new_name in zip(key, new_key_name):
                self.parallel_process(k, safe_func, new_key_name=new_name, add_none_values=add_none_values, n_jobs=n_jobs, *args, **kwargs)
        else:
            if new_key_name is not None:
                self.update(new_key_name, joblib.Parallel(n_jobs=n_jobs)(joblib.delayed(safe_func)(element, *args, **kwargs) for element in tqdm(self.data[key])), add_none_values=add_none_values)
            else:
                self.data[key] = joblib.Parallel(n_jobs=n_jobs)(joblib.delayed(safe_func)(element, *args, **kwargs) for element in tqdm(self.data[key]))
...
class dataset_dict(dataset):
    ...
    def process(self, column, func, new_column_name=None, add_none_values=True, *args, **kwargs):
        if isinstance(column, list):
            ...
            for c, new_name in zip(column, new_column_name):
                self.process(c, func, new_column_name=new_name, add_none_values=add_none_values, *args, **kwargs)
        else:
            for d in self.data.values():
                d.process(column, func, new_column_name, add_none_values=add_none_values, *args, **kwargs)
    def parallel_process(self, column, func, new_column_name=None, add_none_values=True, n_jobs=-1, *args, **kwargs):
        if isinstance(column, list):
            if new_column_name is None:
                new_column_name = [None] * len(column)
            elif not isinstance(new_column_name, list):
                new_column_name = [new_column_name]
            for c, new_name in zip(column, new_column_name):
                self.parallel_process(c, func, new_column_name=new_name, add_none_values=add_none_values, n_jobs=n_jobs, *args, **kwargs)
        for d in self.data.values():
            d.parallel_process(column, func, new_column_name, add_none_values=add_none_values, n_jobs=n_jobs, *args, **kwargs)
...
def parallel_load_ibw_data(folder, n_jobs=-1):
    return joblib.Parallel(n_jobs=n_jobs)(joblib.delayed(ibw2df)(file) for file in get_files(folder))