...
class CertoraContextValidator:
    ...
    def _get_key_and_value(self, attr: Attr.ContextAttribute) -> Tuple[str, Any]:
    ...
    def validate_type_list_of_strings(self, attr: Attr.ContextAttribute) -> None:
        conf_key, value = self._get_key_and_value(attr)
...
def check_contract_name_arg_inputs(context: CertoraContext) -> None:
    ...
    if context.link is not None:
        for link in context.link:
            ...
            if executable not in contract_names:
                _suggest_contract_name(f"link {link} doesn't match any contract name", executable, contract_names,
                                       contract_to_file)
            ...
            try:
                ...
                if library_name not in contract_names:
                    _suggest_contract_name(f"{library_name} in link {link} doesn't match any contract name",
    ...
    if context.assert_contracts is not None:
        for assert_arg in context.assert_contracts:
            ...
            if contract not in contract_names:
                _suggest_contract_name(f"--assert argument {contract} doesn't match any contract name", contract,
    ...
    if context.verify is not None:
        ...
        if contract not in contract_names:
            _suggest_contract_name(f"--verify argument {contract} doesn't match any contract name", contract,
                                   contract_names, contract_to_file)
        spec_files.add(spec)
        context.verified_contract_files.append(contract_to_file[contract])
    ...
    if context.address:
        for address_str in context.address:
            ...
            if contract not in contract_names:
                _suggest_contract_name(f"unrecognized contract in --address argument {address_str}", contract,
                                       contract_names, contract_to_file)
...
def check_args_post_argparse(context: CertoraContext) -> None:
    ...
    if Util.is_new_api():
        if context.solc_allow_path is None:
            context.solc_allow_path = str(_default_path())
    else:
        if context.path is None:
            context.path = str(_default_path())
...
def _suggest_contract_name(err_msg: str, contract_name: str, all_contract_names: Set[str],
