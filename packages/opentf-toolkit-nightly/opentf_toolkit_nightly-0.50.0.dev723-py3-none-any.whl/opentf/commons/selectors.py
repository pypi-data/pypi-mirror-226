...
INSET_EXPR = re.compile(
    rf'^({KEY})\s+(in|notin)\s+\(({VALUE}(\s*,\s*{VALUE})*)\)(?:,|$)'
)
SETIN_EXPR = re.compile(
    rf'^\(({VALUE}(\s*,\s*{VALUE})*)\)\s+(in|notin)\s+({KEY})(?:,|$)'
)
...
def compile_selector(exprs: str, resolve_path: bool = True) -> List[OpCode]:
    ...
    if match := EQUAL_EXPR.match(exprs):
        ...
        instr = _opcode(
            OP_INSET, key, ope == 'notin', {v.strip() for v in vals.split(',')}
        )
    elif match := SETIN_EXPR.match(exprs):
        vals, _, ope, key = match.groups()
        instr = _opcode(
            OP_SETIN, key, ope == 'notin', {v.strip() for v in vals.split(',')}
        )
    else:
