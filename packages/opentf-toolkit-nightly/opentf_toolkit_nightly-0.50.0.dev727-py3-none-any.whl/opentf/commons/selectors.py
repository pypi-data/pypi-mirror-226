...
KEY = r'[a-z0-9A-Z-_./]+'
VALUE = r'[a-z0-9A-Z-_./@]+'
EQUAL_EXPR = re.compile(rf'^({KEY})(\[\'{VALUE}\'\])?\s*([=!]?=)\s*({VALUE})(?:,|$)')
INSET_EXPR = re.compile(
    rf'^({KEY})(\[\'{VALUE}\'\])?\s+(in|notin)\s+\(({VALUE}(\s*,\s*{VALUE})*)\)(?:,|$)'
)
SETIN_EXPR = re.compile(
    rf'^\(({VALUE}(\s*,\s*{VALUE})*)\)\s+(in|notin)\s+({KEY})(\[\'{VALUE}\'\])?(?:,|$)'
)
EXISTS_EXPR = re.compile(rf'^({KEY})(\[\'{VALUE}\'\])?(?:,|$)')
NEXISTS_EXPR = re.compile(rf'^!({KEY})(\[\'{VALUE}\'\])?(?:,|$)')
...
def compile_selector(exprs: str, resolve_path: bool = True) -> List[OpCode]:
    ...
    def _opcode(code, item, third=None, fourth=None, fifth=None):
        if not resolve_path and fifth:
            raise ValueError(
                f'Invalid expression {exprs}: [] not allowed in label selectors.'
            )
        if resolve_path and (fifth or '.' in item):
            return (
                code | OP_RESOLV,
                item.split('.') + ([fifth.strip('[]\'')] if fifth else []),
                third,
                fourth,
            )
    ...
    if match := EQUAL_EXPR.match(exprs):
        key, xtra, ope, value = match.groups()
        instr = _opcode(OP_EQUAL, key, ope == '!=', value, xtra)
    elif match := EXISTS_EXPR.match(exprs):
        instr = _opcode(OP_EXIST, match.group(1), fifth=match.group(2))
    elif match := NEXISTS_EXPR.match(exprs):
        instr = _opcode(OP_NEXIST, match.group(1), fifth=match.group(2))
    elif match := INSET_EXPR.match(exprs):
        key, xtra, ope, vals, _ = match.groups()
        instr = _opcode(
            OP_INSET, key, ope == 'notin', {v.strip() for v in vals.split(',')}, xtra
        )
    elif match := SETIN_EXPR.match(exprs):
        vals, _, ope, key, xtra = match.groups()
        instr = _opcode(
            OP_SETIN, key, ope == 'notin', {v.strip() for v in vals.split(',')}, xtra
        )
    else:
...
def _evaluate(obj: Object, req: OpCode) -> bool:
    ...
    if found and opcode & OP_SETIN:
        return (set(value) >= arg) ^ neq  # type: ignore
