...
from mdata.core import MachineData, MDConcepts, as_base
...
@overload
def compute_measurement_timedeltas(md: MachineData, per_object=False) -> dict[str, pd.Series]:
    ...


@overload
def compute_measurement_timedeltas(md: MachineData, per_object=True) -> dict[str, dict[str, pd.Series]]:
    ...


def compute_measurement_timedeltas(md: MachineData, per_object=False):
    md = as_base(md)
    res = {}
    for m, tsc in md.measurement_series.items():
        if per_object:
            res[m] = {o: tsc.view(o).df[MDConcepts.Time].diff() for o in tsc.objects}
        else:
            res[m] = tsc.df[MDConcepts.Time].diff()
    return res


def estimate_measurement_frequencies(md: MachineData) -> dict[MeasurementSpecLabel, Hz]:
    res = {}
...
def estimate_measurement_deadzones(md: MachineData, measurement_specs: Set[MeasurementSpecLabel]):
    md = as_base(md)
    res = {}
    for m, deltas in compute_measurement_timedeltas(md).items():
