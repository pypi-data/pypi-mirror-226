...
class LoggingManager():
    ...
    * Checking whether we are in debug mode via LoggingManager().is_debugging
    ...
    def _remove_handler(self, handler: logging.Handler) -> None:
        """
    ...
    def set_log_level_and_format(
        ...
        @param show_debug_topics: Ignored if either quiet is True or debug is None. If True, sets the logging message
                                  format to show the topic of the logger that sent them.
        """
        self._format_stdout_log_messages(show_debug_topics)
        self._set_logging_level(is_quiet, debug, debug_topics)

    def remove_debug_logger(self) -> None:
        self._remove_handler(self.debug_log_handler)

    def _format_stdout_log_messages(self, show_debug_topics: bool) -> None:
        """
    ...
    def _set_logging_level(self, is_quiet: bool, debug: bool = False, debug_topics: Optional[List[str]] = None) \
            -> None:
        """
        ...
        if is_quiet or not debug:
            self._set_handlers_level(logging.WARNING)
            self.is_debugging = False
        else:
            self._set_handlers_level(logging.DEBUG)
            self.is_debugging = True

        self._set_topics_filter(debug_topics)
    ...
    def _set_topics_filter(self, debug_topics: Optional[List[str]] = None) -> None:
        """
