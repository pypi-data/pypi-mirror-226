...
class CompilerCollectorFactory:
    ...
    def get_compiler_collector(self, path: Path) -> CompilerCollector:
        ...
        if str(path).endswith(".vy"):
            version = self._get_vyper_version(path)
    ...
    def get_vyper_compiler(contract_file_path: Path, default_compiler: str, compiler_mappings: Dict[str, str]) -> str:
        ...
        vyper_exec = get_relevant_compiler(contract_file_path, compiler,
                                                 compiler_mappings)
        return vyper_exec

    def _get_vyper_version(self, contract_file_path: Path) -> CompilerVersion:
        """
        @param contract_file_path: the contract that we are working on
        ...
        version = self._get_compiler_exe_version(vyper_exec_to_run, self._version_string_handler_vyper)
        return version

    def _get_solc_version(self, contract_file_path: Path) -> CompilerVersion:
        """
        @param contract_file_path: the contract that we are working on
        ...
        version = self._get_compiler_exe_version(solc_path, self._version_string_handler_solc)
        return version

    @lru_cache(maxsize=32)
    def _get_compiler_exe_version(self, compiler_name: str,
    ...
    @staticmethod
    def _version_string_handler_vyper(version_string: str) -> CompilerVersion:
    ...
    @staticmethod
    def _version_string_handler_solc(version_string: str) -> CompilerVersion:
