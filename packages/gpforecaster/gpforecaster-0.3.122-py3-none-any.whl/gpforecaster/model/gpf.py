    ...
    def _preprocess(
        ...
        if self.scale_x_values:
            ...
            for key in ["train", "predict"]:
                if isinstance(groups[key]["x_values"], np.ndarray):
                    tensor = torch.tensor(groups[key]["x_values"], dtype=torch.float32)
                elif isinstance(groups[key]["x_values"], torch.Tensor):
                    tensor = groups[key]["x_values"].to(torch.float32)
                else:
                    raise TypeError("Unsupported data type in groups['train']['x_values']")
