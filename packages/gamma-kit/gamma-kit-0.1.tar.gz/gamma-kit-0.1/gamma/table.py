...
class Table:
    ...
    def _construct_where_clause(self, **kwargs):
        ...
        clause = "WHERE " + " AND ".join([f"{key} = %s" for key in kwargs.keys()])
    ...
    def find_many(self, offset=None, count=None, **kwargs):
        ...
        query = f"SELECT * FROM {self.table} {where_clause} {limit_clause} {offset_clause}"
