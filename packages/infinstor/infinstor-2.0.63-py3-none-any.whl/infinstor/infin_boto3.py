...
class InfinBotoClient():
    ...
    def get_putobject_presigned_url(self, bucket:str, prefix, **kwargs):
        attempt = 0
        while attempt < 2:
            if attempt == 0:
                force = False
    ...
    def upload_file(self, *args, **kwargs):
        ...
        # TODO multipart: what about 'concurrent user buckets' (concurrent-storgage-credentials) and 'mlflow user buckets' (infinstor-Credentials) ?
        if self.timespec_info['use_presigned_url_for_infinsnap'] \
            ...
            if use_put_object_presigned:
                ...
                try:
                    with open(request_file, "rb") as fp:
                        ...
                        create_mult_upload_resp = self._invoke_s3meta_rest(request_bucket, request_object, 'create_multipart_upload', num_of_chunks=num_of_chunks, **kwargs)
                        ...
                        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                            ...
                            for chunk_num in range(1, num_of_chunks+1):
                                ...
                                requests_future:concurrent.futures.Future = executor.submit(requests.put, create_mult_upload_ps_info.ps_url, data=fp.read(this_chunk_size), headers=headers)
                        ...
                        self._invoke_s3meta_rest(request_bucket, request_object, 'complete_multipart_upload', comp_multi_upload=comp_multi_part_upload_arg)
                except Exception as e:
                    ...
                    if create_mult_upload_resp:
                        ...
                        # **kwargs not passed since kwargs['ExtraArgs']['Metadata] is not relevant for abort_multipart_upload().  only relevant for s3_client.generate_presigned_url('create_multipart_upload|upload_part', ...)
                        resp:str = self._invoke_s3meta_rest(request_bucket, request_object, 'abort_multipart_upload', upload_id=create_mult_upload_resp.upload_id)
                        logger.info(f"self._invoke_s3meta_rest(method=abort_multipart_upload..)={resp}")
    ...
    def _invoke_s3meta_rest(self, bucket:str, prefix:str, op_s3meta:str, /, num_of_chunks:int = None, 
                          comp_multi_upload:CompleteMultiPartUploadReq = None, upload_id:str=None, **kwargs):
        """
        invokes the API /api/2.0/mlflow/infinstor/s3meta for the specified bucket, prefix, s3meta operation (op_s3meta).  Returns the json response (as dict)
        ...
        while attempt < 2:
            if attempt == 0:
                ...
                logger.info('_invoke_s3meta_rest: attempt = ' + str(attempt) + ', force True')
                force = True
            attempt = attempt + 1
            token = tokenfile.get_token(builtins.region, force)
            headers = {
            ...
            return js
        logger.error('_invoke_s3meta_rest() failed: Tried twice. Giving up')
        return None
