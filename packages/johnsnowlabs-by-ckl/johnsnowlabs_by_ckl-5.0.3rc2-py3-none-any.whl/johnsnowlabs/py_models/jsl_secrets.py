...
class JslSecrets(WritableBaseModel):
    ...
    def build_or_try_find_secrets(
        ...
        try:
            ...
            if any(
                ...
                print("🔑 Using credentials from kwargs")
                secrets = JslSecrets(
                    HC_SECRET=hc_secret,
                ...
                print("🔑 Using credentials access token")
            ...
            #     found_secrets = JslSecrets.from_email_and_pass(email, passw,local_license_number)
                ...
                print(f"🔑 Using credentials from secret JSON file at {secrets_file}")
                ...
                print(f"🔑 Using credentials from json file {key}")
    ...
    def search_env_vars() -> Union["JslSecrets", bool]:
        ...
        if any(
            ...
            print(f"🔑 Using credentials from Env variables - Generic Check")
            return JslSecrets(
                HC_SECRET=hc_secret,
                HC_LICENSE=hc_license,
            ...
            if "JOHNSNOWLABS_LICENSE_JSON" in os.environ:
                ...
                if JslSecrets.dict_has_jsl_secrets(json_dict):
                    print(
                        f"🔑 Using credentials json file {os.environ['JOHNSNOWLABS_LICENSE_JSON']}"
                    )
        ...
        if os.path.exists("/content"):
            ...
            for f_path in j_files:
                try:
                    ...
                    if JslSecrets.dict_has_jsl_secrets(json_dict):
                        print(f"🔑 Using credentials from secret {f_path}")
    ...
    def from_jsl_home(
        ...
        try:
            ...
            if log and not already_logged:
                already_logged = True
                print(
                    f"🔑 Using cached credentials number {license_number} from {settings.license_dir}/{list(license_infos.infos.keys())[license_number]}"
                )
        except:
