...
# %% auto 0
__all__ = ['PLOTLY_KEY', 'PLAIN_TEXT_KEY', 'TEMPLATE', 'get_custom_code', 'PlotlyProc']
...
def get_custom_code(data):
    return TEMPLATE.replace('{id}', str(uuid4())).replace('{data}', json.dumps(data))
            ...
            for k in output:
                if 'data' in k:
                    ...
                    if PLOTLY_KEY in data:
                        data[PLAIN_TEXT_KEY] = [get_custom_code(data[PLOTLY_KEY])]
