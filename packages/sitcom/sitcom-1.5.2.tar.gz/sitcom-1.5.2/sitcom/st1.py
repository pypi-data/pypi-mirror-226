...
class Ui_SecondWindow(object):
    ...
    def DTP(self):
      ...
      xp=self.ax.imshow(self.xt,cmap=self.colorm,aspect='auto',vmin=vmin,vmax=vmax)
    ...
    def DTP_save(self):
      ...
      fpath1=os.path.join(fp,self.date,"XT_MAP")
      os.makedirs(fpath1,mode=0o777,exist_ok=True)
      if self.comboBox_savext.currentText()=='fits':
        im = fits.PrimaryHDU(self.xt)
        cnm=os.path.join(fpath1,'XT.fits')
        im.writeto(cnm,overwrite=True)
      elif self.comboBox_savext.currentText()=='sav':
        cnm=os.path.join(fpath1,'XT.npy')
        np.save(cnm, self.xt)
        img_from_file = np.load(cnm)
        cnm1=os.path.join(fpath1,'XT.sav')
        np.savetxt(cnm1, img_from_file)
        os.remove(cnm)
      msg = QtWidgets.QMessageBox()
      msg.setStyleSheet("color:'black';")
      msg.setIcon(QtWidgets.QMessageBox.Information)
      msg.setText("The XT map is saved in "+fpath1)
      msg.setWindowTitle("Saved!")
      msg.setStandardButtons(QtWidgets.QMessageBox.Ok)
      retval = msg.exec_()
