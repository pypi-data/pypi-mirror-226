...
from PyQt5.QtCore import Qt, QPointF,  QRectF,  QSizeF, QObject, pyqtSignal
...
class GraphicsPoint(QGraphicsEllipseItem, GraphicsModelItem):  
    ...
    _highlight_style = {SIZE: 50,
                  FACE_COLOR: [0, 255, 0, 128],
                  LINE_COLOR: [0, 0, 0],
                  LINE_WIDTH: 1,
                  ZVALUE: 10}
    
    _select_style = {SIZE: 50,
                  FACE_COLOR: [0, 255, 0, 128],
                  LINE_COLOR: [0, 0, 0],
                  LINE_WIDTH: 1,
                  ZVALUE: 20}
    
    _disabled_style = {SIZE: 10,
                       FACE_COLOR: [83, 83, 83],
                       LINE_COLOR: [0, 0, 0],
                       LINE_WIDTH:2,
                       ZVALUE: 5}
...
class GraphicsSourceNM(GraphicsPoint):
    ...
    _highlight_style = {SIZE: 20,
                  FACE_COLOR: [139, 0, 0, 128],
                  LINE_COLOR: [0, 0, 0],
                  LINE_WIDTH: 2,
                  ZVALUE: 10}
    
    _select_style = {SIZE: 20,
                  FACE_COLOR: [139, 0, 0, 128],
                  LINE_COLOR: [0, 0, 0],
                  LINE_WIDTH: 2,
                  ZVALUE: 20}
...
class Marker(QGraphicsRectItem, GraphicsItem):
    ...
    _highlight_style = {WIDTH: 20,
                        HEIGHT: 20,
                        LINE_WIDTH: 2,
                        ZVALUE: 10}
    
    _select_style = {WIDTH: 20,
                     HEIGHT: 20,
                     LINE_WIDTH: 2,
                     ZVALUE: 20}
...
class GraphicsWall(GraphicsLine):   
    ...
    def __init__(self, model=None, pixel_size_cm=1):
        self.model = model
        self.shieldings().styleChanged.connect(self.updateStyle)
        self.model.parent().shieldingChanged.connect(self.updateStyle)
        super().__init__(model=model, pixel_size_cm=pixel_size_cm)
               
        
        
    def _pen(self):
        pen = super()._pen()
        if self.shielding().name == labels.EMPTY_SHIELDING:
    ...
    def linecolor(self):
        color = self.shielding().color
        if isinstance(color, str):
            color = QColor(self.shielding().color)
        elif isinstance(color, (tuple, list)):
            color =  QColor(*self.shielding().color)
        return color
    
    def linewidth(self):
        width = self.pointsToSceneUnits(self.shielding().linewidth) / self.zoomFactor()
    ...
    def shielding(self):       
        for item in self.shieldings().items:                      
            if item.name == self.model.shielding:
                return item
        return self.shieldings()[0]
...
class RulerMarker(Marker):
    ...
    _highlight_style = {WIDTH: 6,
                        HEIGHT: 20,
                        LINE_WIDTH: 0,
                        ZVALUE: 10}
    
    _select_style = {WIDTH: 6,
                     HEIGHT: 20,
                     LINE_WIDTH: 0,
                     ZVALUE: 20}
