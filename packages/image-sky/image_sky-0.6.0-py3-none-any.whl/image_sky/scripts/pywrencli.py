...
import click
import image_sky
import image_sky.runtime
from image_sky import ec2standalone
import oss2
from ..fun_client import fun_client
...
@click.group()
@click.option('--filename', default=image_sky.wrenconfig.get_default_config_filename())
@click.pass_context
...
# FIXME use image_sky main module SOURCE_DIR
...
def get_aws_account_id():
    ...
    """
    config = os.path.join(os.path.expanduser("~/.ossutilconfig"))
    ...
    for l in lines:
        splits = l.split("=")
        if splits[0] == "accessKeyID":
            accessKeyID = splits[1]
        if splits[0] == "accessKeySecret":
            accessKeySecret = splits[1]
        if splits[0] == "endpoint":
            endpoint = splits[1]
...
@click.pass_context
@click.option('--aws_region', default=image_sky.wrenconfig.AWS_REGION_DEFAULT,
              help='aws region to run in')
@click.option('--bucket_name', default=image_sky.wrenconfig.AWS_S3_BUCKET_DEFAULT,
              help='s3 bucket name for intermediates')
@click.option('--bucket_prefix', default=image_sky.wrenconfig.AWS_S3_PREFIX_DEFAULT,
              help='prefix for S3 keys used for input and output')
@click.option('--lambda_role',
              default=image_sky.wrenconfig.AWS_LAMBDA_ROLE_DEFAULT,
              help='name of the IAM role we are creating')
@click.option('--function_name',
              default=image_sky.wrenconfig.AWS_LAMBDA_FUNCTION_NAME_DEFAULT,
              help='lambda function name')
@click.option('--sqs_queue', default=image_sky.wrenconfig.AWS_SQS_QUEUE_DEFAULT,
              help='sqs queue name for standalone execution')
@click.option('--standalone_name', default='image_sky-standalone',
              help='ec2 standalone server name and profile name')
@click.option('--force', is_flag=True, default=False,
              help='force overwrite an existing file')
@click.option('--pythonver', default=image_sky.runtime.version_str(sys.version_info),
              help="Python version to use for runtime")
def create_config(ctx, service_name, force, aws_region, lambda_role, function_name, bucket_name,
                  bucket_prefix, accessKeyID, accessKeySecret,endpoint,user_id,
    ...
    default_yaml = default_yaml.replace('pywren_service_name', service_name)
    default_yaml = default_yaml.replace('BUCKET_NAME', bucket_name)
    default_yaml = default_yaml.replace('ACCESS_KEY_ID', accessKeyID)
    default_yaml = default_yaml.replace('ACCESS_KEY_SECRET', accessKeySecret)
    default_yaml = default_yaml.replace('ENDPOINT', endpoint)
    default_yaml = default_yaml.replace('USER_ID', user_id)
    default_yaml = default_yaml.replace('image_sky.jobs', bucket_prefix)
    default_yaml = default_yaml.replace('image_sky-queue', sqs_queue)
    default_yaml = default_yaml.replace('image_sky-standalone', standalone_name)
    if pythonver not in image_sky.wrenconfig.default_runtime:
        print('No matching runtime package for python version ', pythonver)
        print('Python 2.7 runtime will be used for remote.')
        pythonver = '2.7'

    if aws_region in image_sky.wrenconfig.RUNTIME_BUCKET_REGION:
        runtime_bucket = image_sky.wrenconfig.RUNTIME_BUCKET_REGION[aws_region]
        target_ami = image_sky.wrenconfig.TARGET_AMI_REGION[aws_region]
    else:
        print("WARNING: Runtime not deployed for your region")
        print("using runtime from us-west-2.")
        print("Performance may be impacted")
        runtime_bucket = image_sky.wrenconfig.RUNTIME_BUCKET_REGION['cn-beijing']
        target_ami = image_sky.wrenconfig.TARGET_AMI_REGION['cn-beijing']

    default_yaml = default_yaml.replace("RUNTIME_BUCKET",
                                        runtime_bucket)
    k = image_sky.wrenconfig.default_runtime[pythonver]
...
def test_config(ctx): # pylint: disable=unused-argument
    ...
    # config = image_sky.wrenconfig.default()
...
def create_role(ctx):
    ...
    config = image_sky.wrenconfig.load(config_filename)

    iam = boto3.resource('iam')
    iamclient = boto3.client('iam')
    json_policy = json.dumps(image_sky.wrenconfig.basic_role_policy)
    ...
    if (len(roles) == 0):
        ...
        more_json_policy = json.dumps(image_sky.wrenconfig.more_permissions_policy)
...
def create_bucket(ctx):
    ...
    auth = oss2.Auth(config["account"]["access_key_id"], config["account"]["access_key_secret"])
    # Endpoint以杭州为例，其它Region请按实际情况填写。
    bucket = oss2.Bucket(auth, config['runtime']['endpoint'], config['s3']['bucket'])
    # 设置存储空间为私有读写权限。
...
def get_zip():
    file_like_in = io.BytesIO()
    zipfile_obj = zipfile.ZipFile(file_like_in, mode='w')
    ...
    # with open('input.zip', 'rb') as fin, open('output.zip.b64', 'wb') as fout:
    #     base64.encode(fin, fout)
    #
    # with open('output.zip.b64', 'r') as f:
    #     read = f.read()
    #     return read
    return base64.b64encode(file_like_in.getvalue()).decode()
...
def deploy_lambda(ctx, update_if_exists=True):
    ...
    client = fun_client.create_client(user_id, access_key_id, access_key_secret)

    function_name_list = fun_client.list_functions(client, service_name)
    ...
    while retries < 10:
        try:
            if function_exists:
                ...
                if update_if_exists:
                    fun_client.update_function(client, service_name, function_name, code)
                    return True
                else:
                    raise Exception() # FIXME will this work?
            else:

                fun_client.create_function(client, service_name, function_name, code, handler, runtime, memory, timeout)
...
def test_function(ctx):
    ...
    wrenexec = image_sky.default_executor(config=config)
    def hello_world(_):
        return "Hello world"
