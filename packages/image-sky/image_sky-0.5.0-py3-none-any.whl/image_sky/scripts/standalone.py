...
import image_sky
from image_sky import wrenhandler
...
def process_message(m, local_message_i, max_run_time, run_dir):
    ...
    # id this in a thread: image_sky.wrenhandler.generic_handler(event)
...
def copy_runtime(tgt_dir):
    files = glob(os.path.join(image_sky.SOURCE_DIR, "jobrunner/*.py"))
    for f in files:
...
def job_handler(event, job_i, run_dir,
    ...
    debug_pid = open("/tmp/image_sky.scripts.standalone.{}.{}.log".format(os.getpid(),
                                                                       time.time()), 'w')
...
@click.option('--run_dir', default="/tmp/image_sky.rundir",
              help='directory to hold intermediate output')
@click.option('--aws_region', default="us-west-2",
              help='aws region')
@click.option('--sqs_queue_name', default="image_sky-queue",
              help='queue')
...
def server(aws_region, max_run_time, run_dir, sqs_queue_name, max_idle_time,
    ...
    def async_log_setup():
        ''' None of this stuff should be on the critical path to launching an
            ...
            * This function is called once per image_sky executor process
        '''
        success = False
        ...
        while (not success):
            try:
                ...
                handler = watchtower.CloudWatchLogHandler(send_interval=20,
                                                          log_group="image_sky.standalone",
                ...
                wren_log = image_sky.wrenhandler.logger
