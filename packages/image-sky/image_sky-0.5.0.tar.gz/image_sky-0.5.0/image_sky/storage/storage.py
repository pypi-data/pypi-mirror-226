...
from .OssBackend import OssBackend
...
class Storage(object):
    ...
    def __init__(self, config):
        ...
        if config['storage_backend'] == 's3':
            self.backend_handler = OssBackend(config['backend_config'])
        else:
...
def get_runtime_info(config_info):
    ...
    config['endpoint'] = runtime_config['endpoint']
    config['access_key_id'] = config_info["account"]['access_key_id']
    config['access_key_secret'] = config_info["account"]['access_key_secret']
    handler = OssBackend(config)
