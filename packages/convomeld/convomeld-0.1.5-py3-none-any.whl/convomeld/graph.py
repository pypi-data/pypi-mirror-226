...
from convomeld.matchers import SimpleActionMatcher, SimpleTriggerValueMatcher
...
class ConvoGraph:
    def __init__(
        ...
        for state in self._states.values():
            ...
            new_state_name = self._generate_state_name(seq=state.attrs.get('_seq'), repeat=state.attrs.get('_repeat'))
    ...
    def rename_states(self, mapping) -> None:
        ...
        for state in self._states.values():
            ...
            for trigger in new_states[state_name].triggers:
                if trigger.state_name in mapping:
                    trigger.state_name = mapping[trigger.state_name]
    ...
    def _mark_seq_repeat(self) -> None:
        ...
        while len(state_queue):
            ...
            for trigger in state.triggers:
                next_state = self.find_state(name=trigger.state_name)

                if next_state in visited_states or next_state.name == state.name:
                    continue

                next_trigger = next_state.find_trigger(TriggerPlaceholder.next(), self._trigger_matcher)

                while next_trigger:
                    next_state = self.find_state(name=next_trigger.state_name)
                    next_trigger = next_state.find_trigger(TriggerPlaceholder.next(), self._trigger_matcher)
    ...
    def compare(self, other) -> bool:
        ...
        while len(state_queue):
            ...
            for trigger1 in state1.triggers:
                ...
                state_queue.append(
                    (
                        self._states[trigger1.state_name],
                        other.find_state(name=trigger2.state_name),
                    )
                )
    ...
    def from_yaml_file(
        ...
        return cls.from_states_list(
            script_list, action_matcher, trigger_matcher, use_uuid=use_uuid
        )
