...
def setDFTitle(df,title):
    ...
    return df
def imageArrayPixelToDataFrame(arrayImg,pt,scale=None,crs = None, transform = None,title = None,index = None,columns = None,bandName = None, reducer = ee.Reducer.first()):
    # Pull the values
    vals = arrayImg.reduceRegion(reducer,pt,scale=scale,crs = crs, crsTransform = transform).getInfo()

    # If no band is specified, pull the first band
    if bandName == None:
...
# #     # featureCollection_to_csv(fc,out_csv,overwrite = True,maxNumberOfFeatures=500)
    ...
    df = imageArrayPixelToDataFrame(lt,pt,crs = 'EPSG:5070', scale=30,transform = None,index = ['year','vertex fit'],columns = None,bandName = None, reducer = ee.Reducer.first())
