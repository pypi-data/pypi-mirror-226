...
from molecularprofiles.utils.constants import (
    ...
    RAYLEIGH_SCATTERING_HEIGHT_BINS,
)
...
class MolecularProfile:
    ...
    def _pick_up_reference_atmosphere(self, ceiling, floor, reference_atmosphere):
        ...
        try:
            mask = (
                reference_atmosphere_table["height"]
                >= ceiling.to(reference_atmosphere_table["height"].unit)
            ) | (
                reference_atmosphere_table["height"]
                < floor.to(reference_atmosphere_table["height"].unit)
            )
    ...
    def write_atmospheric_profile(
        ...
        atmospheric_profile_heights=None,
    ):
        """
        Write an output file in the style of a CORSIKA atmospheric configuration file:
        height (km)     density (g/cm^3)   thickness (g/cm^2)   refraction index -1   temperature (K)   pressure (hPA)   partial water pressure
        ...
        if atmospheric_profile_heights is None:
            height = np.arange(floor.to_value(u.m), ceiling.to_value(u.m), 1000) * u.m
        else:
            height = atmospheric_profile_heights
        height = height.to(self.stat_description["avg"]["Altitude"].unit)
        temperature = (
        ...
        for i in np.arange(len(height)):
            outdict = {
                ...
                "atmospheric density": density[i].to(u.g / u.cm**3),
                "atmospheric thickness": thickness[i].decompose().to(u.g / u.cm**2),
                "refraction index - 1": rel_refractive_index[i],
                "temperature": temperature[i],
                "pressure": pressure[i],
                "partial water pressure": rel_water_vapor_pressure[i],
            }
        ...
        corsika_input_table.sort("height")
    ...
    def create_mdp(self, mdp_file):
        ...
        heights = np.arange(0.0, 20000.0, 1000) * u.m
        heights = heights.to(self.stat_description["avg"]["Altitude"].unit)
        number_density = (
            self._interpolate_cubic(
                self.stat_description["avg"]["Altitude"],
                self.stat_description["avg"]["Density"],
    ...
    def rayleigh_extinction(
        ...
        reference_atmosphere=None,
        altitude_list=STD_CORSIKA_ALTITUDE_PROFILE,
        rayleigh_scattering_height_bins=RAYLEIGH_SCATTERING_HEIGHT_BINS,
    ):
        """
        ...
        altitude_list : Quantity
            Tuple with the altitudes that the atmospheric parameters will be calculated. Units of length.
        rayleigh_scattering_height_bins : tuple
            Quantity with the altitudes that the AOD will be calculated. Units of length.

        Returns:
        --------
            Ecsv file with absolute optical depth per height bin per wavelength bin. The data model is the same with MODTRAN files.
        """

        floor = rayleigh_scattering_height_bins[0].to(u.km)
        ceiling = self._take_closest(
            altitude_list.to(self.stat_description["avg"]["Altitude"].unit),
            (self.stat_description["avg"]["Altitude"][0])
            * self.stat_description["avg"]["Altitude"].unit,
        )
        height = rayleigh_scattering_height_bins.to(u.km)
        if ceiling.to_value(u.km) > 7.0:
            height_middle = np.arange(7.0, ceiling.to_value(u.km), 1)
            height = (
                np.concatenate((height.to_value(u.km), height_middle))
            ) * rayleigh_scattering_height_bins.unit

        interpolation_centers = (height[:-1] + height[1:]) / 2
        temperature_lower = (
            self._interpolate_cubic(
        ...
        if reference_atmosphere:
            ...
            try:
                ...
                pressure = np.concatenate((pressure_lower, reference_atmosphere_table["p"]))
                temperature = np.concatenate((temperature_lower, reference_atmosphere_table["T"]))
                height = np.concatenate((height, reference_atmosphere_table["height"]))
            except Exception as ex:
                template = "An exception of type {0} occurred. Arguments:\n{1!r}"
                message = template.format(type(ex).__name__, ex.args)
                logger.error(message)
        ...
        t = QTable(
            [height[1:], pressure, temperature, relative_humidity],
            names=("height", "pressure", "temperature", "relative_humidity"),
        )
        t.sort("height")
        bin_widths = np.diff(np.sort(height))
        ...
        col_h_max = Column(name="height_max")
        col_h_min = Column(name="height_min")
        rayleigh_extinction_table.add_columns([col_h_max, col_h_min], indexes=[0, 0])
        aod_dict = {
            aod: 0
        ...
        for row in t[mask]:
            new_row = []
            new_row.append(row["height"])
            new_row.append(row["height"] - row["bin_widths"])
