...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
            ...
            custom_train_dataloader=None, custom_val_dataloader=None, scaler=None):
        """
        ...
        if custom_train_dataloader is None:
            train_loader = FastTensorDataLoader(torch.from_numpy(
                np.concatenate(x, axis=-1)).to(self.device), torch.from_numpy(y).to(self.device), batch_size=batch_size,
                                            shuffle=True)
        else :
            tensors_x = torch.from_numpy(
                    np.concatenate(x, axis=-1)).float().to(self.device)

            tensors_y = torch.from_numpy(y).long().to(self.device)

            if hasattr(custom_train_dataloader, "dataset"):
                custom_train_dataloader.dataset.tensors = tensors_x, tensors_y
            else:
                custom_train_dataloader.set_tensors(tensors_x, tensors_y)
