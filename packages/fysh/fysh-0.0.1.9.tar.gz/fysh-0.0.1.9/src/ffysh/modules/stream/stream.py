...
class Stream:
    ...
    def download(self, whole=False, chunk=False, assets=False, reset=False):
        self.save_location = os.path.join(os.getcwd(), os.path.join('downloads', self.dataset_id))
        ...
        if not chunk:
            for asset in self:
                bname = os.path.basename(asset.path)
                shutil.copy(asset.path, self.save_location)
                os.rename(os.path.join(self.save_location, bname), os.path.join(self.save_location, asset.display_name))
        else:
        ...
        print(f'\n[SUCCESS] Dataset saved to {self.save_location}.')

    # def to_yolov5_zip(self, self.save_location: str, confidence_level: float = 0.4):
    #     with ZipFile(self.save_location, "w", allowZip64=True) as new_zip:
    ...
    #
    # def to_yolov5_model(self, self.save_location: str, confidence_level: float = 0.4):
