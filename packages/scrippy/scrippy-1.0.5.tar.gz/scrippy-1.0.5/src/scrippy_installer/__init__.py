...
def install_from_config(config, yes=False):
...
def install(interactive=False, config=None, yes=False):
  if not interactive and config is None:
    return install_from_config(config=load_default(), yes=yes)
  if not interactive and config is not None:
    return install_from_config(config=load_config(config=config), yes=yes)
  if interactive:
    return install_from_config(config=build_config(), yes=yes)
