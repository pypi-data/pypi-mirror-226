    ...
    if model:
      ...
      if goal == 'maximize':
        if metric_value > prev_max:
          ...
          results_dict.update({'best_'+metric_name+'_value': metric_value})
...
def torch_save_best():
    ...
    if save_best_results == True:
      ...
      with open(run_dir_global+'/model/best_model_logs.txt', 'w') as f:
        ...
        f.write('\nBest Value: ' + str(results_dict['best_'+results_dict['metric_name']+'_value']))
          ...
          if self.metric_name not in logs.keys():
            ...
            if self.goal == 'maximize':
              if logs.get(self.metric_name) > prev_max:
                ...
                results_dict.update({'best_'+self.metric_name+'_value': logs.get(self.metric_name)})
                results_dict.update({'best_metric_index': index_count})
