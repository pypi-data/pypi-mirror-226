...
def results(y_true = [], y_predicted = [], threshold=0.5, results_dict = {}, node_name="Results", problem_type = 'binary classification', run_id = None):
    ...
    if len(y_true) != 0 and len(y_predicted) != 0:
      ...
      fpr_spaced, tpr_spaced = evenly_spaced_points(fpr, tpr, 5000)
      ...
      binary = {'precision': round(prec, 4), 'recall': round(rec, 4), 'specificity': round(spec, 4),
              ...
              'hist_one_freq': hist_one_freq.tolist(), 'hist_one_bins': hist_one_bins.tolist(),'fpr': fpr_spaced.tolist(), 'tpr': tpr_spaced.tolist(),
    ...
    if model:
      ...
      if goal == 'maximize':
        if metric_value > prev_max:
          ...
          results_dict.update({'best_epoch': current_epoch})
...
def torch_save_best():
    ...
    if save_best_results == True:
      ...
      with open(run_dir_global+'/model/best_model_logs.txt', 'w') as f:
        ...
        f.write('\nBest Value: ' + str(results_dict['best_'+results_dict['metric_name']]))
        f.write('\nBest Epoch: ' + str(results_dict['best_epoch']))
...
def data(train_set=[], train_labels=[], test_set=[], test_labels=[], val_set=[], val_labels=[], dataset_name="", #checks if data is same as prev
  ...
  if not os.path.exists(current_dir+'/data/'+dataset_name):
    os.mkdir(current_dir+'/data/'+dataset_name)

  # Check if data is new
  existing_files = os.listdir(current_dir+'/data/'+dataset_name)

  if 'train_set.pk' in existing_files or 'val_set.pkl' in existing_files or 'test_set.pkl' in existing_files:
      print("Datasets already exist in folder ", current_dir+'/data/'+dataset_name, " for this project. ")
          ...
          if self.metric_name not in logs.keys():
            ...
            if self.goal == 'maximize':
              if logs.get(self.metric_name) > prev_max:
                ...
                results_dict.update({'best_metric_index': index_count})
                results_dict.update({'best_epoch': epoch})
                results_dict.update({'metric_goal': 'maximize'})
