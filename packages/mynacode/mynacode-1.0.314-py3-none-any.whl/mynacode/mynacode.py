...
def data(train_set=[], train_labels=[], test_set=[], test_labels=[], val_set=[], val_labels=[], dataset_name="", #checks if data is same as prev
  ...
  if os.path.exists(newest_data_folder + '/train_labels.pkl'):
      if len(train_labels) > 0:
          ...
          first_20 = train_labels[:percent_20]
          
          with open(newest_data_folder + '/train_labels.pkl', 'rb') as f:
              x = dill.load(f)
              print(x)
          
  if os.path.exists(newest_data_folder + '/val_labels.pkl'):
      if len(val_labels) > 0:
          percent_20 = int((20/100)*len(val_labels))
          first_20 = val_labels[:percent_20]

          with open(newest_data_folder + '/val_labels.pkl', 'rb') as f:
              x = dill.load(f)
              print(x)

  if os.path.exists(newest_data_folder + '/test_labels.pkl'):
      if len(test_labels) > 0:
          percent_20 = int((20/100)*len(test_labels))
          first_20 = test_labels[:percent_20]
