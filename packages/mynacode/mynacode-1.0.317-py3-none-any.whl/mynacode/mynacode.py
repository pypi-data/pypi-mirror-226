...
def data(train_set=[], train_labels=[], test_set=[], test_labels=[], val_set=[], val_labels=[], dataset_name="", #checks if data is same as prev
  ...
  if save_new_data_flag == 1:
      ...
      if len(train_labels) > 0:
        config_dict.update({'train_labels': newest_data_folder + '/train_labels.pkl'})
      if len(val_labels) > 0:
        config_dict.update({'val_labels': newest_data_folder + '/val_labels.pkl'})
      if len(test_labels) > 0:
        config_dict.update({'test_labels': newest_data_folder + '/test_labels.pkl'})
