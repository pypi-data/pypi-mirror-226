...
class CmdRepo(CmdBase):
    def __init__(self, args): 
        super().__init__(args)
        log_setup(self.args)  
        self.repo = RepoMain(ConfigManager(get_all_config()))     
        if getattr(self.args, "name", None): #name is equivalent of git branch name
            self.args.name = self.args.name.lower()            
            repo_name_valid(self.args.name) #name is equivalent of git branch name
        else:
