...
class RepoMain():
    ...
    def validation(self, repository_name, repository_tag, git_protocol):
        ...
        if git_protocol not in self.PROTOCOL:
            print_err_msg("'{0}' protocol is not available. Please select from {1}".format(git_protocol, self.PROTOCOL))
        if git_protocol == "ssh":
    ...
    def run_repo_create_subprocesses(self,repo_name, repo_tag, git_protocol):    
        ...
        if git_protocol == "ssh":
            ...
            run_command_on_subprocess("git clone https://github.com/{0}/{1}.git .".format(self.GIT_ORG, self.common_repo_name), repo_name, True)
