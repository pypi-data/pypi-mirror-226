                ...
                if (not self.exists):
                    ...
                    if not overwrite:
                        if not self.read(size):
                            raise Exception('%s failed to read!' % (self.relpath))
                    else:
                        if (not records is None):
                            # create new shared memory set value
                            if isinstance(records,pd.DataFrame):
                            ...
                            raise Exception('%s failed to overwrite!' % (self.relpath))
    ...
    def fill_header(self):
        ...
        if write_head:
            ...
            if nb_records_mb<=100:
                ...
                message = 'Creating md5 hash:%iMB %s' % (nb_records_mb,self.relpath)
                block_size = 100 * 1024 * 1024 # or any other block size that you prefer
