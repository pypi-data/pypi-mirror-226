import os,logging,boto3,time,pytz,json
...
class KinesisLogStreamConsumer():
    ...
    def readLogs(self):
        ...
        _logfilepath = _logfilepath / (datetime.utcnow().strftime('%Y%m%d')+'.log')
            ...
            try:
                with open(self.logfilepath, 'r') as file:
                    ...
                    dfnewlines = pd.read_csv(StringIO(newlines), header=None, sep=';', \
                            engine='python', on_bad_lines='skip')
                    dfnewlines.columns = ['shardid','sequence_number','user_name','asctime','logger_name','level','message']
