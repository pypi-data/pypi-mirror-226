...
from addons_installer.cli import (
    _get_addons,
    _get_parser,
    _get_profiles,
    _install_addons,
...
class TestAddonsInstaller(unittest.TestCase):
    ...
    def test_04bis_get_parser_default(self):
        ...
        self._assertParser(
            parser,
            [],
            expected_values={"all": False, "i": None},
        )
    ...
    def test_05_get_parser_specific(self):
        ...
        self._assertParser(
            ...
            expected_values={"i": "monaddons", "profiles": "prof1,prof2", "all": False},
        )
    ...
    def test_07_get_addons(self):
        self.assertEqual(_get_addons(_get_parser().parse_args(["-i", "COMMON15,web_truc"]).i), ["COMMON15", "WEB_TRUC"])

    def test_08_cli_exists(self):
        # entry point must exist
        from addons_installer.cli import install_from_env

        self.assertTrue(install_from_env)
        # entrypoint has no parameters (args will be passed from command line)
        self.assertFalse(inspect.signature(install_from_env).parameters)
