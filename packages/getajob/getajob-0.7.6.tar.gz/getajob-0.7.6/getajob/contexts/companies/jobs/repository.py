...
class JobsRepository(MultipleChildrenRepository[Job]):
    def __init__(
        ...
        *,
        ...
        algolia_jobs: AlgoliaSearchRepository | None = None
    ):
        kafka_event_config = KafkaEventConfig(
            topic=KafkaTopic.jobs,
            message_type_enum=KafkaJobsEnum
        )
        super().__init__(
            RepositoryDependencies(
    ...
    def create_job(self, company_id: str, job: UserCreateJob):
        if not self.algolia_jobs:
            raise Exception("Algolia jobs repository not set")
        return JobsUnitOfWork(self, self.algolia_jobs).create_job(company_id, job)

    def post_job(self, company_id: str, job_id: str):
        if not self.algolia_jobs:
            raise Exception("Algolia jobs repository not set")
        return JobsUnitOfWork(self, self.algolia_jobs).post_job(company_id, job_id)
    
    def unpost_job(self, company_id: str, job_id: str):
        if not self.algolia_jobs:
            raise Exception("Algolia jobs repository not set")
        return JobsUnitOfWork(self, self.algolia_jobs).unpost_job(company_id, job_id)
