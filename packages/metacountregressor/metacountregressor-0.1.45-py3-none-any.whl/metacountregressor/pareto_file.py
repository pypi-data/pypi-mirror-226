...
class Solution(dict):
    ...
    def add_objective(self, bic=None, MAE=None, loglik=None, num_parm=None, pval_exceed=None, aic=None, GOF = None):
        if bic is not None:
            
            self['bic'] = bic
        if MAE is not None:
        
            self['MAE'] = MAE.get('MAE')
            self['MSE'] = MAE.get('MSE')
            #self['MAPE'] = MAE.get('MAPE')
            self['RMSE'] = MAE.get('RMSE')
