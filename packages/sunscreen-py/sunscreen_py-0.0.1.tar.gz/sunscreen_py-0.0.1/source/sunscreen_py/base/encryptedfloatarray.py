from .encryptedfloat import EncryptedFloat, EncryptedFloatZero
...
class EncryptedFloatArray:
    sub_arrays = []
    context = None
    key_set_override = None

    def __init__(self, ciphers, context, key_set_override=None):
        self.context = context
        self.key_set_override = key_set_override
        self.sub_arrays = ciphers

    @classmethod
    def create_zeros_for_dimension(_cls, dimension, context, key_set_override=None):
        if len(dimension) == 0:
            raise Exception("Can't create an array with 0 dimensions")
        if len(dimension) == 1:
            ciphers = [
                EncryptedFloatZero(context, key_set_override) for _ in range(0, dimension[0])
            ]
        else:
            ciphers = [
    ...
    @classmethod
    def create_from_plain_vectors(_cls, vector, context, key_set_override=None):
        # plain_sub_arrays = EncryptedFloatArray.__chunk(
        #    vector, EncryptedFloatInternalArray.STANDARD_LENGTH
        # )
        ...
        if isinstance(vector[0], float):
            ...
            def execute(data):
                return EncryptedFloat.create_from_plain(data, context, key_set_override)

            ciphers = SunscreenPool.get_instance().map(execute, vector)
        else:
    ...
    def create_from_encrypted_cipher_string(
        ...
        if isinstance(cipher_string_array, list):
            if isinstance(cipher_string_array[0], str):
                interim_result = [
                    EncryptedFloat.create_from_encrypted_cipher_string(
                        i, context, is_fresh, noise_budget, key_set_override
                    )
                    for i in cipher_string_array
                ]
                return EncryptedFloatArray.create_from_encrypted_vector_objects(
                    interim_result, context, is_fresh, key_set_override
                )
            if isinstance(cipher_string_array[0], list):
                interim_result = [
                    EncryptedFloatArray.create_from_encrypted_cipher_string(
                        i, context, is_fresh, noise_budget, key_set_override
                    )
                    for i in cipher_string_array
                ]
        ...
        raise Exception("Cipher string must be an array")

    @classmethod
    def create_from_encrypted_vector_objects(
        _cls, ciphers, context, is_fresh=False, key_set_override=None
    ):
    ...
    def __setitem__(self, index, item):
        ...
        if (
            isinstance(self.sub_arrays[0], EncryptedFloat)
            and isinstance(item, EncryptedFloat)
        ) or (
            isinstance(self.sub_arrays[0], EncryptedFloatArray)
            and isinstance(item, EncryptedFloatArray)
        ):
            self.sub_arrays[index] = item
            return
    ...
    def get_string(self):
        if len(self.sub_arrays) > 1:
            if isinstance(self.sub_arrays[0], EncryptedFloat):
                s = [i.get_string() for i in self.sub_arrays]
                return sum(s, [])
            if isinstance(self.sub_arrays[0], EncryptedFloatArray):
                return [i.get_string() for i in self.sub_arrays]
        else:
            return [self.sub_arrays[0].get_string()]
    ...
    def dot(self, other):
        if isinstance(other, EncryptedFloatArray) and isinstance(
            self.sub_arrays[0], EncryptedFloat
        ):
    ...
    def sum(self):
        def execute(data):
            if isinstance(self.sub_arrays[data], EncryptedFloat):
    ...
    def mm(self, other, vectorize=False):
        if len(self.shape()) != 2:
            raise Exception(
                "Matrix Multiplication is only implemented for 2 D matrices"
            )
        ...
        R1 = self.shape()[0]
        C1 = self.shape()[1]
        R2 = other.shape()[0] if isinstance(other, EncryptedFloatArray) else len(other)
        C2 = (
            other.shape()[1]
        ...
        if not vectorize:
            ...
            if R1 == 1:
                ...
                result = EncryptedFloatArray([self.dot(other[i]) for i in range(length)], self.context, self.key_set_override)
            else:
                result = EncryptedFloatArray([self[i].mm(other, vectorize=vectorize) for i in range(self.shape()[0])], self.context, self.key_set_override)
    ...
    def __mul__(self, other):
        if isinstance(other, EncryptedFloatArray):
            ...
            def execute(data):
                return other.sub_arrays[data] * self.sub_arrays[data]
        ...
        if isinstance(other, float) or isinstance(other, EncryptedFloat):

            def execute(data):
                return other * self.sub_arrays[data]
    ...
    @classmethod
    def __chunk(_cls, lst, size):
        n = ceil(len(lst) / size)
        return list(map(lambda x: lst[x * size : x * size + size], list(range(n))))
    
    @classmethod
    def __list_transpose(_cls, lst):
        dim1 = len(lst)
        dim2 = 1
