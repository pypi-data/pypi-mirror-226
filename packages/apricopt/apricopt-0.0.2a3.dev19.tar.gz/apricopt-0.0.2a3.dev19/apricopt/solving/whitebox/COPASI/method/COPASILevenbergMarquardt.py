...
class COPASILevenbergMarquardt(COPASIOptimisationMethod):
    ITERATION_LIMIT = "Iteration Limit"
    TOLERANCE = "Tolerance"
    MODULATION = "Modulation"
    STOP_AFTER_N_ITERATIONS = "Stop after # stalled iterations"
    INITIAL_LAMBDA = "Initial Lambda"
    LAMBDA_DECREASE = "Lambda Decrease"
    LAMBDA_INCREASE = "Lambda Increase"

    def __init__(self):
        super(COPASILevenbergMarquardt, self).__init__()
        self.solver_parameters = [
            COPASISolverParameter(COPASILevenbergMarquardt.ITERATION_LIMIT, 200),
            COPASISolverParameter(COPASILevenbergMarquardt.TOLERANCE, 1e-5)
        ]

    def set_parameters_configuration(self, opt_task: COPASI.COptTask,
                                     solver_parameters: List[COPASISolverParameter] = None):
        ...
        self.opt_task.setMethodType(COPASI.CTaskEnum.Method_LevenbergMarquardt)
