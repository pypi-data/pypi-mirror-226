...
def update_labels(d: dict, FASTA_ALLELES: dict, prefix="insertion") -> dict:
    ...
    while True:
        ...
        d_updated = {f"{prefix}{i+1:0{digits}}": seq for i, seq in enumerate(d_values)}
...
def save_fasta(TEMPDIR: Path | str, SAMPLE_NAME: str, consensus_sequence_insertion: dict) -> None:
    # path_dir = Path(TEMPDIR, SAMPLE_NAME, "fasta", "insertion")
    for header, seq in consensus_sequence_insertion.items():
        # ? name = f"{SAMPLE_NAME}_{header}"
...
def generate_trancated_fasta(TEMPDIR, SAMPLE_NAME, CONTROL_NAME, FASTA_ALLELES) -> None:
