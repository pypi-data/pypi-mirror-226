...
def test_cloud_event_producer_produce(internal_error, cloud_provider_error, exception):
    ...
    expected_error = None
    if internal_error and not cloud_provider_error:
        expected_error = CloudAnalyticsEventError(internal_error=mock_internal_error)
    elif cloud_provider_error and not internal_error:
    ...
    elif internal_error and cloud_provider_error:
        expected_error = CloudAnalyticsEventError(
            internal_error=json.dumps(
                {
                    "cloud_provider_errors": [
                        {
                            "cloud_resource": str(cloud_provider_error.cloud_resource),
                            "error_code": str(cloud_provider_error.error_code),
                        }
                        for cloud_provider_error in [
                            CloudAnalyticsEventCloudProviderError(
