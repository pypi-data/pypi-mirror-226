...
from .hedgeguy_Components import *
import datetime
import pandas as pd
import numpy as np
        ...
        def display_output(sort_method,para_checklist,filter_dropdown_div,filter_input_div,fig_line,
            ...
            if chart2_button_clicks or (not (aggregate_boolean == self.breakdown_toggle_status)):
                if (chart2_button_clicks > self.chart2_button_clicks) or (not (aggregate_boolean == self.breakdown_toggle_status)):
                    ...
                    if self.line_selected > -1:
                        ...
                        breakdown_toggle_div = {'margin-left': '10px', 'display': 'none'}
            ...
            if add_button_clicks:
                if add_button_clicks > self.add_button_count: ## Add Button Pressed
                    ...
                    if filter_name:
                        update_filter = False
                        if filter_name == 'higher_net_profit':
                            self.filter_list.append(['higher_net_profit', ' ', ' '])
                            update_filter = True
                        elif filter_name == 'lower_mdd':
                            self.filter_list.append(['lower_mdd', ' ', ' '])
                            update_filter = True
                        elif filter_name == 'higher_net_profit_mdd':
                            self.filter_list.append(['higher_net_profit_mdd', ' ', ' '])
                            update_filter = True
                        elif filter_name == 'higher_ann_sharpe':
                            self.filter_list.append(['higher_ann_sharpe', ' ', ' '])
                            update_filter = True



                        elif filter_input_value: ## Paremeter
            ...
            if sort_method:
                ...
                if len(self.filter_list) > 0:
                    for element in self.filter_list:
                        ...
                        elif element[0] == 'higher_net_profit':
                            current_df = current_df.loc[current_df['net_profit'] > current_df['bah_net_return']]
                        elif element[0] == 'lower_mdd':
                            current_df = current_df.loc[current_df['mdd_dollar'] < current_df['bah_mdd_dollar']]
                        elif element[0] == 'higher_net_profit_mdd':
                            current_df = current_df.loc[current_df['net_profit_to_mdd'] > current_df['bah_net_profit_to_mdd']]
                        elif element[0] == 'higher_ann_sharpe':
                            current_df = current_df.loc[current_df['annualized_sr'] > current_df['bah_annualized_sr']]
                        elif element[1] == '<':
