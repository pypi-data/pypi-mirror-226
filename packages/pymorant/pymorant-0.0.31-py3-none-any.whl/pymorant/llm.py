...
from langchain.chains.summarize import load_summarize_chain as _load_summarize_chain # noqa
from langchain.docstore.document import Document as _Document
from langchain.llms import OpenAI as _OpenAI
from langchain.output_parsers import CommaSeparatedListOutputParser as _CommaSeparatedListOutputParser # noqa
from langchain.prompts import PromptTemplate as _PromptTemplate
from langchain.prompts import ChatPromptTemplate as _ChatPromptTemplate, HumanMessagePromptTemplate as _HumanMessagePromptTemplate # noqa
...
def corregir_texto(text, model, openai_key):
    ...
    chatopenai = _ChatOpenAI(model_name=model, openai_api_key=openai_key)
    llmchain_chat = _LLMChain(llm=chatopenai, prompt=prompt)

    corrected_text = llmchain_chat.run(text)
    cleaned_text = _re.sub(pattern, '', corrected_text)
...
def generar_variantes_texto(text, n, model, openai_key, formal=False, sex="NA"): # noqa
    ...
    prompt = _ChatPromptTemplate(
        messages=[_HumanMessagePromptTemplate.from_template(template_string)],
    ...
    pattern = r'^[\'"]|[\'"](?=\.)|[\'"]$'
    final_list = [_re.sub(pattern, '', s) for s in final_list]
...
def resumir_texto(text, model, openai_key, input_autogenerado):
    ...
    llm = _OpenAI(temperature=0.0, openai_api_key=openai_key)
    ...
    if input_autogenerado:
        ...
        docs = [_Document(page_content=t) for t in text]

        prompt = _PromptTemplate(
            template=prompt_template, input_variables=["text"]
        )

        chain = _load_summarize_chain(
...
def generar_categorias(text, n, model, openai_key):
    ...
    try:
        ...
        categorias_list = _ast.literal_eval(categorias)
