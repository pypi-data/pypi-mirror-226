...
def get_args(args_list: Optional[List[str]] = None) -> Tuple[CertoraContext, Dict[str, Any]]:
    ...
    _remove_parsing_whitespace(args_list)
...
def _remove_parsing_whitespace(arg_list: List[str]) -> None:
    """
    Removes all whitespaces added to args by _alter_args_before_argparse():
...
def _alter_args_before_argparse(args_list: List[str]) -> None:
    """
...
def pre_arg_fetching_checks(args_list: List[str]) -> None:
    ...
    _alter_args_before_argparse(args_list)
...
def _dedup_link(context: CertoraContext) -> None:
    try:
...
def _cannonize_settings(context: CertoraContext) -> None:
    """
...
def _check_single_arg_and_setting_consistency(context: CertoraContext, arg_name: str, setting_name: str,
...
def _check_bool_arg_and_implicit_setting_consistency(context: CertoraContext, arg_name: str,
...
def _check_bool_arg_and_explicit_setting_consistency(context: CertoraContext, arg_name: str,
                                                     setting_name: str) -> None:
    """
    We accept two syntaxes for settings: --rule or --settings -rule.
...
def check_arg_and_setting_consistency(context: CertoraContext) -> None:
    ...
    for (argument, setting) in val_arg_to_setting.items():
        _check_single_arg_and_setting_consistency(context, argument, setting, False)

    for (argument, setting) in val_arg_to_list_setting.items():
        _check_single_arg_and_setting_consistency(context, argument, setting, True)

    for (argument, setting) in bool_arg_to_implicit_setting.items():
        _check_bool_arg_and_implicit_setting_consistency(context, argument, setting)

    for (argument, setting) in bool_arg_to_explicit_setting.items():
        _check_bool_arg_and_explicit_setting_consistency(context, argument, setting)
...
def _convert_settings(context: CertoraContext) -> None:
...
# Conf convert code from old API to new - temporary
def _convert_java_args(context: CertoraContext) -> None:
...
def _convert_solc_args(context: CertoraContext) -> None:
...
def _rename_key(context: CertoraContext, old_key: str, new_key: str) -> None:
...
def convert_context(context: CertoraContext) -> CertoraContext:
    ...
    _convert_settings(context)
    _convert_solc_args(context)
    _convert_java_args(context)
    _rename_key(context, 'disableLocalTypeChecking', 'disable_local_typechecking')
    _rename_key(context, 'optimize', 'solc_optimize')  # assume no conflicts with solc_args
    _rename_key(context, 'optimize_map', 'solc_optimize_map')  # assume no conflicts with solc_args
    _rename_key(context, 'path', 'solc_allow_path')
    return context
