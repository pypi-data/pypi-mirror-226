...
def change_periodicity(df, date_col, output_freq, categorical_cols=None, input_starting=True, output_starting=True, agg=None):
    ...
    if periodicity == 'D':
        ...
        if output_freq.upper() in ['D', 'DAILY']:
            ...
            df = weekly_to_daily(df, date_col, categorical_cols, input_starting, agg)
            return daily_to_weekly(df, date_col, output_freq[2:], categorical_cols, output_starting, agg)
        elif output_freq.upper() in ['M', 'MONTHLY']:
            df = weekly_to_daily(df, date_col, categorical_cols, input_starting, agg)
            return  daily_to_monthly(df, date_col, categorical_cols, agg)
    elif periodicity == 'M':
        if output_freq.upper() in ['D', 'DAILY']:
            ...
            df = monthly_to_daily(df, date_col, categorical_cols, agg)
            return daily_to_weekly(df, date_col, output_freq[2:], categorical_cols, output_starting, agg)
        elif output_freq.upper() in ['M', 'MONTHLY']:
            return df
    else:
