...
def data_anonymization(df, categorical_cols, numerical_cols=None, columns_to_remove=None):
    """
    ...
    Args:
        ...
        categorical_cols (dict): A dictionary where keys are substrings to search for in column names and values
    ...
    for key,val in categorical_cols.items():
        for col in df.columns:
            if key.upper() in col.upper():
                df.rename(columns={col: col.upper().replace(key.upper(), val)}, inplace=True)
                col = col.upper().replace(key.upper(), val)
            try:
                df[col] = df[col].str.replace(key, val, case=False, regex=True)
            except:
                pass
    
    for col in [col for col in df.columns if is_numeric_dtype(df[col])]:
        if col in numerical_cols.keys():
            ...
            df[col] = df[col].apply(lambda x: min(max(a * x + b, minimum), maximum))
