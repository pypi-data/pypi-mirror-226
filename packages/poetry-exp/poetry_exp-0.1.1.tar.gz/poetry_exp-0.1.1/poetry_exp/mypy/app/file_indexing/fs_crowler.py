...
def start_fs_crowler(index_name, dir_path):
    LOG.info('Creating file system crowler job...')
    ...
    LOG.info('Starting file system crowler job: {0} ...'.format(index_name))
...
def edit_config_file(file_path, dir_path, es_url):
    LOG.info("Updating the crowler job config: {0}...".format(file_path))
    ...
    LOG.info('Successfully updated the file system crowler job config file')
...
def get_device_paths(disk_ids):
    ...
    LOG.info("Device paths to mount: {0}".format(dev_paths))
...
def mount_disks(disk_ids, snapshot_info):
    ...
    mount_paths = []
    size = list(xrange(1, len(dev_paths) + 1))
    LOG.info('snapshot info: {0}'.format(snapshot_info))
    print('snapshot info: {0}'.format(snapshot_info))
    snap_id = snapshot_info['id']
    snap_name = snapshot_info['name'].replace(" ", "T")
    LOG.info('snap_name: {0}'.format(snap_name))
    print('snap_name: {0}'.format(snap_name))
    try:
        for i in size[0: len(size)-1]:
            mount_path = MOUNT_DIR.format(snap_id, snap_name, i)
            LOG.info('Creating mount dir: {0}...'.format(mount_path))
            print('Creating mount dir: {0}...'.format(mount_path))

            os.makedirs(mount_path)
            LOG.info('Mounting device: {0} with dir: {1}...'.format(dev_paths[i], mount_path))
        ...
        traceback.print_exc()
        LOG.exception('Failed to mount disks: {0}'.format(disk_ids))
    ...
    if mount_paths:
        for mount_path in mount_paths:
            if os.path.exists(mount_path):
                LOG.info('Un mounting path : {0}'.format(mount_path))
                ...
                try:
                    ...
                    traceback.print_exc()
                    LOG.exception("ERROR: Failed to un mount,"
                          " error: {0}".format(str(e)))
                    LOG.info("ERROR: Failed to un mount,"
                          " error: {0}".format(str(e)))
                    print("ERROR: Failed to un mount,"
                             " error: {0}".format(str(e)))
                LOG.info('Deleting mount path : {0}'.format(mount_path))
                print('Deleting mount path : {0}'.format(mount_path))
                try:
                    exe_cmd2('rm -rf ' + mount_path)
                except Exception as e:
                    LOG.exception("ERROR: Failed to delete mount path,"
                          " error: {0}".format(str(e)))
                    traceback.print_exc()
                    LOG.info("ERROR: Failed to delete mount path,"
                          " error: {0}".format(str(e)))
...
def delete_fs_crowl_job(vm_id):
    ...
    if os.path.exists(job_config_dir):
        LOG.info('Deleting file system crowler job config... : {0}'.format(job_config_dir))
        print('Deleting file system crowler job config... : {0}'.format(job_config_dir))

        exe_cmd2('rm -rf ' + DEFAULT_SETTINGS_DIR + vm_id)
    else:
        LOG.info('No file system crowler job found for path : {0}'.format(job_config_dir))
...
def mount_and_crowl(vm_id, disk_ids, snapshot_info):
    ...
    LOG.info('Virtual machine disk ids: {0}'.format(disk_ids))
    ...
    try:
        ...
        LOG.exception('Failed to start file system crowler,'
                        ' error: {0}'.format(str(e)))
        traceback.print_exc()
...
if __name__ == '__main__':
   ...
   snap_info = {
       'id': 'fd4d12f4-b31c-4263-ae50-5246f46c19a3',
       'name': '2020-07-05 12:36:35.44346'
   }
