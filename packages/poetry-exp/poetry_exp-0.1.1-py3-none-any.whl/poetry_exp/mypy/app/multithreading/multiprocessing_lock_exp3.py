...
def f(l, i):
    print(f'Process: {i}, acquiring lock..., lock dict: {lock_dict}, pid: {os.getpid()}')
    lock_dict[i] = l
    lock_acquire_result = l.acquire(timeout=100)
    print(f'Process: {i}, acquired lock, result: {lock_acquire_result},, lock dict: {lock_dict}, pid: {os.getpid()}')
    print(f'Process: {i}, starts execution....., pid: {os.getpid()}')
    # Critical section of the code
    for k in range(5):
        print(f'process:{i}, data: {k} ')
        time.sleep(1)
    print(f'Process: {i}, execution completed., pid: {os.getpid()}')

    if is_locked(l):
        print(f'Process: {i}, releasing lock., pid: {os.getpid()}')
        l.release()
