...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~kymang.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~kymang.Client.sign_in` or :meth:`~kymang.Client.sign_up`).

        next_type (:obj:`~kymang.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~kymang.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~kymang.Client.resend_code`.
    """
