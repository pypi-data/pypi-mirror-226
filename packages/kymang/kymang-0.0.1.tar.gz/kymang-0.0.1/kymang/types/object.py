...
class Object:
    def __init__(self, client: "kymang.Client" = None):
        self._client = client

    def bind(self, client: "kymang.Client"):
        """Bind a Client instance to this and to all nested kymang objects.

        Parameters:
            client (:obj:`~kymang.types.Client`):
                The Client instance to bind this object with. Useful to re-enable bound methods after serializing and
                deserializing kymang objects with ``repr`` and ``eval``.
        """
        self._client = client
