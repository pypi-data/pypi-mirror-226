...
import kymang
from kymang.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~kymang.Client.start`, :meth:`~kymang.idle` and :meth:`~kymang.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that kymang's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`kymang.compose`.
