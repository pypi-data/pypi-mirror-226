...
USER_AGENT = f"python-cryprum/{__version__}"


class BaseClient:
    def __init__(self, token: Optional[str] = None, timeout: Optional[float] = 10):
        if token is None:
            token = os.getenv("CRYPRUM_TOKEN")
        assert (
            token is not None
        ), "Token not found. Use Client(token='<token>') or set env CRYPRUM_TOKEN=<token>"
        self._url = "https://api.cryprum.com{}"
        self._token = token
