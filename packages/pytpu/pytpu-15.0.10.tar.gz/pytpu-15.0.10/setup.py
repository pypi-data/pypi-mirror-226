...
VERSION = "15.0.10"  # WARNING: build script replace 15.0.10 with an actual three-decimal version using `sed` in file!

if not VERSION or VERSION == "15.0.10":
    warnings.warn(f'Invalid package version "{VERSION}". If that is NOT a local debug installation you must replace '
                  f'VERSION with a valid value or provide 15.0.10 environment variable for the build script.')
