...
class EmailBot(object):
    def __init__(self, email='user@nauta.cu',email_password='xxx',type='nauta.cu',imapssl=True,startls=True,smtptag=None,typeimap=None):
        self.imapssl = imapssl
        self.startls = startls

        self.type = type
        self.smtp_host_server = f'smtp.{type}'
        
        if not typeimap:
        	self.imap_host_server = f'imap.{type}'
        else:
        	self.imap_host_server = f'imap.{typeimap}'
        
        if smtptag:
        	self.smtp_host_server = self.smtp_host_server.replace('smtp',smtptag)
