...
class DameNamsor(Gender):
    ...
    def download_csv(self, path="files/names/min.csv", *args, **kwargs):
        ...
        # file and uses them to get the gender and scale of each name.
        dir0 = "files/tmp/"
        backup_all = kwargs.get('backup_all', dir0 + 'namsorall.csv')
        backup_females = kwargs.get('backup_females', dir0 + 'namsorfemales.csv')
        backup_males = kwargs.get('backup_males', dir0 + 'namsormales.csv')        
        name_position = kwargs.get('name_position', 0)
        names = self.csv2names(path, name_position=name_position,
                               surname_position=surname_position, surnames=True)
        if backup_females:
