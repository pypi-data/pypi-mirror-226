...
class Lumina():
    ...
    def putText(self,image:np.array,texts:str,bbox:Union[List,Tuple],font:cv2.FONT_HERSHEY_PLAIN=cv2.FONT_HERSHEY_PLAIN,textcolors:Tuple=(255,255,255),scale:int=2,thickness:int=2,border:bool=True,rect_color:Tuple=(0,255,0),border_size:int=2) -> np.array:
        ...
        : prarm textcolors : expects a tuple (0,255,0)
        ...
        if border:
            ...
            cv2.putText(image,text=str(texts),org=(bbox[0],bbox[1]),fontFace=font,fontScale=scale,color=textcolors,thickness=thickness,lineType=cv2.LINE_AA)
        else:
            cv2.putText(image,text=str(texts),org=(bbox[0],bbox[1]),fontFace=font,fontScale=scale,color=textcolors,thickness=thickness,lineType=cv2.LINE_AA)
        return image
    
    def putText_center(self,image:np.array,texts:str,bbox:List,font:cv2.FONT_HERSHEY_PLAIN=cv2.FONT_HERSHEY_PLAIN,textcolors:Tuple=(255,255,255),scale:int=2,thickness:int=2,border:bool=True,rect_color:Tuple=(0,255,0),border_size:int=2) -> np.array:
        ...
        if border:
            ...
            cv2.putText(image,text=str(texts),org=(center_x-10,center_y),fontFace=font,fontScale=scale,color=textcolors,thickness=thickness,lineType=cv2.LINE_AA)
        else:
            cv2.putText(image,text=str(texts),org=(center_x-10,center_y),fontFace=font,fontScale=scale,color=textcolors,thickness=thickness,lineType=cv2.LINE_AA)
        return image
    ...
    def line_create(self,image:np.array,start_point:Tuple=(10,20),end_point:Tuple=(50,50),linecolors:Tuple=(0,255,0,10),lineThickness:int=55,transparency:float=.6) -> np.array:
        ...
        overlay = image.copy()
        cv2.line(image,start_point,end_point,color=linecolors,thickness=lineThickness,lineType=cv2.LINE_AA)
        image = cv2.addWeighted(overlay, transparency, image, 1 - transparency, 0)
        del overlay
        return image
    
    def polygon_create(self,image:np.array,poly_points:List[List]=[[10,20],[20,30],[30,40]],polycolors:Tuple=(0,255,0),isClosed:True=True,thickness:int=2) -> np.array: 
        ...
        : prarm polycolors : expects a tuple (0,255,0)
        ...
        """
        cv2.polylines(image, [np.array(poly_points,dtype=np.int32)], isClosed=isClosed, color=polycolors, thickness=thickness,lineType=cv2.LINE_AA)
        return image
    
    
    def fillPolygon_create(self,image:np.array,poly_points:List[List]=[[10,20],[20,30],[30,40]],polycolors:Tuple=(0,255,0),transparency:float=.8) -> np.array:
        """
        ...
        """
        overlay = image.copy()
        cv2.fillPoly(image, [np.array(poly_points,dtype=np.int32)], polycolors,lineType=cv2.LINE_AA)
        image = cv2.addWeighted(overlay, transparency, image, 1 - transparency,0)
        del overlay
        return image
    ...
    def line_in_out_counter(self,image:np.array,line_start_pos:Tuple=(10,20),line_end_pos:Tuple=(30,40),detection_pos:Tuple=(50,50),line_color:Tuple=(0,255,0),lineThickness:int=2,lineTransparecy:float=0.5) -> Tuple[np.array,bool]:
        ...
        if line_start_pos[0] < detection_pos[0] < line_end_pos[0] and line_start_pos[1]-25  < detection_pos[1]  < line_start_pos[1]+25:
            detect = True
        else:
            detect = False
    ...
    def apply_mask_on_image(self,image:np.array,mask:np.array) -> np.array:
        """
        THIS FUNCTION WILL APPLY MASK ON THE IMAGE USING AND OPERATOR
        
        : param image : expects cv2.imread() image
        : param mask : expects BW mask and cv2.imread() image
        
        : return : masked image
        """
        zone_masked = cv2.bitwise_and(image,image,mask=mask)
        return zone_masked
    
    
    def putText_reverse_T(self,image:np.array,texts:str,bbox:List=[10,20,30,40],font=cv2.FONT_HERSHEY_PLAIN,fontThickness:int=2,fontScale:int=2,colors:Tuple=(0,0,255),circleRadius:int=5,thickness:int=2,border:bool=True,text_color:Tuple=(255,255,255),border_size:int=2,line_len:int=30) -> np.array:
        
        """
        THIS FUNCTION WILL PUT TEXT  AND CREATE A GOOD VISUALIZATION
        
        : param image : cv2.imread() image
        : param texts : expects a text
        : param bbox  : A list containing rect top left x , y and bottom right x , y --> [x1,y1,x2,y2]
        : param font : expects cv2.FONT_
        : param fontThickness : thickness of the font
        ...
        : param text color : will change text's color
        : param border size : it will change border size of text
        : param line_len : it will increase straight line length 
        
        : return : customized image
        
        """
        top_center_x , top_center_y = int((bbox[0] + bbox[2])/2) , int((bbox[1] + bbox[1])/2)
      
        # line creation
        cv2.line(image,(bbox[0],bbox[1]),(bbox[2],bbox[1]),color=colors,thickness=thickness,lineType=cv2.LINE_AA)
        cv2.line(image,(top_center_x,top_center_y),(top_center_x,top_center_y-line_len),color=colors,thickness=thickness,lineType=cv2.LINE_AA)
        cv2.line(image,(bbox[0],bbox[1]),(bbox[0],bbox[1]+line_len),color=colors,thickness=thickness,lineType=cv2.LINE_AA)
        cv2.line(image,(bbox[2],bbox[1]),(bbox[2],bbox[1]+line_len),color=colors,thickness=thickness,lineType=cv2.LINE_AA)
        
        # circle creation over the line
        cv2.circle(image,center=(top_center_x,top_center_y-line_len),radius=circleRadius,color=colors,thickness=-1,lineType=cv2.LINE_AA)
        cv2.circle(image,center=(top_center_x,top_center_y-line_len),radius=circleRadius*2,color=colors,thickness=thickness,lineType=cv2.LINE_AA)
        
        #puttext
        self.putText(image,texts=texts,bbox=(top_center_x-line_len-20,top_center_y-line_len-20),font=font,colors=text_color,scale=fontScale,thickness=fontThickness,border=border,rect_color=colors,border_size=border_size)
        
        return image
    
    
    def putText_I(self,image:np.array,texts:str,bbox:List=[10,20,30,40],font=cv2.FONT_HERSHEY_PLAIN,fontThickness:int=2,fontScale:int=2,colors:Tuple=(0,0,255),circleRadius:int=5,thickness:int=2,border:bool=True,text_color:Tuple=(255,255,255),border_size:int=2,line_len:int=50) -> np.array:
        ...
        # line creation
        cv2.line(image,(center_x , center_y),(center_x , center_y-line_len),color=colors,thickness=thickness,lineType=cv2.LINE_AA)
        
        
        # circle creation over the line
        cv2.circle(image,center=(center_x , center_y),radius=circleRadius,color=colors,thickness=-1,lineType=cv2.LINE_AA)
        cv2.circle(image,center=(center_x , center_y),radius=circleRadius*2,color=colors,thickness=thickness,lineType=cv2.LINE_AA)
        
        #puttext
        self.putText(image,texts=texts,bbox=(center_x-10, center_y-line_len-5),font=font,colors=text_color,scale=fontScale,thickness=fontThickness,border=border,rect_color=colors,border_size=border_size)
