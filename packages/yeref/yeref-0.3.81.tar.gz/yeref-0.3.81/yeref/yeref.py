...
async def generate_calendar_admin(bot, state, lz, chat_id, message_id=None, is_new=True, is_timer=False):
    try:
        ...
        for i in range(0, len(buttons_)):
            ...
            if len(tmp) >= 7:
                ...
                tmp = []
        text = yeref.l_post_date[lz] if is_timer else yeref.l_post_timer[lz]
...
async def edit_simple2(bot, chat_id, user_id, target_id, post_id, message_id, current, BASE_CHN):
    ...
    try:
        ...
        if POST_ISGALLERY:
            ...
            reply_markup = await create_replymarkup3(target_id, post_id,
                                                     POST_BUTTON) if POST_ISBUTTON else InlineKeyboardBuilder()
