...
def list_split_by_step(
    ...
    try:
        ...
        # 数据平分时, 每份数据的最大长度
        _index_number_list = list(range(0, _data_length, number))
        # 数据平分后的结果
        _split_result = []

        if _data_length == 0 or _data_length <= number:
            _split_result.append(_data_object)
        else:
            for index_number in _index_number_list:
                _split_result.append(deepcopy(_data_object[index_number:index_number + number]))
...
def list_split_equally(
    ...
    try:
        ...
        if _data_length == 0 or _data_length <= _step_number:
            _split_result.append(_data_object)
        else:
            if _data_length % number == 0:
                ...
                for index_number in previous_index_number_list:
                    _split_result.append(deepcopy(_data_object[index_number:index_number + _step_number]))
                # 后一部分
                next_number_list = list(range(previous_end_number, _data_length, _step_number - 1))
                for index_number in next_number_list:
                    _split_result.append(deepcopy(_data_object[index_number:index_number + (_step_number - 1)]))
