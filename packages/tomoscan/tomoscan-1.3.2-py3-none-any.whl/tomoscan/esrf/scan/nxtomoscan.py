    ...
    def detector_transformations(self) -> typing.Optional[tuple]:
        ...
        if self._detector_transformations is None:
            ...
            if self.nexus_version < 1.3:
                ...
                transformations_nexus_paths = (
                    nexus_paths.nx_sample_paths().nx_transformations_paths
                )
                if self.nexus_path.nx_detector_paths.NX_TRANSFORMATIONS is not None:
                    transformations_as_nx_dict = nxtodict(
                        self.master_file, path=self.entry
                    )
