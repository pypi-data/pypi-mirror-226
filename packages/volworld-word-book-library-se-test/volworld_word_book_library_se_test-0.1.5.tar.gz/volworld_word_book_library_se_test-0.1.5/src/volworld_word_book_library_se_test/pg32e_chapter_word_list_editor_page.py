...
def then_words_in_word_list(c, words: str):
    target_page = getattr(c, CotA.TargetPage)
    if PageGroup.is_group(c, PageGroup.Pg32E_Chapter_Word_List_Editor_Page):
        check_word_count_in_32e_page(c, words)
    else:
        assert False, f"Not support page = [{target_page}]"


@then('there are "{words}" unknown words in word list')
def then_unknown_words_in_word_list(c, words: str):
    target_page = getattr(c, CotA.TargetPage)
...
def then_the_word_list_is_sort_by(c, sort_type: str, dir_type: str):
...
def is_word_in_used_word_list(c, text: str):
    if PageGroup.is_group(c, PageGroup.Pg32E_Chapter_Word_List_Editor_Page):
        ...
        assert False, f"Not support page"
...
def then_word_is_in_word_list(c, word: str):
...
def when_click_on_remove_button_of_word(c, mentor: str, word: str):
...
def when_click_on_surprise_icon_of_word(c, mentor: str, word: str):
    click_on_word_row_button(c, word, 1)


@when('"{mentor}" click on [remove icon] of word "{word}" row in [32E_Chapter-Word-List-Editor-Page]')
def when_click_on_remove_icon_of_word(c, mentor: str, word: str):
