...
def then_open_can_not_edit_non_head_ver_book_dialog(c):
    elm = w__get_element_by_shown_dom_id(c, [A.Book, A.Not, A.HeadVersion, A.Dialog])
    assert elm is not None


@when('{mentor} click on [warning circle button] on [bottom app bar]')
...
def then_close_can_not_edit_non_head_ver_book_dialog(c):
