...
mod cost_utils_test {
    ...
    fn gradiant_test() {
        ...
        let mut c = CostFunction::new(usage, &models, false, false);

        let x = array![5., 5., 7., 4.2, 0., 6., 4., 0.];
        let g = c.gradient(x.view());

        
        let mut h = Array2::from_diag(&c.steps);
        h += &x;
        let k = h.map_axis_mut(Axis(1), |row| {
            println!(">>>> {:?}", row);
            c.call(row.view())
        });
