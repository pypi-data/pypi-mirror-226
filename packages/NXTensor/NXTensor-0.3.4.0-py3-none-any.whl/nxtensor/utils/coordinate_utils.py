...
def __generate_mapping_degrees_north_inc_to_dec(resolution):
    values = __generate_float_range(-90, 90+resolution, resolution)
    keys   = values
    return dict(zip(keys, values))


# From -180° (included) / +180° (excluded) translated into 0° (included) / 360° (excluded).
def __generate_mapping_degrees_east_m_180_to_0(resolution):
    dest_values = __generate_float_range(0, 360, resolution)
    keys_1 = __generate_float_range(0, 180, resolution)
    keys_2 = __generate_float_range(-180, 0, resolution)
    result = dict()
    index = 0
    for key in keys_1:
...
# Start included and stop NOT included.
# If start < stop then stop - resolution is included.
# If start > stop then stop + resolution is included.
def __generate_float_range(start, stop, resolution):
    if resolution < 1:
        ...
        if start <= stop:
            ...
            int_range = range(int(start * factor), int(stop * factor), int(resolution * factor * -1))
        result = list()
