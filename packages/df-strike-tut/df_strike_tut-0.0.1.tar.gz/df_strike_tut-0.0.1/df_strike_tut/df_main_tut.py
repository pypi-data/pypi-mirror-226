...
def df_strikeprice_func(client, iDict, Scripmaster):
    ...
    ######### Codes for fetching Banknifty live value into df_strikeprice ##########
    ...
    #print(today)
    banknifty= client.historical_data('N','C',Scripcode_BN,'1m',today,today)
    ...
    ######### Codes for strikes generation for Banknifty into df_strikeprice ##########

    banknifty_ref= round(banknifty["Close"][len(banknifty)-1],-2)
    ...
    for i in range(num_strikes):   ## finding all the strikes referring ATM calculated as above
        df_strikeprice["Strike_Selection"][i]=banknifty_ref - 100*((num_strikes-1)/2-i)
    ...
    Scripmaster_CE=Scripmaster[(Scripmaster.ISIN == "BANKNIFTY")  & (Scripmaster.Expiry == expiry) & (Scripmaster.CpType=="CE")]  ## Setting filter in columns
    Scripmaster_PE=Scripmaster[(Scripmaster.ISIN == "BANKNIFTY") & (Scripmaster.Expiry == expiry) & (Scripmaster.CpType=="PE")] ## Setting filter in columns
    ...
    print("\nBanknifty LTP=",round(banknifty["Close"][len(banknifty)-1],0))
