...
class Whalegistic:
	...
	def getBrands(this, send_obj):
		...
		brands_obj = requests.post(url = send_url, data = send_obj, headers = send_headers)
	...
	def getProducts(this, send_obj):
		...
		products_obj = requests.post(url = send_url, data = send_obj, headers = send_headers)

		print(f'PRODUCTS : {products_obj}')
