...
model_path = pkg_resources.resource_filename('picta_gui', 'data/08_17.keras')
...
def display_segmentation():
    ...
    segment_image = giu.apply_model_to_image(model, 128, file_path)
    ...
    for i in range(len(jpg_files)):
        try:
            if '.jpg' in jpg_files[i]:
                ...
                mask_png = giu.apply_model_to_image(model, 128, os.path.join(input_directory, base_filename+'.jpg'))
        ...
        if not cancel_image_process:
            try:
                if '.NEF' in raw_files[i]:
                    ...
                    mask_png = giu.apply_model_to_image(model, 128, os.path.join(output_directory, base_filename+'.jpg'))
                    mask_png.save(os.path.join(output_directory, base_filename+'_masked.png'), 'PNG') #Second arg of 'PNG' ensures image will be saved with PNG encoding
...
def pick_new_segmentation():
    ...
    def __create_cropped_sections(path_to_image):
        ...
        tensor_left_or_top = giu.apply_model_to_tensor(left_or_top, model)
        ...
        tensor_middle = giu.apply_model_to_tensor(middle, model)
        segment_middle = tf.keras.utils.array_to_img(tensor_middle)
        tensor_right_or_bottom = giu.apply_model_to_tensor(right_or_bottom, model)
    ...
    def __resize_segment_to_image_size(segment, offset, scaled_image):
        ...
        if scaled_image.width > scaled_image.height:
            filled_canvas = giu.copy_single_segment_to_canvas(segment, offset, 0, canvas)
        else:
            filled_canvas = giu.copy_single_segment_to_canvas(segment, 0, offset, canvas)
