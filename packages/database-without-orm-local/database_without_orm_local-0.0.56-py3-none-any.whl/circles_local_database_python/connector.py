...
class Connector:
    ...
    def get_connection(schema_name: str):
        ...
        if schema_name in connections_pool:
            logger.end("Return existing connections_pool", object={'connections_pool': str(connections_pool)} )
            return connections_pool[schema_name]
        else:
            connector=Connector(schema_name)
            connections_pool[schema_name]=connector
            logger.end("Return connections_pool with a new connection", object={'connections_pool': str(connections_pool)} )
            return connector.connect()
    
    
    
    def __init__(self, schema_name, host=None, user=None, password=None):
        ...
        self.database = os.getenv("RDS_DATABASE") or schema_name
    ...
    def cursor(self, dictionary=False,buffered=False):
        ...
        cursor_instance = Cursor(self.connection.cursor(dictionary=dictionary,buffered=buffered))
    ...
    def set_schema(self, new_schema):
        ...
        if self.connection and self.connection.is_connected():
            try:
                self.cursor.execute(f"USE {new_schema};")
