...
class CollectionServiceClient(StandardCollectionServiceClient):
    ...
    def patch(self,
        ...
        if id is not None:
            if isinstance(collection, Collection):
                raise AmbiguousArgumentsError('Only require either a collection ID OR object, but not both, at the '
                                              'same time.')
            if not attrs:
                raise AssertionError('The overriding attributes must be defined.')
        else:
            if attrs:
        ...
        update_patches = [
            ...
            if k not in COLLECTION_READ_ONLY_PROPERTIES
        ]
        ...
        with self.create_http_session() as session:
            ...
            assert etag, 'ETag is not provided. Unable to update the collection.'
