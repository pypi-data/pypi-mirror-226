        ...
        for which_end in hists.keys():
            for b in range(bins):
                data.append([name, hists[which_end].name, which_end, b,
                             hists[which_end].hist[b],
                             hists[which_end].total_count(),
                             total_in_bed, lib_count])
    df = pd.DataFrame(data, columns=['file', 'ref', 'which_end', 'bin',
                                     'count', 'sum_count', 'total_in_bed',
                                     'lib_count'])

    df['density'] = df['count'] / df['sum_count']
    df['density_weighted'] = df['density'] * (df['total_in_bed'] / df['lib_count'])

    return df


def process_final_results(results, args):
    ...
    # Create figures.
    figs = []
    figs.append(create_fig_for_df(df[df['which_end'] == '5p'], 'density', '5p'))
    figs.append(create_fig_for_df(df[df['which_end'] == '3p'], 'density', '3p'))
    figs.append(create_fig_for_df(df[df['which_end'] == '5p'], 'density_weighted', '5p'))
    figs.append(create_fig_for_df(df[df['which_end'] == '3p'], 'density_weighted', '3p'))
    save_figs_in_pdf(figs, args.pdf)

    # Print the data frame with histograms to a file.
    df.to_csv(args.ohist, sep="\t", index=False)
...
def main():
    ...
    process_final_results(dataframes, args)
