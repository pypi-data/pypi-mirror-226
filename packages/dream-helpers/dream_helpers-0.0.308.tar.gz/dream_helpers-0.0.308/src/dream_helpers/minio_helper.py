...
def add_to_minio_and_psql(
    ...
    try:
        ...
        with open(dataset_name, 'r') as f:
            logger.info(" sql --> skipping first %s rows" % str(offset))
            for _ in range(offset):
                next(f) 
            cur.copy_from(f, table_name, sep=',')
            logger.info(" sql --> copied from dataset to: %s" % table_name)
        cur.close()
...
list_cols = {
        "field_name_1" : "varchar",
        "field_name_2" : "varchar",
        "field_name_3" : "varchar",
        "field_name_4" : "varchar",
        "field_name_5" : "varchar",
        "field_name_6" : "varchar",
        "field_name_7" : "varchar",
        "field_name_8" : "varchar"
}.items()

# (Re)create a table in postgresql
pgsql_endpoint = "localhost"
pgsql_client = get_pgsql_auth_session(pgsql_endpoint)
conn = pgsql_client
conn.autocommit = True
