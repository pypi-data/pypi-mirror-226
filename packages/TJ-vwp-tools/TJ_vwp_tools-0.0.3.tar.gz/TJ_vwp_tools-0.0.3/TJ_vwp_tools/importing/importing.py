    ...
    class GEOKON_VWP:
        ...
        def __init__(self,vwp_fpath,metadata_fpath,**kwargs):
            ...
            class plotting:
                ...
                def timeseries(self, field_name = 'water_elevation', save_fig = False, fname = None):
                    """
                    ...
                    save_fig : bool, optional
                        True to save the plot as a png file. Default is False
                    ...
                    if save_fig:
                        if not fname:
                            fname = self._vwp.name + f'_{field_name}_timeseries.png'
        ...
        def to_csv(self,fname = None):
            ...
            for i in range(self.n_sensors):
                data[f'water_level_{str(i+1).zfill(2)}'] = self.sensor_data['water_level'][i,:]
                
                data[f'water_elevation_{str(i+1).zfill(2)}'] = self.sensor_data['water_elevation'][i,:]
                

            df = pd.DataFrame(data = data, index = self.timestamps)
