...
class Stream:
    ...
    '''
    def __init__(self, filename : str,masks : list = [],interpolation_masks : list = [],angle : float = 0):
        '''
    ...
    def __call__(self):
        ...
        if len(self._interpolation_masks) != 0:
            ...
            if not isinstance(self._interpolation_masks, list):
                raise TypeError(f"'interpolation_masks' of non-list type {type(self._interpolation_masks)}")
            ...
            for mask in self._interpolation_masks:
                ...
                if not self.data.shape == mask.shape:
                    raise ValueError(f"Mask of wrong shape (expected {self.data.shape}, got {mask.shape})")
                
                self.data = util.multiply_mask(data = self.data, mask = mask)
                self.interpolation_mask = util.multiply_mask(data = self.interpolation_mask, mask = mask)
