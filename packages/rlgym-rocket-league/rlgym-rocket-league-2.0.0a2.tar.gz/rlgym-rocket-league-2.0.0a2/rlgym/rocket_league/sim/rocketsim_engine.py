...
class RocketSimEngine(TransitionEngine[AgentID, GameState, np.ndarray]):
    ...
    def __init__(self):
        ...
        # Optimizations
        self._controls = rsim.CarControls()
    ...
    def step(self, actions: Dict[AgentID, np.ndarray]) -> GameState:
        ...
        for agent_id, car in self._cars.items():
            ...
            controls.throttle = action[0, 0]
            controls.steer = action[0, 1]
            controls.pitch = action[0, 2]
            controls.yaw = action[0, 3]
            controls.roll = action[0, 4]
            controls.jump = bool(action[0, 5])
            controls.boost = bool(action[0, 6])
            controls.handbrake = bool(action[0, 7])

            car.set_controls(controls)

        self._arena.step(steps)
