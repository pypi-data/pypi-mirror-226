...
class Bank:
    ...
    def _kelly_formula(
        ...
        g = self.gain_rate if not gain else self.gain_rate
        return b * ((c * (g + 1) - 1) / g)
    ...
    def act(self, execution: base_strategy.Execution, asset_name: str, target: float, volume: float = 0) -> None:
        ...
        if asset_name not in self.hold_asset:
            self.hold_asset[asset_name] = Holder(asset_name)
        
        asset: Holder = self.hold_asset[asset_name]
