...
class BaseAgent(KoiledModel):
    ...
    async def abuild_actor_for_template(
        self, template: TemplateFragment, passport: Passport, transport: AgentTransport
    ) -> Actor:
        if not template.extensions:
            try:
                ...
                raise ProvisionException(
                    "No Actor Builder found for template and no extensions specified"
                ) from e
        ...
        if not actor:
            raise ProvisionException("No extensions managed to spawn an actor") from e
    ...
    async def aspawn_actor_from_provision(self, provision: Provision) -> Actor:
        ...
        )  # Forwards messages to the agent transport
